...
// Build SQL
$sSql = "SELECT credito.credito_num, vencimiento.vencimiento_num, vencimiento.importe as impvenc, vencimiento.interes, vencimiento.iva, vencimiento.interes_moratorio, vencimiento.iva_mor, vencimiento.total_venc, recibo.*, solicitud.solicitud_id 
...
on recibo.recibo_id = recibo_vencimiento.recibo_id join solicitud on solicitud.solicitud_id = credito.solicitud_id ";
...
if($_POST["x_medio_pago_id"]){
	$_SESSION["x_medio_pago_id"] = $_POST["x_medio_pago_id"];
	$sDbWhere .= "(recibo.medio_pago_id = ".$_SESSION["x_medio_pago_id"].") AND ";
}else{
	if(empty($_SESSION["x_medio_pago_id"]) || $_SESSION["x_medio_pago_id"] == ""){
...
if($_SESSION["x_credito_num_filtro"] != ""){
	$sDbWhere .= "(credito.credito_num+0 = ".$_SESSION["x_credito_num_filtro"].") AND ";
}
...
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_rpt_pagos_old.php?export=excel&x_credito_num_filtro=<?php echo $x_credito_num_filtro; ?>&x_fecha_desde=<?php echo FormatDateTime(@$x_fecha_desde,7); ?>&x_fecha_hasta=<?php echo FormatDateTime(@$x_fecha_hasta,7); ?>">Exportar a Excel</a><?php } ?>
</span></p>
<p>
<?php
if (@$_SESSION["ewmsg"] <> "") {
...
<form action="php_rpt_pagos_old.php" name="filtro" id="filtro" method="post">
...
if ($rswrk) {
	...
	while ($datawrk = phpmkr_fetch_array($rswrk)) {
		...
		if ($datawrk["medio_pago_id"] == @$_SESSION["x_medio_pago_id"]) {
			$x_medio_pago_idList .= " selected";
		}
...
?>
	...
	<tr>
	  ...
	  <td><a href="php_rpt_pagos_old.php?cmd=reset">Mostrar Todos</a></td>
	  </tr>
	<tr>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
...
<?php if ($sExport == "") { ?>
<form action="php_rpt_pagos_old.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_rpt_pagos_old.php?start=<?php echo $PrevStart; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_rpt_pagos_old.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_rpt_pagos_old.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_rpt_pagos_old.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_rpt_pagos_old.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_rpt_pagos_old.php?start=<?php echo $NextStart; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
