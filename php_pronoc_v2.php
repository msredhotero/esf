...
$sSqlPENLIZACIONES = "SELECT vencimiento.* FROM vencimiento, credito where credito.credito_id  = vencimiento.credito_id and credito.credito_status_id in (1,4) and vencimiento.fecha_vencimiento < '$currdate' and vencimiento.vencimiento_status_id in (7,1)  and vencimiento_num > 2000 order by vencimiento.vencimiento_num";// (1064,2499)  ";
echo $sSqlPENLIZACIONES."<br><br>";
$rswrkPena = phpmkr_query($sSqlPENLIZACIONES,$conn) or die("error el query 1".phpmkr_error()."ql. :");
while($datawrkPena = phpmkr_fetch_array($rswrkPena)){
	$x_vencimiento_id = $datawrkPena["vencimiento_id"];
		$sSqlWrk = "update vencimiento set vencimiento_status_id = 3 where vencimiento_id = $x_vencimiento_id ";
		phpmkr_query($sSqlWrk,$conn) or die ("ERROR AL CATULIZAR LAS PENALIZACIONES".phpmkr_error()."sql: ".$sSqlWrk);
	echo $sSqlWrk."<br>";	
}


$sSqlPENLIZACIONES = "SELECT vencimiento.* FROM vencimiento, credito where credito.credito_id  = vencimiento.credito_id and credito.credito_status_id in (1,4) and vencimiento.fecha_vencimiento < '$currdate' and vencimiento.vencimiento_status_id in (6) order by vencimiento.vencimiento_num";// (1064,2499)  ";
echo $sSqlPENLIZACIONES."<br><br>";
$rswrkPena = phpmkr_query($sSqlPENLIZACIONES,$conn) or die("error el query 1".phpmkr_error()."ql. :");
while($datawrkPena = phpmkr_fetch_array($rswrkPena)){
	$x_vencimiento_id = $datawrkPena["vencimiento_id"];
...
echo "<BR><BR> ***********************************CREDITO CONDICIONES DE ".$sqlUpdateCond."<BR>";
