...
switch ($sAction)
{
	...
	case "A": // Add
		...
		promotor_comision.credito_id = $x_credito_id
		";
		...
		if($x_pagada > 0){
			$_SESSION["ewmsg"] = "La comision ya existe.";						
		}else{
...
if (validada == true && EW_this.x_credito_id && !EW_hasValue(EW_this.x_credito_id, "SELECT" )) {
	if (!EW_onError(EW_this, EW_this.x_credito_id, "SELECT", "El credito es requerido."))
		validada = false;
}
...
<?php 
if (!(!is_null($x_credito_id)) || ($x_credito_id == "")) { $x_credito_id = 0;} // Set default value 
$x_solicitud_idList = "<select name=\"x_credito_id\" >";
$x_solicitud_idList .= "<option value=''>Seleccione</option>";
$sSqlWrk = "SELECT credito_id, credito_num FROM credito join solicitud on solicitud.solicitud_id = credito.solicitud_id where solicitud.promotor_id = $x_promotor_id and credito.credito_status_id = 3";
$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
if ($rswrk) {
	$rowcntwrk = 0;
	while ($datawrk = phpmkr_fetch_array($rswrk)) {
		...
		$x_solicitud_idList .= ">" . $datawrk["credito_num"] . "</option>";
		$rowcntwrk++;
	}
}
@phpmkr_free_result($rswrk);
...
?>
	...
	<tr>
      ...
      <input type="text" name="x_comision_importe" class="importe" value="<?php echo htmlspecialchars(@$x_comision_importe) ?>" size="10" maxlength="10" onKeyPress="return solonumeros(this,event)"/>
      </td>
	  </tr>
	<tr>
