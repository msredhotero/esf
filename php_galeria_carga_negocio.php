...
<table width="700" border="0" align="center">
  ...
  <tr background="images/headTUpload1.jpg" id="file10"  class="clonedInput">
    ...
    <td colspan="2">
     ...
     <?php if($x_edita_fotos_existententes == 0){?>
        ...
        <input type="radio" name="a_x_negocio_10" value="3">Cambiar<br />
            <?php }?>
        <?php } else {?>
        <input type="hidden" name="a_x_negocio_10" value="3">
  ...
  <tr background="images/headTUpload2.jpg">
    <td colspan="2">&nbsp;</td>
    <td width="320"><input type="button" name="neg" id="neg"  onclick="checkMyForm();" value="Guardar" /><div> <input type="button" name="neg" id="neg"  onclick="checkMyForm();" value="Cargar 1 foto mas" /></div></td>
    </tr>
</table>
<?php }?>
</form>
	...
	$fieldList = NULL;
		...
		if ($a_x_negocio_1 == "2") { // Remove
			...
			// Field IFE 2
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_2"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_2"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_2_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					
							if (!move_uploaded_file($_FILES["x_foto_negocio_2"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			$fieldList["`foto_negocio_2`"] = " '" . $theName . "'"; 					
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_3"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_3"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_3_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_3"]["tmp_name"],$nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			$fieldList["`foto_negocio_3`"] = " '" . $theName . "'"; 					
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_4"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_4"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_4_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_4"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			$fieldList["`foto_negocio_4`"] = " '" . $theName . "'"; 					
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_5"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_5"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_5_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_5"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes($x_nuevo_nombre) :  $x_nuevo_nombre;
	 ...
	 if ($a_x_negocio_6 == "2") { // Remove
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_6"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_6"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_6_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_6"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			...
			// Field IFE 2
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_7"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_7"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_7_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_7"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			$fieldList["`foto_negocio_7`"] = " '" . $theName . "'"; 					
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_8"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_8"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_8_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_8"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			$fieldList["`foto_negocio_8`"] = " '" . $theName . "'"; 					
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_9"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_9"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_9_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					 
					 
							if (!move_uploaded_file($_FILES["x_foto_negocio_9"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
			...
			// Field IFE 6
				...
				if (is_uploaded_file($_FILES["x_foto_negocio_10"]["tmp_name"])) {
					...
					//aqui es donde debo genera el nombre del archivo
					 #copiamos la extension del archivo					 
					 $ext = explode('.',$_FILES["x_foto_negocio_10"]["name"]);
					 $extension = $ext[1];					 
					 $x_nuevo_nombre = "FOTO_NEGOCIO_10_".strtoupper($x_nombre_galeria).".".$extension;	
					 $nuevo_destfile = ewUploadPath(1).$x_nuevo_nombre;	
					
							if (!move_uploaded_file($_FILES["x_foto_negocio_10"]["tmp_name"], $nuevo_destfile)) // move file to destination path
							die("You didn't upload a file or the file couldn't be moved to" . $nuevo_destfile);
				// File Name
			$theName = (!get_magic_quotes_gpc()) ? addslashes( $x_nuevo_nombre) :  $x_nuevo_nombre;
