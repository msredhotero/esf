...
if (($sAction == "") || (is_null($sAction))) {
	...
	// permite modicicacion 
	$x_readonly = $_POST["x_readonly"];
	
	//cuerpo mail
	$x_cuerpo_mail = @$_POST["x_cuerpo_mail"];
	$x_destinatario_mail = @$_POST["x_destinatario_mail"];
	...
	$x_zona_id = @$_POST["x_zona_id"];
	$x_tarjeta_referenciada = @$_POST["x_tarjeta_referenciada"];
	$x_numero_cheque= @$_POST["x_numero_cheque"];
	$x_monto_maximo_aprobado = @$_POST["x_monto_maximo_aprobado"];
	
	// se garega el campo de vendedor
	...
	$x_dia_otorga_credito = @$_POST["x_dia_otorga_credito"];
...
//en direccion
	...
	$x_actualiza_fecha_supervision = @$_POST["x_actualiza_fecha_supervision"];
...
?>
        ...
        <td>&nbsp;</td>
        <td> <div align="right"><span class="texto_normal">Vendedor</span>:</div></td>
        <td><div class="phpmaker">
            <?php
		$x_estado_civil_idList = "<select name=\"x_vendedor_id\" $x_readonly2 class=\"texto_normal\">";
		$x_estado_civil_idList .= "<option value=''>Seleccione</option>";
		if($_SESSION["crm_UserRolID"] == 7) {
		...
		if ($rswrk) {
			...
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_estado_civil_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["vendedor_id"] == @$x_vendedor_id) {
					$x_estado_civil_idList .= "' selected";
				}
				$x_estado_civil_idList .= ">" . htmlentities($datawrk["nombre_completo"]) . "</option>";
				$rowcntwrk++;
...
?></td>
      ...
      <tr>
        <td><span class="texto_normal"><?php if ($x_solicitud_status_id < 12){ echo "Importe solicitado:";}else{ echo "Monto otorgado";}?> </span></td>
      ...
      <tr>
        <td><span class="texto_normal">Monto maximo a otorgar</span></td>
        <td><input type="text" class="importe"  size="10" maxlength="10" name="x_monto_maximo_aprobado" value="<?php echo  FormatNumber($x_monto_maximo_aprobado,0,0,0,0);?>"  onchange="muestraMontoSolicitado();" <?php echo $x_readonly;?> /> </td>
        <td>&nbsp;</td>
      ...
      <tr>
        <td><span class="texto_normal">Monto solicitado</span></td>
        <td><input class="importe"  size="10" maxlength="10" type="text" name="x_monto_solicitado"  <?php echo $x_readonly;?> id="x_monto_solicitado" value="<?php if ($x_solicitud_status_id == 2 and $_SESSION["php_project_esf_status_UserRolID"] != 2 ){echo "----";}else{ echo  FormatNumber(@$x_monto_solicitado,0,0,0,0);}?>" /></td>
        <td>&nbsp;</td>
        <td style="color:#00F"><?php if ($x_solicitud_status_id == 3 || $x_solicitud_status_id == 12 ){ ?><div align="right"><strong>Indique el d&iacute;a en que se otorgar&aacute; el cr&eacute;dito</strong></div><?php }?></td>
...
echo $x_delegacion_idList;
  ...
  <tr>
    ...
    <tr>
      ...
      <td colspan="5">&nbsp;N&uacute;mero exterior&nbsp;&nbsp;<input type="text" name="x_numero_exterior2" id="x_numero_exterior2" value="<?php echo ($x_numero_exterior);?>"  maxlength="20" size="20"/></td>
    </tr>
    <tr>
      <td>Colonia</td>
...
echo $x_delegacion_idList;
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
   ...
   <tr>
    ...
    <?php if($x_solicitud_status_id == 9 and ($_SESSION["php_project_esf_status_UserRolID"] == 12 ||  $_SESSION["php_project_esf_status_UserRolID"] == 1|| $_SESSION["php_project_esf_status_UserRolID"] == 7 )){?>
  ...
  <tr>
    ...
    <td align="left" valign="top">
	  <div align="center">
       <?php if($x_solicitud_status_id == 12 and ($_SESSION["php_project_esf_status_UserRolID"] == 12 ||  $_SESSION["php_project_esf_status_UserRolID"] == 1||  $_SESSION["php_project_esf_status_UserRolID"] == 2)){?>     
	    <textarea name="x_comentario_comite" cols="100" rows="5" ><?php echo $x_comentario_comite; ?></textarea>
        <?php }else{?>        
...
<?php if(($_SESSION["php_project_esf_status_UserRolID"] == 1) || ($_SESSION["php_project_esf_status_UserRolID"] == 2) || ($_SESSION["php_project_esf_status_UserRolID"] == 3) ||($_SESSION["php_project_esf_status_UserRolID"] == 4) ||($_SESSION["php_project_esf_status_UserRolID"] == 7)||($_SESSION["php_project_esf_status_UserRolID"] == 12) ){ ?>
 <?php if(($x_solicitud_status_id != 6) || (($_SESSION["php_project_esf_status_UserRolID"] == 1) || ($_SESSION["php_project_esf_status_UserRolID"] == 2) || ($_SESSION["php_project_esf_status_UserRolID"] == 3)) || ($_SESSION["php_project_esf_status_UserRolID"] == 12) ){ ?>
...
if($GLOBALS["x_visita_id_3"] != 0 ){
		...
		$fieldList["`comentario_comite`"] = $theValue;
			...
			if(strlen($GLOBALS["x_comentario_comite"]) > 10){			
	...
	// Varios destinatarios
		...
		// Mail it						
		//die();
		$para2= "zortiz@financieracrea.com";
		mail($para, $titulo, $x_mensaje, $cabeceras);	
		mail($para2, $titulo, $x_mensaje, $cabeceras);			
			
			}	
	
		if((intval($GLOBALS["x_solicitud_status_id"]) == 9)&&($GLOBALS["x_actualiza_fecha_supervision"] == "si")){		
