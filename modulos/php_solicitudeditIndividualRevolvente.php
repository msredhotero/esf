...
numer_pag = EW_this.x_plazo_id.value;
				 if((numer_pag > 1)){
					 ...
					 alert("El numero de pago es incorreto verifique por favor, ES A 1 PAGO");			 
					 validada = false;
					 }
 }
...
function valorMax(elemento){
				...
				numer_pag = document.getElementById("x_plazo_id").value;
				 if((numer_pag > 1)){
					 // el numero de pagos es incorrecto deben ser minimo 2 maximo 88
					 alert("El numero de pago es incorreto verifique por favor,DEBE SER 1 PAGO ");					 
    ...
    <td colspan="3" align="left" valign="top"><table width="674" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <td>
            ...
            <?php		$x_estado_civil_idList = "<select name=\"x_solicitud_status_id\"  class=\"texto_normal\">";
		$x_estado_civil_idList .= "<option value=''>Seleccione</option>";	
		$sSqlWrk = "SELECT `solicitud_status_id`, `descripcion` FROM `solicitud_status` where solicitud_status_id  in (1,3,5,10) ";		
		#echo $sSqlWrk;
		$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
		if ($rswrk) {
			$rowcntwrk = 0;
...
if($GLOBALS["x_visita_id_3"] != 0 ){
	...
	// Varios destinatarios
		...
		$sSql .= " WHERE " . $sWhere;
		echo "<BR> UPDATE SOLICITUD ".$sSql."<BR>";
		if(empty($x_readonly)){	
		echo "entra en readonly";
		$x_result = phpmkr_query($sSql,$conn);
		if(!$x_result){
			echo phpmkr_error() . '<br> errorSQL: ' . $sSql;
			phpmkr_query('rollback;', $conn);
