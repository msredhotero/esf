...
include("utilerias/gen_pass_ec.php");
	...
	$responseGid = phphmkr_query($sqlGrupo, $conn) or die("Error al seleccionar grupo id".phpmkr_error()."sql:".$sqlGrupo);
	...
	if(!empty($x_grupo_id)){
		...
		$resposeClientes = phphmkr_query($sqlClientes, $conn) or die("Error a seleccionar los clientes del grupo".phpmkr_error()."sql:".$sqlClientes);
		$rowClientes = phpmkr_fetch_array($resposeClientes);
			...
			while($x_cont_g <= 10){
				...
				if ( !empty($GLOBALS["x_cliente_id_$x_cont_g"]) && ($GLOBALS["x_cliente_id_$x_cont_g"] != "") && intval($GLOBALS["x_cliente_id_$x_cont_g"])){
					...
					// seleccionamos el numero de cliente
					$sqlnc = "SELECT  cliente_num  WHERE cliente_id = ". $GLOBALS["x_cliente_id_$x_cont_g"]."";
					$responsenc = phpmkr_query($sqlnc, $conn) or die("Error al seleccionar el numero de credito".phpmkr_error()."sql:".$sqlnc);
					$rownc = phpmkr_fetch_array($responsenc);
					$x_cliente_num =  $rownc["cliente_num"];
