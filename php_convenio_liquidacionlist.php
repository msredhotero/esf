...
$sSql = "SELECT  convenio_liquidacion.*, promotor.nombre_completo as promotor,promotor.sucursal_id,sucursal.nombre as suc from  convenio_liquidacion join promotor on promotor.promotor_id = convenio_liquidacion.usuario_id  join sucursal on promotor.sucursal_id = sucursal.sucursal_id   ";
...
?>
<p><span class="phpmaker">Convenio Liquidaci&oacute;n
  <?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_convenio_liquidacionlist.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="php_convenio_liquidacionlist.php?export=word">Export to Word</a>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
<p>
...
<form action="php_convenio_liquidacionlist.php" name="ewpagerform" id="ewpagerform">
<table width="785" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
	  ...
	  <td>&nbsp;</td>
	  <td><span class="phpmaker"><a href="php_convenio_lidacionlist.php?cmd=reset">Mostrar Todos</a>&nbsp;&nbsp; </span></td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_convenio_liquidacionlist.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_convenio_liquidacionlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_convenio_liquidacionlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_convenio_liquidacionlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_convenio_liquidacionlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
			  <a href="php_convenio_liquidacionlist.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
ID
<?php }else{ ?>
	<a href="php_convenio_liquidacionlist.php?order=<?php echo urlencode("convenio_liquidacion_id"); ?>" style="color: #FFFFFF;" onMouseDown="ewsort(event, this.href);">ID<?php if (@$_SESSION["convenio_liquidacion_x_convenio_liquidacion_id_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["convenio_liquidacion_x_convenio_liquidacion_id_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</span></td>
        	<td valign="top"><span>
<?php if ($sExport <> "") { ?>
...
<td><span>
<?php echo $monto_liquida; ?>
</span></td>
<td><span>
<?php echo $fecha_liquida; ?>
</span></td>
<td><span>
...
<td>
<?php if(($convenio_liquidacion_id> 0)){
...
<span>
		<div id="capaConvenioLiquidacion_<?php echo $convenio_liquidacion_id;?>"><input type="checkbox" name="convenio_liquidacion<?php echo $convenio_liquidacion_id; ?>" <?php echo $x_disable; ?> <?php echo $x_checked;?> onclick="ConvenioLiquidacion(<?php echo $convenio_liquidacion_id; ?>);"  /></div><?php } ?>
