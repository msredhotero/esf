...
// Build SQL
$sSql = "SELECT * FROM folio_factura ";
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount >= $nStartRec) {
		...
		$x_total = 0;
		$x_total_iva = 0;
		$sqlMovimientos = "SELECT monto, tasa FROM detalle_factura WHERE folio_factura = $x_folio and  descripcion = \"INTORD\"";		
		...
		$x_ordinarios = $rowMovimientos["monto"];
		
		$x_tasa = $rowMovimientos["tasa"];		
		$sqlMovimientos = "SELECT monto, tasa FROM detalle_factura WHERE folio_factura = $x_folio and  descripcion = \"INTMOR\"";
		$rsMovimiento = phpmkr_query($sqlMovimientos,$conn) or die ("Error la seeccionar los mivimientos del folio".phpmkr_error()."sql: ".$sqlMovimientos );
		$rowMovimientos = phpmkr_fetch_array($rsMovimiento);
		$x_moratorios = $rowMovimientos["monto"];
				
		$sqlMovimientos = "SELECT monto, tasa FROM detalle_factura WHERE folio_factura = $x_folio and  descripcion = \"COMCOB\"";		
		$rsMovimiento = phpmkr_query($sqlMovimientos,$conn) or die ("Error la seeccionar los mivimientos del folio".phpmkr_error()."sql: ".$sqlMovimientos );
		$rowMovimientos = phpmkr_fetch_array($rsMovimiento);
		$x_comision = $rowMovimientos["monto"];
		
		$x_total =  $x_comision + $x_moratorios + $x_ordinarios;
...
<td align="center"><?php echo FormatNumber($x_ordinarios,2,0,0,1);?></td>
		<td align="left"><?php echo  FormatNumber($x_moratorios,2,0,0,1);?></td>
<td align="left"><?php echo  FormatNumber($x_comision,2,0,0,1);?></td>
