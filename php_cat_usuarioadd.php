...
switch ($sAction)
{
	...
	case "A": // Add
		...
		if (AddData($conn, $x_cliente_id)) { // Add New Record
			echo "
			<script type=\"text/javascript\">
			<!--
			window.opener.document.clienteview.submit();
			window.close();
			//-->
			</script>";
		}else{
...
<form name="usuarioadd" id="usuarioadd" action="php_cat_usuarioadd.php" method="post" onSubmit="return EW_checkMyForm(this);">
<p>
...
$sSqlWrk = "SELECT `usuario_rol_id`, `descripcion` FROM `usuario_rol` where descripcion = 'CLIENTE'";
...
function AddData($conn, $x_cliente_id)
{
	...
	$sSql = "update cliente set usuario_id = $x_usuario_id where cliente_id = ".$GLOBALS["x_cliente_id"];
	$x_result = phpmkr_query($sSql, $conn);
	if(!$x_result){
		echo phpmkr_error() . '<br>SQL: ' . $sSql;
		phpmkr_query('rollback;', $conn);	 
