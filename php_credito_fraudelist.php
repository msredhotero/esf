...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=credito_FRAUDE.xls');
}
if ($sExport == "word") {
	header('Content-Type: application/vnd.ms-word');
	header('Content-Disposition: attachment; filename=credito_FRAUDE.doc');
}
?>
<?php include ("db.php") ?>
...
#fecha hoy menos dos meses

$sqlFechaFraude = "SELECT DATE_SUB('$x_fecha_hoy', INTERVAL 2 MONTH) AS  fecha_fraude";
$rsFechaFraude =phpmkr_query($sqlFechaFraude, $conn) or die ("Erroe al seleccionar la fecha de fraude". phpmkr_error()."sql:".$sqlFechaFraude);
$rowFcehaFraude = phpmkr_fetch_array($rsFechaFraude);
$x_fecha_fradue = $rowFcehaFraude["fecha_fraude"];


#echo "fecha_fraude ".$x_fecha_fradue."<br>";

// seleccionamos todos los  credito activos que tengan fecha de otorgamiento menor a la fecha de fraude
$x_lista_credito_sin_pagos = "";
$sqlCreditoMenor = "SELECT credito_id FROM  credito WHERE fecha_otrogamiento <= \"$x_fecha_fradue\" AND  	credito_status_id IN (1) ";
$rsCreditoMenor = phpmkr_query($sqlCreditoMenor, $conn) or die ("Error al seleccionar los credito menores". phpmkr_error()."sql:".$sqlCreditoMenor);
#echo "sql.".$sqlCreditoMenor;
while($rowCreditoMenor = mysql_fetch_array($rsCreditoMenor)){
	$x_crediot_id_menor = $rowCreditoMenor["credito_id"];
	//echo "credito_id".$x_crediot_id_menor.", ";	
	$x_sin_pago = 1;
	#seleccionamos los vencimeintos del credito para ver si tiene laguno pagado
	$sqlVenVen = " SELECT vencimiento_status_id FROM vencimiento WHERE credito_id = $x_crediot_id_menor ";
	$rsVenVen = phpmkr_query($sqlVenVen, $conn) or die ("Error al selccioanr los vencimeinto". phpmkr_error()."sql:".$sqlVenVen);
	while($rowVenVen = phpmkr_fetch_array($rsVenVen)){
		$x_status_vencimeinto = $rowVenVen["vencimiento_status_id"];
		if ($x_status_vencimeinto == 2){
			// si en algun vencimeito encuantra un pago, cambia el valor de la variable para que ya no se tome en cuenta ese credito en la lista.
			$x_sin_pago = 0;			
			}
		
		
		}// while vencimiento
	#termina de recorrer todos los vencimientos, si la variable sigue en 1 entonces el credito no tiene ningun pagao registrado y se debe incluir en  la liste de los d credtos a buscar
	  if ($x_sin_pago == 1){
		  # inclimos el credito en la lista
		  $x_lista_credito_sin_pagos .= $x_crediot_id_menor. ", ";
		  
		  }// if
	} // terminamos de recorrer todos los creditos
	
	$x_lista_credito_sin_pagos = trim($x_lista_credito_sin_pagos, ", ");
...
if ($sWhere != "") {
	$sSql .= " WHERE credito_id in ($x_lista_credito_sin_pagos) and " . $sWhere;
}else{
	$sSql .= " WHERE  credito_id in ($x_lista_credito_sin_pagos) " ;
...
?>
<p><span class="phpmaker">CREDITOS FRAUDE
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_credito_fraudelist.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;<a href="php_credito_fraudelist.php?export=word">Exportar a Word</a>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
<form action="php_credito_fraudelist.php" name="filtros" method="post">
...
<!-- Grupo --></td>
	...
	<tr>
	  ...
	  <td>&nbsp;</td>
	  <td><span class="phpmaker"><a href="php_credito_fraudelist.php?cmd=reset">Mostrar Todos</a>&nbsp;&nbsp; </span></td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
...
?>
<?php if ($sExport == "") { ?>
<form action="php_credito_fraudelist.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_credito_fraudelist.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_credito_fraudelist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_credito_fraudelist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_credito_fraudelist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_credito_fraudelist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_credito_fraudelist.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
Cliente
<!---
	<a href="php_credito_fraudelist.php?order=<?php //echo urlencode("cliente"); ?>">Cliente<?php //if (@$_SESSION["credito_x_cliente_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php //} elseif (@$_SESSION["credito_x_cliente_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php //} ?></a>
-->    
<?php } ?>
		</span></td>		
