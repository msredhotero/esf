...
$x_integrante_id = @$_GET["integrante_id"];
...
//CREDITO
$sSql = "SELECT credito.credito_id, credito.tarjeta_num, fecha_vencimiento, referencia_pago, tasa, tasa_moratoria, fecha_otrogamiento FROM credito where solicitud_id = $x_solicitud_id";
$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
$row = phpmkr_fetch_array($rs);
$x_cheque_valor = $row["referencia"];
$x_tasa_valor = $row["tasa"];
...
if($GLOBALS["x_creditoSolidario_id"]>0){
	$x_lista_acreditados =	"<table class='ewTable' align='center' border=1 cellspacing=0 cellpadding=2 >";
	...
	while ($x_cont_int <= 10){
		...
		$x_lista_acreditados .= "<tr><td> $x_cont_int.-</td><td> $x_nombre_completo_int </td> <td>______________________________</td></tr>";
...
//INTEGRANTES DEL GRUPO
if($GLOBALS["x_creditoSolidario_id"]>0){
	$x_lista_acreditados =	"<table class='ewTable' align='center' >";
	//$x_lista_acreditados .= "<tr>";
	$x_cont_int = 1; //$GLOBALS["x_cliente_id_$x_cont_g"]
	while ($x_cont_int <= 10){
		$x_cliente_id_act = "x_cliente_id_$x_cont_int";
		...
		$GLOBALS["x_nombre_integrante_$x_cont_int"] =  $rowInT["nombre_completo"];
		$GLOBALS["x_nombre_completo_$x_cont_int"] = $rowInT["nombre_completo"]." ".$rowInT["apellido_paterno"]." ".$rowInT["apellido_materno"];
		phpmkr_free_result($rowInT);
		
		
		$x_lista_acreditados .= "<tr><td> $x_cont_int.-</td><td> $x_nombre_completo_int </td></tr>";
...
$x_filas_tabla_2 .= "<tr>
    <td>".FechaFormateada(strtotime(@$x_fecha_vencimiento_tab))." </td>";
   ...
   $x_filas_tabla_2 .=" <td>". round($x_total * 100) / 100 ." </td>
  </tr> ";
  $x_int_g = 1;
 while($x_int_g<=10){
...
$x_ini_z = 1;
 $x_numero_integrantes = 0;
while($x_ini_z <= 10){
	$x_nombre_i = "x_nombre_integrante_$x_ini_z";
	$x_nombre_i = $$x_nombre_i;
	
	if(!empty($x_nombre_i)){
			  $x_numero_integrantes ++;
			  }
	$x_ini_z ++;
	}
//calculos de la tabla dos
	$x_saldo_grupo = $x_importe_solicitado;
	$x_total_a_pagar = $x_monto_total_pago ; // monto de todos los vencimientos
	//$x_numero_integrantes = $x_numero_integrantes_grupo_n; 
	//echo "numero de integrantes=".$x_numero_integrantes."<br>";
	$x_gasto_seguro = intval($x_numero_integrantes)*5;
	//echo "gasto del seguro".$x_gasto_seguro."<br>";
	$x_total_a_pagar = $x_total_a_pagar - $x_gasto_seguro;
	$x_cont_cal = 1;
	while($x_cont_cal <= 10){
		
		$x_cant_soli_integrante = "x_monto_$x_cont_cal"; // monto del integrante
		...
		$x_monto_por_integrante = $x_monto_por_integrante + 5;
...
//sugue tabla dos
	...
	<tr>
	<td align='justify' style='font-family: Verdana; font-size: 13px;'> Cada integrante se compromete a entregar su dep&oacute;sito en casa del Representante de grupo, &oacute; en su caso, el lugar en el que los integrantes y el representante de grupo acuerden 1 d&iacute;a antes de la fecha de cada dep&oacute;sito.<p>
	</td></tr>
	<tr>
	<td align='justify' style='font-family: Verdana; font-size: 13px;'> La obligaci&oacute;n de realizar el pago es de todos los integrantes, si alguno no puede por cualquier circunstancia realizar el dep&oacute;sito correspondiente a alg&uacute;n pago, el resto de los integrantes se comprometen a cubrir el importe total del pago, de lo contrario, se generar&aacute; una penalizaci&oacute;n correspondiente a $250.00 (doscientos cincuenta pesos, 00/100 M.N.) por cada pago que se realice despu&eacute;s de la fecha pactada. <p></td></tr>
	<tr>
	<td align='justify' style='font-family: Verdana; font-size: 13px;'>El pago oportuno del cr&eacute;dito es responsabilidad de todos los integrantes del grupo. En caso de existir atraso en los pagos, Financiera Crea gestionara la cobranza exclusivamente con el Tesorero de grupo. Sin embargo, tanto el historial crediticio as&iacute; como las penalizaciones a las que en su caso se hagan acreedores, tendr&aacute;n implicaciones para todos los integrantes sin importar el desempe&ntilde;o individual de cada uno, &oacute; las parcialidades que cada integrante haya realizado.<p></td>
	</tr>
  ...
  <tr>
   ...
   <tr>
    <td>PAGO</td>";
     if(!empty($x_nombre_integrante_1)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_1* 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_2)){ $x_grupo_t .= "<td>". round ($x_monto_pago_int_2* 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_3)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_3* 100)/100 ."</td>";}
  	 if(!empty($x_nombre_integrante_4)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_4 * 100)/100 ."</td>";}
   	 if(!empty($x_nombre_integrante_5)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_5 * 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_6)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_6 * 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_7)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_7 * 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_8)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_8 * 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_9)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_9 * 100)/100 ."</td>";}
     if(!empty($x_nombre_integrante_10)){ $x_grupo_t .= "<td>". round($x_monto_pago_int_10* 100)/100 ."</td>";}
     $x_grupo_t .= "<td>&nbsp;</td>
  ...
  <tr>
	...
	$x_grupo_t .= "<table width='800' align='center'>";
	$x_grupo_t .= "<tr><td> &nbsp;</td><td> &nbsp;</td></tr>";
	$x_grupo_t .= "<tr><td colspan='2'>Comprendo y estoy de acuerdo en las condiciones del cr&eacute;dito: </td></tr>";
	$x_grupo_t .= "<tr><td> &nbsp;</td><td> &nbsp;</td></tr>";
	if(!empty($x_nombre_integrante_1)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_1 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_2)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_2 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_3)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_3 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_4)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_4 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_5)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_5 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_6)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_6 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_7)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_7 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_8)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_8 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_9)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_9 </td><td>______________________________</td></tr>";}
	if(!empty($x_nombre_integrante_10)){ $x_grupo_t .= "<tr><td align='justify' style='font-family: Verdana; font-size: 12px;'> $x_nombre_completo_10 </td><td>______________________________</td></tr>";}
	$x_grupo_t .= "</table>";
...
while($x_ciclo < $x_cont_fetc_v){
    ...
    <td>".FechaFormateada(strtotime(@$x_fecha))."</td>
...
$x_contenido = str_replace("\$x_group_tabla_2",$x_grupo_t,$x_contenido); 
$x_contenido = str_replace("\$_integrates_tablas",$_integrates_tablas,$x_contenido); 
	...
	if (phpmkr_num_rows($rs) == 0) {
		...
		if($GLOBALS["x_credito_tipo_id"] == 2){
			...
			while($x_cont_g <= 10){
				...
				if(!empty($rowGrupo["integrante_$x_cont_g"]) &&($rowGrupo["integrante_$x_cont_g"] != "") ){
					$GLOBALS["x_numero_integrantes_grupo_n"] =  $GLOBALS["x_numero_integrantes_grupo_n"] + 1;
					}// numero de integrantes
...
//CLIENTE
	...
	/*	if($x_gara > 0){
		...
		$sSql = "SELECT COUNT(*) AS avales FROM datos_aval WHERE solicitud_id = ".$GLOBALS["x_solicitud_id"];
		$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
		$row = phpmkr_fetch_array($rs);
		$x_aval = $row["avales"];
