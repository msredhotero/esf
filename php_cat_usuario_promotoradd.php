...
$x_promotor_id = @$_GET["x_promotor_id"];
...
switch ($sAction)
{
	...
	case "A": // Add
		...
		$sSql = "update promotor set usuario_id = $x_usuario_id where promotor_id = ".$GLOBALS["x_promotor_id"];
		$x_result = phpmkr_query($sSql, $conn);
		if(!$x_result){
			echo phpmkr_error() . '<br>SQL: ' . $sSql;
			phpmkr_query('rollback;', $conn);	 
...
<form name="usuarioadd" id="usuarioadd" action="php_cat_usuario_promotoradd.php" method="post" onSubmit="return EW_checkMyForm(this);">
<p>
...
<p>
<div align="center" class="phpmaker"><strong><u>Usuarios para Promotores </u></strong></div>
<br>
...
$sSqlWrk = "SELECT `usuario_rol_id`, `descripcion` FROM `usuario_rol` where descripcion = 'PROMOTOR'";
...
function AddData($conn, $x_promotor_id)
{
