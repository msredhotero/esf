...
while($rowcasos = phpmkr_fetch_array($rsCaso)){
	...
	$sqlTarea = "SELECT * from crm_tarea where crm_caso_id = $x_crm_caso_id   and fecha_registro >= \"2013-04-01\" order by crm_tarea_id ";
	...
	while($rowTareas = phpmkr_fetch_array($rsTarea)){
		...
		if($x_asunto == "Carta 2"){
			...
			//vemos si tine carta PROMESA DE PAGO NULA
	$sqlTareai = "SELECT * from crm_tarea where crm_caso_id = $x_crm_caso_id  AND orden = 5 and crm_tarea_tipo_id = 5 order by crm_tarea_id  ";
	...
	while($rowTareasi = phpmkr_fetch_array($rsTareai)){
		...
		if($x_tarea_idi > 0 ){
			echo "++++++++++++++++++++++++++  TIENE promesa de pago carta 1+++++++++++++++++<br>";
			 $sqlPPC1 = "SELECT promesa_pago from crm_tarea_cv  where crm_tarea_id = $x_tarea_idi ";
			 $rsPPC1 = phpmkr_query($sqlPPC1,$conn)or die ("Error en fecha ppc1".phpmkr_error().$sqlPPC1."sql:".$sqlPPC1);
			 $rowPPC1 = phpmkr_fetch_array($rsPPC1);
			 $x_fecha_pp = $rowPPC1["promesa_pago"];
			 echo "fecha de promesa de pago  ... ". $x_fecha_pp ." --<br>";
			 echo "caso id ".$x_crm_caso_id."<br>";
			 echo "tareas _id ".$x_tarea_idi."<br>";
			if($x_fecha_pp == ""){
				$x_lista_vacia = $x_lista_vacia.$x_crm_caso_id.", ";
				$x_lista_ta = $x_lista_ta.$x_tarea_idi.", ";
				}
				}
			}
		}
...
echo "<br><br>".$x_lista_vacia;
echo "<br><br>".$x_lista_ta;
