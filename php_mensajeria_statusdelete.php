...
while ($i < count($arRecKey)){
	...
	// Build the SQL
	$sDbWhere .= "`mensajeria_status_id`=" . $sRecKey . " AND ";
...
<p><span class="phpmaker">Delete from TABLE: Status de mensajeria<br><br><a href="php_mensajeria_statuslist.php">Back to List</a></span></p>
<form action="php_mensajeria_statusdelete.php" method="post">
<p>
...
while ($i < count($arRecKey)) {
	...
	$x_mensajeria_status_id = $sRecKey;
	if (LoadData($conn)) {
?>
...
{
	global $x_mensajeria_status_id;
	$sSql = "Delete FROM `mensajeria_status`";
	$sSql .= " WHERE " . $sqlKey;
