...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=garantia_liquida_pgos.xls');
}
if ($sExport == "word") {
	header('Content-Type: application/vnd.ms-word');
	header('Content-Disposition: attachment; filename=garantia_liquida_pgos.doc');
}
?>
<?php include ("db.php") ?>
...
// Build SQL
$sSql = "SELECT garantia_liquida.*, garantia_liquida_status.descripcion, credito.credito_id, credito.credito_num, cliente.nombre_completo, cliente.apellido_paterno, cliente.apellido_materno FROM garantia_liquida JOIN garantia_liquida_status ON garantia_liquida_status.garantia_liquida_status_id = garantia_liquida.status JOIN credito ON  credito.credito_id = garantia_liquida.credito_id JOIN solicitud ON solicitud.solicitud_id = credito.solicitud_id JOIN solicitud_cliente ON solicitud_cliente.solicitud_id = solicitud.solicitud_id JOIN  cliente ON cliente.cliente_id = solicitud_cliente.cliente_id WHERE  credito.credito_status_id = 3  and garantia_liquida.status in (2) ";
...
?>
<p><span class="phpmaker">pagos cubiertos con garantía líquida
  <?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_listado_garantia_credito_liquidado.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="php_listado_garantia_credito_liquidado.php?export=word">Export to Word</a>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
<p>
...
?>
<?php if ($sExport == 1) { ?>
<form action="php_listado_garantia_credito_liquidado.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_listado_garantia_credito_liquidado.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_listado_garantia_credito_liquidado.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_listado_garantia_credito_liquidado.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_listado_garantia_credito_liquidado.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_listado_garantia_credito_liquidado.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
			  <a href="php_listado_garantia_credito_liquidado.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
ID
<?php }else{ ?>
	<a href="php_listado_garantia_credito_liquidado.php?order=<?php echo urlencode("garantia_liquida_id"); ?>" style="color: #FFFFFF;" onMouseDown="ewsort(event, this.href);">ID<?php if (@$_SESSION["garantia_liquida_x_garantia_liquida_id_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["garantia_liquida_x_garantia_liquida_id_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</span></td>
		<td valign="top"><span>
<?php if ($sExport <> "") { ?>
...
monto
<?php }else{ ?>
	<a href="php_listado_garantia_credito_liquidado.php?order=<?php echo urlencode("monto"); ?>" style="color: #FFFFFF;" onMouseDown="ewsort(event, this.href);">monto<?php if (@$_SESSION["garantia_liquida_x_monto_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["garantia_liquida_x_monto_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</span></td>
		<td valign="top"><span>
  <?php if ($sExport <> "") { ?>
		  status
  <?php }else{ ?>
		  <a href="php_listado_garantia_credito_liquidado.php?order=<?php echo urlencode("status"); ?>" style="color: #FFFFFF;" onMouseDown="ewsort(event, this.href);">status<?php if (@$_SESSION["garantia_liquida_x_status_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["garantia_liquida_x_status_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
  <?php } ?>
		  </span></td>
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount % 2 <> 0) {
		...
		// seleccionamos el recibo del crdito que pertenece a el pago de garantia liaquida
		$sqlPagos = " SELECT recibo.recibo_id, recibo.recibo_status_id, SUM( vencimiento.importe ) AS importe, SUM( vencimiento.interes ) AS interes, SUM( vencimiento.iva ) AS iva, SUM( total_venc ) AS total
						FROM recibo
						...
						WHERE recibo.referencia_pago_2 LIKE '%garantia%'
						AND recibo.recibo_status_id in( 1 ) AND vencimiento.credito_id = $x_id  
						GROUP BY vencimiento.credito_id ";		
		$rsPagos = phpmkr_query($sqlPagos,$conn) or die("Error al seleccionar los pagos".$phpmkr_error()."sql:".$sqlPagos);
		$rowPagos = phpmkr_fetch_array($rsPagos);
		$x_importe_v = $rowPagos["importe"];
		$x_interes_v = $rowPagos["interes"];
		$x_iva_v = $rowPagos["iva"];
		$x_total_v = $rowPagos["total"];
...
<td>&nbsp;</td>
<td>&nbsp;</td>
<td><a href="php_creditoedit.php?credito_id=<?php echo $x_id;?>">Ver</a>
</td>
<td>&nbsp;</td>
<?php } ?>
		<!-- garantia_liquida_id -->
...
<?php echo $x_status; ?>
		...
		<!-- fecha -->
		<!-- fecha_modificacion -->
		<td><?php echo $x_importe_v;?></td>
<td><?php echo $x_interes_v;?></td>
<td><?php echo $x_iva_v;?></td>
<td><?php echo $x_total_v;?></td>
	</tr>
<?php
	}
}
