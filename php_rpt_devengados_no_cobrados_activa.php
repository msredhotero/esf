...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=reporte_devengado_no_cobrados_activa'.date("Y-m-d").'.xls');
}
?>
<?php include ("db.php") ?>
...
<p><span class="phpmaker">REPORTE DEVENGADOS NO COBRADOS - activa -
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<?php } ?>
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_rpt_devengados_no_cobrados_activa.php?export=excel&x_mes=<?php echo $x_mes; ?>&x_year=<?php echo $x_year; ?>&x_empresa_id=<?php echo $x_empresa_id; ?>&x_fondeo_credito_id=<?php echo $x_fondeo_credito_id; ?>&x_sucursal_srch=<?php echo $x_sucursal_srch; ?>&x_promo_srch=<?php echo $x_promo_srch; ?>&x_pagado_srch=<?php echo $x_pagado_srch; ?>&x_credito_status_srch=<?php echo $x_credito_status_srch; ?>">Exportar a Excel</a><?php } ?>
</span></p>
</span></p>
...
<div align="left">
  <table id="ewlistmain" class="ewTable" align="left">
    ...
    <tr class="ewTableHeader" >
        ...
        <td width="175" valign="top">Capital</td>
		<td width="175" valign="top"><span> Interes</span></td>  
        <td width="175" valign="top">IVA</td>      
		<td width="196" valign="top"><span> Moratorios</span></td>
        <td width="175" valign="top">IVA Mor</td>
		<td width="133" valign="top">Comision</td>
        <td width="133" valign="top">IVA Com</td>
		<td width="192" valign="top"><span> Total</span></td>   
	</tr>
<?php
...
$x_hoy_es = date("Y-m-d");
$sSql = "SELECT sum(vencimiento.interes) as interes_devengado_no_pagado ,vencimiento.importe as capital, vencimiento.iva as iva,  credito.credito_id from vencimiento  join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id where (vencimiento.vencimiento_status_id in(1,3,6,7))   AND (vencimiento.credito_id not in (select credito_id from credito where credito_status_id in (2,5,4))) and vencimiento.fecha_vencimiento >= \"2014-03-01\" and vencimiento.fecha_vencimiento < \"$x_hoy_es\" ".$x_filtros."GROUP BY credito_id";
...
$sSql1 = "SELECT sum(vencimiento.interes_moratorio) as mora_devengado_no_pagado,sum(iva_mor) as iva_mor from vencimiento  join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id where (vencimiento.vencimiento_status_id in(1,3,6,7))   AND (vencimiento.credito_id not in (select credito_id from credito where credito_status_id in (2,5,4))) and vencimiento.fecha_vencimiento >= \"2014-03-01\"  and vencimiento.fecha_vencimiento < \"$x_hoy_es\" and vencimiento.vencimiento_num < 3000  and credito.credito_id = $x_credito_id".$x_filtros;	
...
?>
		...
		<tr<?php echo $sItemRowClass; ?><?php echo $sListTrJs; ?>>
            ...
            <td align="right"><?php echo FormatNumber($capital,2,0,0,1); ?></td>
			<td align="right"><strong><?php echo FormatNumber($interes_devengado_no_pagado,2,0,0,1); ?></strong></td>
            <td align="right"><?php echo FormatNumber($iva,2,0,0,1); ?></td>	
			<!-- importe -->
			<td align="right"><strong><?php echo FormatNumber($x_mora_devengado_no_pagado,2,0,0,1); ?></strong></td>
            <td align="right"><?php echo FormatNumber($x_mora_iva,2,0,0,1); ?></td>
			<td align="right"><strong><?php echo FormatNumber($x_mora_devengado_no_pagado_com,2,0,0,1); ?></strong></td>
            <td align="right"><?php echo FormatNumber($x_mora_iva_com,2,0,0,1); ?></td>
			<!-- importe -->
			<td><strong><?php echo FormatNumber($interes_devengado_no_pagado + $x_mora_devengado_no_pagado +$x_mora_devengado_no_pagado_com,2,0,0,1); ?></strong></td>
