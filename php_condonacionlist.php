...
if($filter["x_fecha_desde"] != ""){
	$sDbWhere .= "(condonacion.fecha_registro >= '".ConvertDateToMysqlFormat($filter["x_fecha_desde"])."') AND ";
	$sDbWhere .= "(condonacion.fecha_registro <= '".ConvertDateToMysqlFormat($filter["x_fecha_hasta"])."') AND ";	
}
...
$sSql .= "  order by condonacion_id desc ";
...
?>
<p><span class="phpmaker">CONDONACIONES
  <?php if ($sExport == "") { ?><?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
<p>
...
<p>
  ...
  <td><table width="895" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td><div align="right"><span class="phpmaker"> condonacion desde: </span></div></td>
      <td>&nbsp;</td>
      <td><span>
      ...
      <td>&nbsp;</td>
      <td><div align="right"> condonacion<span class="phpmaker"> hasta: </span></div></td>
      <td>&nbsp;</td>
      <td><span>
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_condonacioneslist.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_condonacioneslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_condonacioneslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_condonacioneslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_condonacioneslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
			  <a href="php_condonacioneslist.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
<?php } ?>
  ...
  <?php if ($sExport <> "") { ?>
		  ID
  <?php }else{ ?>
		  <a href="php_condonacioneslist.php?order=<?php echo urlencode("convenio_liquidacion_id"); ?>" style="color: #FFFFFF;" onMouseDown="ewsort(event, this.href);">ID<?php if (@$_SESSION["convenio_liquidacion_x_convenio_liquidacion_id_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["convenio_liquidacion_x_convenio_liquidacion_id_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
  <?php } ?>
		  </span></td>
        	<td width="156" valign="top"><span>
  <?php if ($sExport <> "") { ?>
        	  Fecha condonacion
  <?php }else{ ?>
        	  Fecha Condonacion
 ...
 <td>Cliente Num</td>
 <td>Nombre</td>
 <td width="356" valign="top"><span>
          <?php if ($sExport <> "") { ?>
         Ver Cr&eacute;dito
          <?php }else{ ?>Ver Cr&eacute;dito<?php } ?>
        </span></td>
 <td width="356" valign="top"><span>
          <?php if ($sExport <> "") { ?>
          Monto
...
<td width="177" valign="top"><span>
  <?php if ($sExport <> "") { ?>Usuario<?php }else{ ?>
  Usuario
  <?php } ?>
</span></td>
        <td width="356" valign="top"><span>
...
while (($row = @mysql_fetch_assoc($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount % 2 <> 0) {
		...
		$x_tipo = "";	
		if($usuario_id>0){
		$SQLtIPO = "SELECT nombre FROM usuario WHERE usuario_id = $usuario_id";
		$rsDescri = phpmkr_query($SQLtIPO,$conn) or die("Erro al seleccionar".phpmkr_error());
		$rowDesc = phpmkr_fetch_array($rsDescri);
		$x_tipo = 	$rowDesc["nombre"];
		}
		
		$SQLtIPO = "SELECT credito_num, cliente_num, solicitud_id ,credito_status.descripcion as credito_status FROM credito join credito_status WHERE credito_status.credito_status_id =  credito.credito_status_id and credito_id = $credito_id";
		...
		$x_cliente_num =$rowDesc["cliente_num"];
		
		$SQLtIPO = "SELECT solicitud.promotor_id, sucursal.nombre as sucursal, promotor.nombre_completo as promotor  FROM solicitud, promotor , sucursal WHERE solicitud.promotor_id =  promotor.promotor_id and sucursal.sucursal_id = promotor.sucursal_id and  solicitud_id = $solicitud_id";
		$rsDescri = phpmkr_query($SQLtIPO,$conn) or die("Erro al seleccionar".phpmkr_error());
		$rowDesc = phpmkr_fetch_array($rsDescri);
		$x_sucursal =$rowDesc["sucursal"];
		$x_promotor =$rowDesc["promotor"];

		$SQLtIPO = "SELECT  solicitud_cliente.*, cliente.nombre_completo, cliente.apellido_paterno, cliente.apellido_materno FROM cliente, solicitud_cliente  WHERE solicitud_cliente.cliente_id = cliente.cliente_id  and solicitud_cliente.solicitud_id = $solicitud_id";
		$rsDescri = phpmkr_query($SQLtIPO,$conn) or die("Erro al seleccionar".phpmkr_error());
		$rowDesc = phpmkr_fetch_array($rsDescri);
		$x_cliente =$rowDesc["nombre_completo"]." ".$rowDesc["apellido_paterno"]." ".$rowDesc["apellido_materno"]; 
		
		
		$SQLtIPO = "SELECT  * FROM vencimiento WHERE vencimiento_id = $vencimiento_id";
		$rsDescri = phpmkr_query($SQLtIPO,$conn) or die("Erro al seleccionar".phpmkr_error());
		$rowDesc = phpmkr_fetch_array($rsDescri);
		$x_importe =$rowDesc["importe"]; 
		$x_interes =$rowDesc["interes"];
		$x_interes_moratorio =$rowDesc["interes_moratorio"];
		$x_iva =$rowDesc["iva"];
		$x_iva_mor =$rowDesc["iva_mor"];
		$x_total_venc =$rowDesc["total_venc"];
		$x_suma_importe =  $x_importe + $x_interes + $x_interes_moratorio + $x_iva + $x_iva_mor ;
...
<?php } ?>
		...
		<td><span>
  <?php echo $condonacion_id; ?>
</span></td>
<td><span>
  <?php echo FormatDateTime($fecha_registro,7)  ; ?>
</span></td>
<td><?php echo $x_sucursal;?> </td>
<td><?php echo $x_promotor;?></td>
<td><span>
  <?php echo $x_credito_num; ?>
</span></td>
<td><?php echo $x_credito_status;?></td>
<td><?php echo $x_cliente_num;?></td>
<td><?php echo $x_cliente;?></td>
<td><span>
   <a href="php_creditoedit.php?credito_id=<?php echo $credito_id?>" target="_blank">Ir a cr&eacute;dito</a>
</span></td>
<td align="right"><span>
  <?php echo FormatNumber($x_suma_importe,2,0,0,1); ?>
</span></td>
<td><span>
  <?php echo $x_tipo; ?>
</span></td>
<td><span>
  <?php if(!empty($bitacora)){echo $bitacora; }else{?><a href="php_condonacion_bitacora.php?x_condonacion_id=<? echo $condonacion_id;?>&x_credito_num=<?php echo $x_credito_num;?>" target="_blank">registrar motivos</a><?php
	  }?>
</span></td>
</tr>
<?php
