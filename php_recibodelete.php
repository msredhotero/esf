...
while ($i < count($arRecKey)){
	...
	// Build the SQL
	$sDbWhere .= "`recibo_id`=" . $sRecKey . " AND ";
...
<p><span class="phpmaker">Delete from TABLE: RECIBOS<br><br><a href="php_recibolist.php">Back to List</a></span></p>
<form action="php_recibodelete.php" method="post">
<p>
...
<table class="ewTable">
	<tr class="ewTableHeader">
		...
		<td valign="top"><span>Vencimiento</span></td>
...
<?php echo $x_recibo_id; ?>
		...
		<td><span>
<?php
if ((!is_null($x_vencimiento_id)) && ($x_vencimiento_id <> "")) {
	$sSqlWrk = "SELECT `vencimiento_id` FROM `vencimiento`";
	$sTmp = $x_vencimiento_id;
	$sTmp = addslashes($sTmp);
	$sSqlWrk .= " WHERE `vencimiento_id` = " . $sTmp . "";
	$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
	if ($rswrk && $rowwrk = phpmkr_fetch_array($rswrk)) {
		$sTmp = $rowwrk["vencimiento_id"];
	}
	@phpmkr_free_result($rswrk);
} else {
	$sTmp = "";
}
$ox_vencimiento_id = $x_vencimiento_id; // Backup Original Value
$x_vencimiento_id = $sTmp;
?>
<?php echo $x_vencimiento_id; ?>
<?php $x_vencimiento_id = $ox_vencimiento_id; // Restore Original Value ?>
</span></td>
		<td><span>
<?php
