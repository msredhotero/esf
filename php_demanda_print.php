...
if($x_delegacion == "VILLA DE ALVAREZ COLIMA "){
	$x_delegacion = "VILLA DE ALVAREZ";
	}
	
if($x_entidad == "COLIMA"){
	$x_entidad = " EL ESTADO DE COLIMA";
	}	

$x_direccion = " calle ".$x_calle." ".$x_numero_exterior." Colonia ".$x_colonia.". en la Delegacion ".$x_delegacion.", ".$x_entidad;
...
$sSql = "SELECT entidad_id, descripcion FROM delegacion where delegacion_id = $x_delegacion_id2";
$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query dg: " . phpmkr_error() . '<br>SQL: ' . $sSql);
$row = phpmkr_fetch_array($rs);
...
$sSql = "SELECT nombre FROM entidad where entidad_id = $x_entidad_id2";
$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query dg: " . phpmkr_error() . '<br>SQL: ' . $sSql);
$row = phpmkr_fetch_array($rs);
$x_entidad2 = $row["nombre"];
phpmkr_free_result($rs);
}
if(!empty($x_direccion_id2)){
$x_domicilio_empresa = " en la calle ".$x_calle2.". Colonia ".$x_colonia2." en  ".$x_delegacion2.",". $x_entidad2;
}else{
	$x_domicilio_empresa = "";
	}

$x_solocliente = $x_cliente;
#echo "sexo cliente ".$x_sexo." -sexo aval".$x_sexo_aval;
if($x_sexo == 1){
	$x_1_cliente = $x_cliente;
	$x_cliente = "".$x_cliente;

	$x_el = "el";
	$x_demandado = "demandado";
...
//CREDITO
$sSql = "SELECT credito.banco_id, 	credito.num_pagos, credito.tarjeta_num, fecha_vencimiento, referencia_pago, tasa, tasa_moratoria, fecha_otrogamiento, credito.credito_id FROM credito where solicitud_id = $x_solicitud_id";
$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
$row = phpmkr_fetch_array($rs);
...
$x_numero_de_pagos = $row["num_pagos"];
...
//echo "tasa mora".$x_tasa_moratoria_valor."<br>";
//echo "PENALIZACION ".$x_penalizacion;
...
$x_today = date("Y-m-d");
$x_fecha_hoy = FechaLetras(strtotime(ConvertDateToMysqlFormat($x_today)));
...
if(!empty($x_representante_cliente_id)){
	...
	if($x_sexo_rep == 1){
	$x_representate_grupo = "".$x_nombre_completo_rep;
}else{
	$x_representate_grupo = "".$x_nombre_completo_rep;
}
...
$x_1_aval = strtoupper($row["nombre_completo"]." ".$row["apellido_paterno"]." ".$row["apellido_materno"]);
$x_aval .= strtoupper($row["nombre_completo"]." ".$row["apellido_paterno"]." ".$row["apellido_materno"]);
phpmkr_free_result($rs);
...
$x_diraval = " en la calle ".$x_aval_calle." Colonia ".$x_aval_colonia.", en la Delegacion ".$x_aval_delegacion.", ".
$x_aval_entidad;
...
$x_contenido = str_replace("\$x_primer_parrafo",$x_primer_parrafo,$x_contenido);
$x_contenido = str_replace("\$x_segundo_parrafo",$x_segundo_parrafo,$x_contenido);
$x_contenido = str_replace("\$x_tercer_parrafo",$x_tercer_parrafo,$x_contenido);
$x_contenido = str_replace("\$x_cuarto_parrafo",$x_tercer_parrafo,$x_contenido);
...
$x_contenido = str_replace("\$x_1_aval",htmlentities($x_1_aval),$x_contenido);
$x_contenido = str_replace("\$x_cliente",$x_cliente,$x_contenido);
$x_contenido = str_replace("\$x_1_cliente",$x_1_cliente,$x_contenido);
...
$x_pena = $rowC["penalizacion"];
//echo "vALOR PENA".$x_pena;
if(strlen($x_pena)>1){
	$x_contenido = str_replace("\$x_mora",$x_penalizacion,$x_contenido);
	}else{
	$x_contenido = str_replace("\$x_mora",$x_moratorios,$x_contenido);	
		}

$x_contenido = str_replace("\$x_tasa",$x_anual_tasa,$x_contenido);
...
$x_contenido = str_replace("\$x_estado",$x_estado_nombre,$x_contenido);
$x_contenido = str_replace("\$x_municipio",$x_delegacion_nombre,$x_contenido);
$x_contenido = str_replace("\$x_esta_aval",$x_esta_aval,$x_contenido);
$x_contenido = str_replace("\$x_mun_aval",$x_mun_aval,$x_contenido);
...
$x_contenido = str_replace("\$x_numpagos",$x_numero_de_pagos,$x_contenido);
...
// demanda
$x_contenido = str_replace("\$x_el",$x_el,$x_contenido);
$x_contenido = str_replace("\$x_los",$x_los,$x_contenido);
$x_contenido = str_replace("\$x_demandado",$x_demandado,$x_contenido);

$x_contenido = str_replace("semanal",$x_forma_pago_periodo,$x_contenido);
$x_contenido = str_replace("CANTIDAD DE PRESTAMO CON LETRA, (CANTIDAD DE PRESTAMO CON NUMERO)",$x_importe,$x_contenido);// $x_lugar_fecha


//$x_lugar_fecha = $GLOBALS["x_entidad"].", ".strtoupper($x_fecha_hoy);
$x_lugar_fecha =$x_fecha_hoy;
$x_contenido = str_replace("\$x_lugar_fe",$x_lugar_fecha,$x_contenido);
...
function LoadData($conn)
{
	...
	if (phpmkr_num_rows($rs) == 0) {
		...
		$x_credito_id = $rowC["credito_id"];
		if ($rowC["penalizacion"]>0){
		$GLOBALS["x_primer_parrafo"] =  "correspondiente a un monto fijo de ";
		$GLOBALS["x_segundo_parrafo"] =  " por ";
		$GLOBALS["x_tercer_parrafo"]= ", calculando desde la fecha en que debió realizarse el pago y hasta la fecha de su pago total, acordado por nuestra endosante y por los demandados y conforme a la tabla plasmada en el pagare base de la acción, los cuales se calcularan en la ejecución de sentencia. ";
		$GLOBALS["x_cuarto_parrafo"]= ", calculando desde la fecha en que debió realizarse el pago y hasta la fecha de su pago total, acordado por nuestra endosante y por el demandado  y conforme a la tabla plasmada en el pagare base de la acción, los cuales se calcularan en la ejecución de sentencia";
		
		}else{
			$GLOBALS["x_primer_parrafo"] =  "correspondiente a  ";
			$GLOBALS["x_segundo_parrafo"] =  " diarios sobre ";
			$GLOBALS["x_tercer_parrafo"]= ", calculando desde la fecha en que debió realizarse el pago y hasta la fecha de su pago total, acordado por nuestra endosante y por los demandados y conforme a la tabla plasmada en el pagare base de la acción, los cuales se calcularan en la ejecución de sentencia. ";
			$GLOBALS["x_cuarto_parrafo"]= ", calculando desde la fecha en que debió realizarse el pago y hasta la fecha de su pago total, acordado por nuestra endosante y por el demandado  y conforme a la tabla plasmada en el pagare base de la acción, los cuales se calcularan en la ejecución de sentencia";
...
//CLIENTE
		...
		if($GLOBALS["x_numero_exterior"] == "0"){
			$GLOBALS["x_numero_exterior"] ="";
			}
		
		// seleccionamos los datos del estdo
		
		if(!empty($GLOBALS["x_entidad"])){
		$sqlEdo = "SELECT * FROM entidad WHERE entidad_id = ".$GLOBALS["x_entidad"] ." ";
		...
		$sqlEdo = "SELECT * FROM delegacion WHERE  delegacion_id = ".$GLOBALS["x_delegacion_id"] ." ";
		$rsEdo = phpmkr_query($sqlEdo,$conn) or die("Error al seleccionar el estado".phpmkr_error()."sql".$sqlEdo);
		$rowEdo = phpmkr_fetch_array($rsEdo);
		$GLOBALS["x_delegacion_nombre"] = $rowEdo["descripcion"];
		}else{
			$GLOBALS["x_delagacion_nombre"]= "";
			}
			
			
		if($GLOBALS["x_delegacion_nombre"] == "VILLA DE ALVAREZ COLIMA"){
	$GLOBALS["x_delegacion_nombre"] = "VILLA DE ALVAREZ";
	}
	
if($GLOBALS["x_estado_nombre"] == "COLIMA"){
	$GLOBALS["x_estado_nombre"] = " EL ESTADO DE COLIMA";
	}	
	...
	/*	if($x_gara > 0){
		...
		$sSql = "SELECT nombre_completo as avales,entidad,delegacion FROM datos_aval WHERE solicitud_id = ".$GLOBALS["x_solicitud_id"];
		$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
		$row = phpmkr_fetch_array($rs);
		$x_aval = $row["avales"];
		$x_aval_entidad = $row["entidad"];
		$x_aval_delegacion = $row["delegacion"];
		 //echo "dir aval--".$x_aval_entidad ."--".$x_aval_delegacion;
		if( strlen(trim($x_aval)) > 0  ){
			$x_aval = 1;
			}
			
		if(!empty($x_aval_entidad))	{
			$sqlEdo = "SELECT * FROM entidad WHERE entidad_id = ".$x_aval_entidad ." ";
		$rsEdo = phpmkr_query($sqlEdo,$conn) or die("Error al seleccionar el estado".phpmkr_error()."sql".$sqlEdo);
		$rowEdo = phpmkr_fetch_array($rsEdo);
		$GLOBALS["x_esta_aval"] = $rowEdo["nombre"];
		}else{
			$GLOBALS["x_esta_aval"]= "";
			}
			
			
		if(!empty($x_aval_delegacion)){
			$sqlEdo = "SELECT * FROM delegacion WHERE  delegacion_id = ".$x_aval_delegacion ." ";
		$rsEdo = phpmkr_query($sqlEdo,$conn) or die("Error al seleccionar el estado".phpmkr_error()."sql".$sqlEdo);
		$rowEdo = phpmkr_fetch_array($rsEdo);
		$GLOBALS["x_mun_aval"] = $rowEdo["descripcion"];
		
			if($GLOBALS["x_mun_aval"] == "VILLA DE ALVAREZ COLIMA "){
					$GLOBALS["x_mun_aval"] ="VILLA DE ALVAREZ";
					}
					
				if($GLOBALS["x_esta_aval"] == "COLIMA"){
					$GLOBALS["x_esta_aval"] =" EL ESTADO DE COLIMA";
					}	
				...
				if($GLOBALS["x_estado_nombre"] == ""){
					$GLOBALS["x_estado_nombre"] =" EL ESTADO DE COLIMA";
					}	
		if($GLOBALS["x_mun_aval"]!= $GLOBALS["x_delegacion_nombre"]){
				
					
					
					
				$GLOBALS["x_delegacion_nombre"] = $GLOBALS["x_delegacion_nombre"]." ". $GLOBALS["x_estado_nombre"].",";
				$GLOBALS["x_estado_nombre"] = $GLOBALS["x_mun_aval"]." ".$GLOBALS["x_esta_aval"];
				}
		
		}else{
			$GLOBALS["x_mun_aval"]= "";
			}
		//	echo "del avl777------".$GLOBALS["x_mun_aval"];
		///*	echo "del tit".$GLOBALS["x_delegacion_nombre"];
			/*if($GLOBALS["x_mun_aval"]!= $GLOBALS["x_delegacion_nombre"]){
				$GLOBALS["x_delegacion_nombre"] = $GLOBALS["x_delegacion_nombre"]." ". $GLOBALS["x_estado_nombre"].",";
				$GLOBALS["x_estado_nombre"] = $GLOBALS["x_mun_aval"]." ".$GLOBALS["x_esta_aval"];
				}*/
...
$x_fecha_h = date("Y-m-d");
								$sqlInsert = " UPDATE  `crm_credito_status` SET  `crm_cartera_status_id` =  '8' , `fecha` = '$x_fecha_h' WHERE  `credito_id` =$x_credito_id " ;							
