...
#$x_importe_solicitado = $row["importe"];
...
// buscamos el lugar de otorgamiento
$x_entidad_otorgamiento = '';
$sqlLO = "SELECT lugar_otorgamiento FROM solicitud WHERE solicitud_id = ".$x_solicitud_id." ";
$rsLO = phpmkr_query($sqlLO,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sqlLO);	
$rowLO = phpmkr_fetch_array($rsLO);
$x_lugar_otorgamiento = $rowLO["lugar_otorgamiento"];

if(empty($x_lugar_otorgamiento)){
	$x_lugar_otorgamiento = 9;
	}

if(!empty($x_lugar_otorgamiento)){

$sSqln = "SELECT nombre FROM entidad where entidad_id = $x_lugar_otorgamiento";

$rsn = phpmkr_query($sSqln,$conn) or die("Failed to execute query dg: " . phpmkr_error() . '<br>SQL: ' . $sSql);

$rown = phpmkr_fetch_array($rsn);
#echo $rown["nombre"];
if($rown["nombre"]=='CIUDAD DE MEXICO'){
	$x_entidad_otorgamiento = 'Ciudad de M&eacute;xico el d&iacute;a ';
	}else{
		$x_entidad_otorgamiento = ucwords($rown["nombre"])." el d&iacute;a ";
		}



phpmkr_free_result($rsn);
...
$x_contenido = str_replace("\$x_edo",$x_entidad_otorgamiento,$x_contenido);
...
$x_contenido = str_replace("\$x_usr_c"," ".$x_usuario,$x_contenido);
$x_contenido = str_replace("\$x_passw_c"," ".$x_clave,$x_contenido);
...
$recaNuevo = '<p><font size="2"><table border="0" cellspacing="0" cellpadding="0" width="800" align="center">
				<tbody>
					<tr>
						<td>
							<p align="right">
...

{
	...
	if (phpmkr_num_rows($rs) == 0) {
		...
		$GLOBALS["x_importe_solicitado"] = $rowC["importe"];
...
$GLOBALS["x_usuario"] = $rowa["usuario"];
$GLOBALS["x_usuario_pp"] = $rowa["usuario"];

$GLOBALS["x_clave"] = $rowa["clave"];

#echo "<br>".$sSqla." =>".$GLOBALS["x_clave"].$rowa["usuario"];
...
$GLOBALS["x_consulta_cuenta"] = "<p>Consulta tu estado de cuenta desde www.financieracrea.com en la seccion ingreso para clientes, tu cuenta es: $x_usuario , tu clave es: $x_clave";
	...
	/*	if($x_gara > 0){
		...
		$sSql = "select contenido from formato_docto where formato_docto_id = 59";
