...
function calculaTotalVencimientos( $credito_id, $conn){
	...
	// si el credito tiene garntia y penalizacion entonces no debe tomar en cuenta el numero de vencimientos que corresponda a la garantia 
	$sqlgarantia = "SELECT garantia_liquida, num_pagos, forma_pago_id  FROM credito WHERE credito_id = $credito_id ";
	$rsgarantia = phpmkr_query($sqlgarantia,$conn)or die("Error gar..".phpmkr_error()."sql .".$sqlgarantia);
	$rowgarantia = phpmkr_fetch_array($rsgarantia);
	$x_gar_liquida = $rowgarantia["garantia_liquida"];
	$x_forma_pago_id = $rowgarantia["forma_pago_id"];
	$x_num_pagos = $rowgarantia["num_pagos"];
	$x_vencimeintos_fuera = 0;
	echo "gar liquida ".	$x_gar_liquida."<br> num pagod".$x_num_pagos;
	if ($x_gar_liquida > 0){
	
		 #seleccionamos el penultimo vencimiento
				 $sqlUltimoVencimiento = "SELECT vencimiento_num FROM  vencimiento WHERE  credito_id = $credito_id AND vencimiento_status_id in (1,3) ";
				 ...
				 $x_antepenultimo_nn_menos_2 = $x_ultimo_vencimiento_nn - 2;
				 echo "tiene garantia liquida<br> ";
				 echo $sqlUltimoVencimiento;
				 echo "<br> ultimo ". $x_ultimo_vencimiento_nn." row ".$rowUltimoVencimiento["vencimiento_num"];
				 echo "<br> penultimo ". $x_ultomo_vencimeinto_nn_menos_1." ";
				 echo "<br> antepultimo ".  $x_antepenultimo_nn_menos_2." ";
				 #forma pago 
				 #1 semenala
				 #2 catorcenal
				 #3 mensual
				 ...
				 if ($x_forma_pago_id == 1){
					 // el credito es semanal y la garantia liquida corresponde a lo de dos pagos asi que debemos cambair el status a dos pagso para que no entren al ciclo de pago		echo 
					 echo "forma pago selamal<br>";
					 echo "numero de pagos ".$x_num_pagos."<br>";
					 # si es semanal pueden ser dos o tres pagos de garantia_liquida	
					 if($x_num_pagos == 40){
						 // se deben seleccionar 3 vencimientos
						 if($x_ultomo_vencimeinto_nn_menos_1 > 0 && $x_antepenultimo_nn_menos_2 > 0  && $x_ultimo_vencimiento_nn > 0){
						 $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7) and vencimiento.vencimiento_num not in ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1 ,$x_antepenultimo_nn_menos_2)";
						 }else{
							 $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7)";
							 }
						 
					
						
						 }else{		
						 if($x_ultimo_vencimiento_nn > 0 && $x_ultomo_vencimeinto_nn_menos_1 > 0){
						 $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7) and vencimiento.vencimiento_num not in ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1)";			 	
						 }else{
							  $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7)";
							 }
				     ...
				     # echo"dobel vencimeinto cambio status".$sqlUpdateUltimoVencmeinto."<br>";
						 ...
						 echo "no es semanal <br>";
						 if((($x_forma_pago_id == 2) || ($x_forma_pago_id == 4)) && ($x_num_pagos == 24)){
							 #catorcenal y quincenal con 24 pagos se toas dos pasgo de garantia....
							 if($x_ultimo_vencimiento_nn > 0 && $x_ultomo_vencimeinto_nn_menos_1 > 0 ){
							 $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7 ) and vencimiento.vencimiento_num not in ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1)";
							 }else{
								 $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7)";
						 ...
						 if($x_ultimo_vencimiento_nn > 0){
						 $sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 3 || vencimiento_status_id = 6 || vencimiento_status_id = 7) and vencimiento.vencimiento_num not in ($x_ultimo_vencimiento_nn)";
	...
	$responseV = phpmkr_query($sqlVnec ,$conn) or die ("Error al seleccionar la sumade los vencimientos pendientes o venciedos 3".phpmkr_error()."sql: ".$sqlVnec);
	$rowv = phpmkr_fetch_array($responseV);
	
	$total_vencimientos = $rowv["importe_vencim"];
	
	
	echo "sql tol venc".$sqlVnec."<br>";
...
function calculaMoratorios($x_credito_id, $conn,$x_fecha_movimiento){
	...
	$responseV = phpmkr_query($sqlVnec ,$conn) or die ("Error al seleccionar la sumade los vencimientos pendientes o venciedos 1 ".phpmkr_error()."sql: ".$sqlVnec);
	...
	$x_total_del_importe_vencimiento = $rowv["importe_vencim"];
			...
			$sqlVnec = "SELECT SUM(total_venc) AS importe_vencim  FROM vencimiento WHERE credito_id = $x_credito_id AND (vencimiento_status_id = 1 || vencimiento_status_id = 6)";
			$responseV = phpmkr_query($sqlVnec ,$conn) or die ("Error al seleccionar la sumade los vencimientos pendientes o venciedos 2".phpmkr_error()."sql: ".$sqlVnec);
...
function AplicaPagos($conn){
	...
	// SELECCIONAMOS  LA FECHA DE CIERRE DE PERIDO
	$sqlcIERREpERIODO = " SELECT fecha  FROM cierre_periodo ORDER BY cierre_periodo_id DESC  LIMIT 0,1 ";
	$rscIERREpERIODO = phpmkr_query($sqlcIERREpERIODO,$conn) or die("Error en cierre periodo".$sqlcIERREpERIODO);
	$rowsqlcIERREpERIODO = phpmkr_fetch_array($rscIERREpERIODO);
	$x_fecha_cierre_periodo = $rowsqlcIERREpERIODO["fecha"];
	...
	while($row = phpmkr_fetch_array($response)){
		...
		if($x_fecha_movimiento > $x_fecha_cierre_periodo){
		$sqlRef = "SELECT `credito_id` , `credito_num` , `num_pagos`   FROM `credito`  WHERE `tarjeta_num` =  $x_ref_pago  AND (`credito_status_id` = 1 || `credito_status_id` = 4 || `credito_status_id` = 5)";
		...
		echo "condicions del credito ".$rsCondicionesCredito."<br>";
		$x_c_garantia_liquida = $rowCondicionesCredito["garantia_liquida"] + 0;
		$x_c_penalizacion = $rowCondicionesCredito["penalizacion"] + 0;
		...
		if (!empty($x_c_penalizacion) && $x_c_penalizacion > 0  ){
			...
			echo "SQL VENCIDOS ".$sqlVencidos."<br>";
			echo "el vencimiento de referencia es ".$x_no_referencia_ven."<br>";
		...
		#	$sqlSaldoLA = "SELECT vencimiento_id FROM vencimiento WHERE credito_id = $x_credito_id and fecha_vencimiento <= '$x_fecha_fin_mes' and vencimiento_status_id in (1,3,6)";
			...
			$rsSaldoLA = phpmkr_query($sqlSaldoLA, $conn) or die ("error en saldos"." 1 ". phpmkr_error()."sql:".$sqlSaldoLA);
			...
			$sqlSaldoLA  = "SELECT * FROM vencimiento WHERE credito_id = $x_credito_id and   vencimiento_num > $x_no_referencia_ven and vencimiento_num < 2000 and vencimiento_status_id in (1,3,6,7) order by  vencimiento_num asc";
			$rsSaldoLA = phpmkr_query($sqlSaldoLA, $conn) or die ("error en saldos"." 2 ". phpmkr_error()."sql:".$sqlSaldoLA);
			$x_total_liquida_con_interes = 0;
			$x_lista_con_interes = "";
			while ($rowSaldoLA = phpmkr_fetch_array($rsSaldoLA)){
			#PAGAMOS EL VENCIMIENTO, CON TODO Y SUS INTERESES..
			...
			$sqlSaldoLA = "SELECT * FROM vencimiento WHERE credito_id = $x_credito_id and   vencimiento_num > $x_no_referencia_ven and vencimiento_num >= 2000 and vencimiento_status_id in (1,3,6,7) order by  vencimiento_num asc";			
			$rsSaldoLA = phpmkr_query($sqlSaldoLA, $conn) or die ("error en saldos"." 3 ". phpmkr_error()."sql:".$sqlSaldoLA);
			$x_total_liquida_con_interes = 0;
			$x_lista_con_interes = "";
			while ($rowSaldoLA = phpmkr_fetch_array($rsSaldoLA)){
			#PAGAMOS EL VENCIMIENTO, CON TODO Y SUS INTERESES..
			...
			$x_credito_liquidado = 1;
				...
				// el credito no se puede pagar masivo el monto es mayor a loq ue debe, pero no se puede plicar un liquidado x anticipadp
				...
				$GLOBALS["x_no_aplicados_reporte"] .= "<tr>
											  ...
											  <td><font color=\"#FF0000\" style=\"font-size:11px;\" >El monto del importe es mayor a lo que se debe, pero no se puede liquidar por anticipado n&uacute;mero de referencia tiene asociado m&aacute;s de 1 cr&eacute;dito activo.</font></td>
											  </tr>";
		...
		#primero validamos la parte de la garantia
				...
				// si sobrante es == 0
		echo "penalizacion".$x_c_penalizacion."<br>";
		echo "garantia liquida".$x_c_garantia_liquida."<br> liquidado ".$x_credito_liquidado."<br>";		
			...
			#3.- terminando todo el proceso de pago volvemos a cambiar el status del pago 12... y si ya no hay pagos pendiestes ni vencidoe netonces pagamos el pago 12 con la garantia y cambiamos el status de la garantia a utulizado para liquidar credito. 
			 ...
			 if ( $x_total_penalizaciones > 0  ){
				 ...
				 if ($x_c_forma_pago == 1){
					 ...
					 if($x_num_pagos == 40){
						 // se deben seleccionar 3 vencimientos
						 $sqlUpdateUltimoVencmeinto = "UPDATE vencimiento SET  vencimiento_status_id  = 9 WHERE credito_id = $x_credito_id AND vencimiento_num IN ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1 ,$x_antepenultimo_nn_menos_2 )  and vencimiento.vencimiento_status_id not in (2,4,5,8)";
						 echo $sqlUpdateUltimoVencmeinto;
						 }else{					 				 
					 $sqlUpdateUltimoVencmeinto = "UPDATE vencimiento SET  vencimiento_status_id  = 9 WHERE credito_id = $x_credito_id AND vencimiento_num IN ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1) and vencimiento.vencimiento_status_id not in (2,4,5,8)";
					  echo $sqlUpdateUltimoVencmeinto;
					 }
				     ...
				     # echo"dobel vencimeinto cambio status".$sqlUpdateUltimoVencmeinto."<br>";
						 ...
						 echo "no es semanal <br>";
						 if((($x_c_forma_pago == 2) || ($x_c_forma_pago == 4)) && ($x_num_pagos == 24)){
							 #catorcenal y quincenal con 24 pagos se toas dos pasgo de garantia....
						$sqlUpdateUltimoVencmeinto = "UPDATE vencimiento SET  vencimiento_status_id  = 9 WHERE credito_id = $x_credito_id AND vencimiento_num IN ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1) and vencimiento.vencimiento_status_id not in (2,4,5,8) ";
							 
							 }else{
						 //solo se cambia el status al ultimo vencimiento
					       $sqlUpdateUltimoVencmeinto = "UPDATE vencimiento SET  vencimiento_status_id  = 9 WHERE credito_id = $x_credito_id AND vencimiento_num = $x_ultimo_vencimiento_nn  and vencimiento.vencimiento_status_id not in (2,4,5,8) ";
							 }
						 }
		...
		echo  "vencimientos actualizados ".$sqlUpdateUltimoVencmeinto."......<br><br>";
		
			
			if($x_credito_liquidado != 1){
		$x_total_a_pagar_vencimientos = calculaTotalVencimientos($x_credito_id,$conn); //kuki
			}
	...
	echo "despues: ";
		...
		if($x_total_a_pagar_vencimientos < $x_importe){			
			...
			echo "el credito tiene penalizaciones cuano el campo es mayor de 0";		
		...
		// echo "recalculamos los moratorios <br>";
			...
			$SQLpp = "SELECT monto, status as status_garantia FROM garantia_liquida where credito_id = $x_credito_id ";
			$RSPP = phpmkr_query($SQLpp,$conn) or die ("Error al seleccionar los vencimeintos faltantes del credito". phpmkr_error()."sql:".$SQLpp);
			$ROWpp = phpmkr_fetch_array($RSPP);
			$x_monto_garantia = $ROWpp["monto"];
			$x_status_garantia = $ROWpp["status_garantia"];
			...
			************************************************/	
			...
			if(($x_numero_vencimientos_pendientes == $x_vencimientos_pendintes_por_cambio) && ($x_c_garantia_liquida > 0)){
				...
				}// grantia liquida vigente
				}// pendientes igual a monto de garantia
			}
	...
	// mandamos el mensaje de pago efectuado
		 ...
		 if(!empty($x_no_celular) &&  $x_no_celular != "5555555555" && $x_no_celular != "0000000000"){
			// $x_pagado  =  $GLOBALS["x_importe"];
			 ...
			 $x_fecha = ConvertDateToMysqlFormat($GLOBALS["x_fecha_pago"]);
						...
						$x_mensaje = "CREA CRED $x_credito_num PAGO EFECTUADO $x_pagado el $x_fecha_de_pago  ";
	 ...
	 $sqlInsertsms =  "INSERT INTO `sms_enviados` (`id_sms_enviado`, `id_tipo_mensaje`, `contenido`, `no_credito`, `no_celular`, `fecha_registro`, `tipo_envio`, `destino` , `cliente_id`) VALUES (NULL, '2','" .$x_mensaje."', $x_credito_num, '".$x_no_celular."', '".$x_fecha_sms."', '1', '1',$x_cliente_idl)";
...
$sqlInsertsms =  "INSERT INTO `sms_mail_enviados` (`id_sms_mail_enviado`, `id_tipo_mensaje`, `contenido`, `no_credito`, `email`, `fecha_registro`, `tipo_envio`, `destino` , `cliente_id`) VALUES (NULL, '2','" .$mensajeMail."',  $x_credito_num, '".$email."', '".$x_fecha_sms."', '1', '1',$x_cliente_idl)";
				...
				if ($x_sobrante == 1){
					if($x_credito_liquidado == 1){						
						$GLOBALS["x_no_aplicados_reporte"] .= "<tr>
											  ...
											  <td><font color=\"#2d41fb\" style=\"font-size:11px;\" >El cr&eacute;dito fue l&iacute;quidado por anticipado. Se debe regresar el monto de la garant&iacute;a</font></td>
											  </tr>";
					...
					$GLOBALS["x_importe_NO_aplicado"] =	$GLOBALS["x_importe_NO_aplicado"] + $x_importe;
						$GLOBALS["x_no_aplicados_reporte"] .= "<tr>
											  	  <td><font color=\"#FF0000\" style=\"font-size:11px;\">$x_ref_pago</font>&nbsp; &nbsp; &nbsp;</td><td>&nbsp;</td>
											  <td><font color=\"#FF0000\" style=\"font-size:11px;\" >El importe pagado es mayor que el importe de los vencimientos pendientes, el cr&eacute;dito puede tener garant&iacute;a l&iacute;quida.</font></td>
											  </tr>";
						$GLOBALS["x_pagos_NO_aplicados"]++;
						}
...
//	 si no hay vencimeniento vencidos cerramos las tareas del crm y el caso tambien.
	...
	if($x_caso_abierto > 0 && !empty($crm_caso_id_v)){
		...
		$rswrk = phpmkr_query($sSqlWrkcasos,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrkcasos);
			...
			// la fecha del moviiento es mernor  a la fecha de cierre de periodo; no se puede realizar el pago
			$GLOBALS["x_importe_NO_aplicado"] =	$GLOBALS["x_importe_NO_aplicado"] + $x_importe;
						$GLOBALS["x_no_aplicados_reporte"] .= "<tr>
											  	  <td><font color=\"#FF0000\" style=\"font-size:11px;\">$x_ref_pago</font>&nbsp; &nbsp; &nbsp;</td><td>&nbsp;</td>
											  <td><font color=\"#FF0001\" style=\"font-size:11px;\" >La fecha de movimeinto del pago es menor  a la fecha de CIERRE DE PERIODO. NO SE PUEDE REGISTRAR EL PAGO</font></td>
											  </tr>";
						$GLOBALS["x_pagos_NO_aplicados"]++;
	...
	function montoParaLiquidar($x_credito_id, $x_fecha_movimiento, $conn){
		...
		echo "entro a monto para liquidar (funcion)<br>";
...
if(!empty($x_no_referencia_ven)){
	...
	$x_no_referencia_ven = $rowPendiente["vencimiento_num"];			
		...
		$rsSaldo = phpmkr_query($sqlSaldo, $conn) or die ("error en saldos"." 4 ". phpmkr_error()."sql:".$sqlSaldo);
		$x_total_liquida_con_interes = 0;
		$x_lista_con_interes = "";
		...
		$sqlSaldo2 = "SELECT * FROM vencimiento WHERE credito_id = $x_credito_id and   vencimiento_num > $x_no_referencia_ven and vencimiento_num < 2000 and vencimiento_status_id in (1,3,6,7) order by  vencimiento_num asc";
		$rsSaldo2 = phpmkr_query($sqlSaldo2, $conn) or die ("error en saldos"." 5 ". phpmkr_error()."sql:".$sqlSaldo2);
	...
	#	echo "total liquida sin intetres".$x_total_liquida_sin_interes."<br>";
		...
		$sqlSaldo3 = "SELECT * FROM vencimiento WHERE credito_id = $x_credito_id and   vencimiento_num > $x_no_referencia_ven and vencimiento_num >= 2000 and vencimiento_status_id in (1,3,6,7) order by  vencimiento_num asc";
		$rsSaldo3 = phpmkr_query($sqlSaldo3, $conn) or die ("error en saldos"." 6 ".  phpmkr_error()."sql:".$sqlSaldo3);
...
os_correctos"] + $GLOBALS["x_pagos_aplicados_diferente"];
		$pna =$GLOBALS["x_pagos_NO_aplicados"];
		$pai =$GLOBALS["x_importe_aplicado"];
		$pnai= $GLOBALS["x_importe_NO_aplicado"];
		$pc =$GLOBALS["x_pagos_aplicados_correctos"];
