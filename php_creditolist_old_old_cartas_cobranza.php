...
if(!empty($_POST["x_credito_tipo_id"])){
	$_SESSION["x_credito_tipo_id"] = $_POST["x_credito_tipo_id"];
		...
		// si viene vacio significa que no se selecciono ningun tipo de credito
		// y se buscan todos lo tipos de creditos
...
if(($_SESSION["x_credito_tipo_id"] != 2)  ){
 //echo "entra a diferente de dos";
	...
	// En Credito
	if((!empty($_SESSION["x_crenum_srch"])) || (!empty($_SESSION["x_cresta_srch"])) || (!empty($_SESSION["x_credito_tipo_id"])) ){
		$ssrch_cre = "";
		...
		//si se selecciono pero no es TODOS que tiene un valor de 100
				...
				$ssrch_cre .= "(credito.credito_tipo_id = ".$_SESSION["x_credito_tipo_id"].") AND ";
						}	
		}
...
//grupos
 //echo "entra a asociados";
	...
	// EN clientes
	if((!empty($_SESSION["x_nombre_srch"])) || (!empty($_SESSION["x_apepat_srch"])) || (!empty($_SESSION["x_apemat_srch"])) || (!empty($_SESSION["x_clinum_srch"]))){
		//echo "entra a nombre paellidos etc";
		$ssrch = "";
		if(!empty($_SESSION["x_nombre_srch"])){
			$ssrch .= "(cliente.nombre_completo like '%".$_SESSION["x_nombre_srch"]."%') AND ";
		}
		...
		$ssrch_sql = "select solicitud.solicitud_id from solicitud join referencia on referencia.solicitud_id = solicitud.solicitud_id join 
		solicitud_inciso on solicitud_inciso.solicitud_id = solicitud.solicitud_id join cliente on cliente.cliente_id = solicitud_inciso.cliente_id where ".$ssrch;
		$rs_qry = phpmkr_query($ssrch_sql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
		$nTotalRecs = phpmkr_num_rows($rs_qry);
		if($nTotalRecs >0){
			while ($row_sqry = @phpmkr_fetch_array($rs_qry)) {
	...
	if((!empty($_SESSION["x_crenum_srch"])) || (!empty($_SESSION["x_cresta_srch"])) || (!empty($_SESSION["x_crenum_srch"]) )){
...
<table width="785" border="0" cellpadding="0" cellspacing="0">
	<tr>
	  ...
	  <td>&nbsp;</td>
	  <td><!-- <input name="x_credito_tipo_id" type="radio" id="x_credito_tipo_id" value="1" <?php if($_SESSION["x_credito_tipo_id"] == 1){ echo "checked='checked'"; }?> onclick="javascript: document.filtros.submit();" /> -->
<!-- Personal --> <?php
		$conn = phpmkr_db_connect(HOST, USER, PASS, DB, PORT);  
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_creditolist.php?start=<?php echo $PrevStart; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_creditolist.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_creditolist.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_creditolist.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_creditolist.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_creditolist.php?start=<?php echo $NextStart; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
Crédito No
<?php }else{ ?>
	<a href="php_creditolist.php?order=<?php echo urlencode("credito_id"); ?>">Crédiro No<?php if (@$_SESSION["credito_x_credito_id_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["credito_x_credito_id_Sort"] == "ASC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
<?php } ?>
		</span></td>
		<td valign="top"><span>
Fondo
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount >= $nStartRec) {
		...
		$sqlFormato = "SELECT formato_nuevo FROM  solicitud WHERE solicitud_id = $x_solicitud_id ";
