...
//calculos de la tabla dos
	...
	while($x_cont_cal <= 10){
		...
		$GLOBALS["x_monto_pago_int_$x_cont_cal"] = round($x_monto_por_integrante * 100) / 100 ;
...
while($x_no_integrante <= 10){
 ...
 // si el nombre no esta vacio y el cliente_id corresponde con el que se ingreso
	if(!empty($x_nombre_int) && ($x_id_int == $x_integrante_id)){
	...
	$x_monto_integrante =$x_monto_integrante + 5;
	$x_monto_integrante = round($x_monto_integrante * 100) / 100 ;
...
<table class='ewTable' width='700' border='1' align='center'>
  <tr>
    <td colspan='5' align='left'>$x_nombre_int</td>
  </tr>
  <tr>
    <td>FECHA</td>
	 <td>MONTO DE PAGO</td>
...
while($x_ciclo < $x_cont_fetc_v){	
	...
	$x_fecha = $$x_fecha;
	//$x_fecha =  FormatDateTime(@$x_fecha;

	$_integrates_tablas .="<tr>
    <td>".FechaFormateada(strtotime(@$x_fecha))."</td>
    <td>".round($x_monto_integrante * 100)/100 ."</td>
	<td>&nbsp;</td>
	<td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>";
 ...
 $_integrates_tablas .= "</table>";
	$_integrates_tablas .= " <table  width='800' align='center'>
	<tr><td>&nbsp;</td></tr>
	<tr><td>&nbsp;</td></tr>
	<tr>
	<td align='justify' style='font-family: Verdana; font-size: 13px;'> Este documento es &uacute;nicamente de car&aacute;cter informativo no tiene ninguna validez oficial, y solo se entrega como un apoyo  de control interno para el grupo y procurar el pago puntual del cr&eacute;dito suscrito.<p>
	</td></tr>
	</table>";
