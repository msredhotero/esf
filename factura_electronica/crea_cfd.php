...
//mientras no se termine el archivo, lee el archivo y lo guarda en la cadena de certificado. 
while (!feof($ar))
	  {
		$certificado_texto.= fgets($ar);
	  }	  
// cierra el archivo de certificado.	  
fclose($ar);
	
//recupero las variables
$forma_pago=trim($_REQUEST[forma_pago]);
$tipo_cfd=trim($_REQUEST[tipo_cfd]);
trim($fecha=$_REQUEST[fecha]);
$aprobacion=trim($_REQUEST[aprobacion]);
$year_aprobacion=trim($_REQUEST[year_aprobacion]);
$serie=trim($_REQUEST[serie]);
$folio=trim($_REQUEST[folio]); 
$dias_credito=trim($_REQUEST[dias_credito]); 
$iva=trim($_REQUEST[iva]);
$num_certificado=trim($_REQUEST[numero_certificado]);
	
$rfc=trim($_REQUEST[rfc]); 
$razon_social=trim($_REQUEST[razon_social]);
$calle= trim($_REQUEST[calle]);
$num_exterior= trim($_REQUEST[num_exterior]);
...
$codigo_postal=trim($_REQUEST[codigo_postal]);
