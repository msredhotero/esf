...
<p><span class="phpmaker">Add to TABLE: RECIBOS<br><br><a href="php_recibolist.php">Back to List</a></span></p>
...
<table class="ewTable">
	<tr>
		<td class="ewTableHeader"><span>Vencimiento</span></td>
		<td class="ewTableAltRow"><span>
<?php if (!(!is_null($x_vencimiento_id)) || ($x_vencimiento_id == "")) { $x_vencimiento_id = 0;} // Set default value ?>
<?php
$x_vencimiento_idList = "<select name=\"x_vencimiento_id\">";
$x_vencimiento_idList .= "<option value=''>Please Select</option>";
$sSqlWrk = "SELECT `vencimiento_id` FROM `vencimiento`";
$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
if ($rswrk) {
	$rowcntwrk = 0;
	while ($datawrk = phpmkr_fetch_array($rswrk)) {
		$x_vencimiento_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
		if ($datawrk["vencimiento_id"] == @$x_vencimiento_id) {
			$x_vencimiento_idList .= "' selected";
		}
		$x_vencimiento_idList .= ">" . $datawrk["vencimiento_id"] . "</option>";
		$rowcntwrk++;
	}
}
@phpmkr_free_result($rswrk);
$x_vencimiento_idList .= "</select>";
echo $x_vencimiento_idList;
?>
</span></td>
	</tr>
	<tr>
