...
$ssrch_cre_2 = "((credito.credito_id in ($x_listado_credito)) ) AND ";
#echo "sql ".$ssrch_cre ."<br>";	
}
	...
	if(!empty($filter["x_empresa_id"])){
		...
		//echo "<br>susrsal antes de  if".$filter["x_sucursal_srch"]."...<br>";
																																																																																																																			  if ((!empty($filter["x_sucursal_srch"])) || (!empty($filter["x_promo_srch"]))){
					#echo "x_sucursal_srch viene llena".$filter["x_sucursal_srch"]."<br>"; 																																																																																																														   // se unen los dos queries..
		if(($filter["x_sucursal_srch"] != "1000") && (($filter["x_promo_srch"] != "1000"  )&& ($filter["x_promo_srch"] >0))){
	...
	if(!empty($filter["x_fecha_desde"]) && !empty($filter["x_fecha_hasta"]) ){
		...
		if(!empty($ssrch_cre)){
			//echo "<br> sql suc".$ssrch_cre ."<br>";
			$ssrch_cre  = $ssrch_cre . "AND ";
			}
		}
...
$sWhere = $ssrch_cli.$ssrch_cre.$ssrch_cre_2.$sql_fet.$sDbWhereEncargado;
...
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_credito_liquidadolist.php?export=excel&x_sucursal_srch=<?php echo $filter["x_sucursal_srch"];?>&x_promo_srch=<?php echo $filter["x_promo_srch"];?>&x_fecha_desde=<?php echo FormatDateTime(@$filter["x_fecha_desde"],7); ?>&x_fecha_hasta=<?php echo FormatDateTime(@$filter["x_fecha_hasta"],7); ?> ">Exportar a Excel</a>
&nbsp;&nbsp;
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
	...
	<tr>
	  ...
	  <td><span class="phpmaker">
		...
		if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
			...
			$sSqlWrk = "SELECT sucursal_id, nombre FROM sucursal Where sucursal_id = ".$_SESSION["php_project_esf_SucursalID"];
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount >= $nStartRec) {
		...
		$x_status = $row["status"];
		$dias_plazo = (strtotime($x_fecha_vencimiento)-strtotime($x_fecha_otrogamiento))/86400;
		$sqlDiasD =  " SELECT DATEDIFF('$x_fecha_vencimiento','$x_fecha_otrogamiento') as dias_plazo";
		$rsDiasD  = phpmkr_query($sqlDiasD,$conn) or die ("Error");
		$rowDias = phpmkr_fetch_array($rsDiasD);
		$dias_plazo =  $rowDias["dias_plazo"];
		
		$sqlDiasD =  " SELECT DATEDIFF('$x_fecha_liquida','$x_fecha_otrogamiento') as dias_liquida";
		$rsDiasD  = phpmkr_query($sqlDiasD,$conn) or die ("Error");
		$rowDias = phpmkr_fetch_array($rsDiasD);
		$dias_liquida =  $rowDias["dias_liquida"];
		
		
		//echo "x_fecha_vencimiento".$x_fecha_vencimiento ."- x_fecha_otrogamiento" .$x_fecha_otrogamiento;
		if($x_status == 1){
			$x_status= "ACTIVO";
			}if($x_status == 2){
				$x_status= "CANCELADO";
...
<td><?php echo FormatDateTime($x_fecha_otrogamiento,7); ?></td>
<td><?php echo FormatDateTime($x_fecha_vencimiento,7); ?></td>
...
<?php } ?>
<td align="right"><span>
<?php echo  FormatDateTime($x_fecha_liquida,7); ?>
</span></td>
