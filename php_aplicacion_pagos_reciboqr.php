...
$x_tarjeta = $_GET["x_tarjeta"];
$x_cliente_id = $_GET["x_cliente_id"];// NUMERO DE CLIENTE; NO EL ID	
$x_folio = $_GET["x_folio"];
$x_monto = $_GET["x_monto"];
$x_fecha = $_GET["x_fecha"];
$qr_id = $_GET["x_qr_id"];
echo "x_monto**********".$x_monto."<br>";
...
if(!AplicaPagosQR($conn,$x_credito_id,$x_tarjeta,$x_cliente_id,$x_folio,$x_monto,$x_fecha,$qr_id)){
...
if(!AddReport($conn)){
	echo "No se guardo el reporte de la transaccio ====n";
//	die();
	}else{
		//die();
		}
...
<?php if ($sExport == "") { ?>
Aplicaci&oacute;n de Pagos por recibo QR (Resultado - <?php echo FormatDateTime($currdate,7);?>)
<?php }else{ ?>
Aplicaci&oacute;n de Pagos por recibo QR (Resultado - <?php echo FormatDateTime($currdate,7);?>)
<?php } ?>
</span></p>
...
function calculaTotalVencimientos($credito_id, $conn){
	...
	*/
	...
	echo "sql venci tol ".$sqlVnec."<br>";
	echo "total a pagar dentro de ven fun ".$total_vencimientos."<br>";
...
function AplicaPagosQR($conn,$credito_id,$tarjeta,$cliente_num,$folio,$importe,$fecha,$qr_id){
	
	echo "entra a Aplica pagos<br>";
	
	
	$x_credito_id = $credito_id;
	$x_tarjeta = $tarjeta;
	$x_cliente = $cliente_num;
	$x_monto_importe = $importe;
	$x_folio_qr = $folio;
	$x_fecha = $fecha;
	$x_qr_id = $qr_id;
	echo "monto_importe".$x_monto_importe."- ";
	...
	//datos para lso recibos
	$x_today_z = date("Y-m-d");
	$GLOBALS["x_banco_id"] = 1;// banco inbursa
	$GLOBALS["x_medio_pago_id"] = 6;// referenciado inbursa;porque las promotoras hacen el deposito a la cuenta.
	$GLOBALS["x_referencia_pago_1"] = "Inbursa QR ".$x_folio_qr;
	$GLOBALS["x_fecha_pago"] = $x_today_z;
	$x_referencia_pago2 = "Pag mas. 0";
	...
	if(!empty($x_credito_id)){
		...
		$x_ref_pago = $x_tarjeta;		
		$x_numero_cliente = $x_cliente;
		$x_importe = $x_monto_importe;		
		echo "importe---".$x_importe;
		$x_fecha_movimiento = $x_fecha;		
		...
		if (!empty($x_c_penalizacion) && $x_c_penalizacion > 0  ){
			...
			echo "entrara an monto para liquidar<br>";
		...
		#	$sqlSaldoLA = "SELECT vencimiento_id FROM vencimiento WHERE credito_id = $x_credito_id and fecha_vencimiento <= '$x_fecha_fin_mes' and vencimiento_status_id in (1,3,6)";
			...
			//#6 INSERTA EN RECIBO
					...
					// Field referencia_pago
					$x_referencia_pago21 = "LIQUIDADO ANTICIPADO-QR".$x_folio_qr;
		...
		if($x_c_penalizacion > 0 && $x_c_garantia_liquida > 0 && $x_credito_liquidado != 1){
			...
			#3.- terminando todo el proceso de pago volvemos a cambiar el status del pago 12... y si ya no hay pagos pendiestes ni vencidoe netonces pagamos el pago 12 con la garantia y cambiamos el status de la garantia a utulizado para liquidar credito. 
			 ...
			 if ( $x_total_penalizaciones > 0 ){
				...
				if($x_num_pagos == 40){
						 // se deben seleccionar 3 vencimientos
						 $sqlUpdateUltimoVencmeinto = "UPDATE vencimiento SET  vencimiento_status_id  = 9 WHERE credito_id = $x_credito_id AND vencimiento_num IN ($x_ultimo_vencimiento_nn, $x_ultomo_vencimeinto_nn_menos_1 ,$x_antepenultimo_nn_menos_2 ) ";
			...
			************************************************/	
			...
			if(($x_numero_vencimientos_pendientes == $x_vencimientos_pendintes_por_cambio) && ($x_c_garantia_liquida > 0)){
				...
				while($rowV9 = phpmkr_fetch_array($rsV9)){
					...
					$x_referencia_pago2 = $x_lista_v9_ne."QR ".$x_folio_qr."";
				...
				} // la garantia esta vigente
				}// vencimientos pendietes = garantia_liquida
			}
			 ...
			 ////echo"Salimos del while";
				...
				phpmkr_query($sSqlUr,$conn) or die("Error al actulizar referencia de pago dos".phpmkr_error()."sql :".$sSqlUr);
					...
					$sSqlUr = "UPDATE recibo SET referencia_pago_2  = \"RP $x_folio_qr QR 0\" WHERE  recibo_id = $x_recibo_id ";
					phpmkr_query($sSqlUr,$conn) or die("Error al actulizar referencia de pago dos".phpmkr_error()."sql :".$sSqlUr);
					}
				}// if x_recibo_id > 0
...
//	 si no hay vencimeniento vencidos cerramos las tareas del crm y el caso tambien.
	...
	if($x_caso_abierto > 0 && !empty($crm_caso_id_v)){
		...
		$rswrk = phpmkr_query($sSqlWrkcasos,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrkcasos);
			...
			//actualizamos el registro del recibo QR para que ya no se peuda usar el link de pago.
			$sqlLink = "UPDATE recibo_qr SET pago_aplicado = 1 WHERE recibo_qr_id = $x_qr_id";
			$rsLink = phpmkr_query($sqlLink,$conn) or die ("Error al actualizar QR".phpmkr_error()."sql:".$sqlLink);
	...
	function montoParaLiquidar($x_credito_id, $x_fecha_movimiento, $conn){
		...
		echo "entra a monto para liquidar<br>";
	//	die();
