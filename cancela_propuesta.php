...
if($x_convenio_propuesta_id > 0) {	
		...
		if($x_status == 1){
			$sql  = "UPDATE `convenio_propuesta` SET status_id = 2 WHERE convenio_propuesta_id= $x_convenio_propuesta_id";
			if (($_SESSION["php_project_esf_status_UserRolID"] == 1) || (($_SESSION["php_project_esf_status_UserRolID"] == 12))){
			$x_result .='<input type="checkbox" name="convenio_orden'.$x_convenio_propuesta_id.'" checked="checked" onclick="CancelaPropuesta('.$x_convenio_propuesta_id.');" />';
			$x_result .= "CANCELADO";
			}
			$x_status = "CANCELADO";		
			}else {
				...
				$x_result .='<input type="checkbox" name="cancela_propuesta" disabled="disabled"   />';
				}else{
					$x_result .='<input type="checkbox" name="cancela_propuesta'.$x_convenio_propuesta_id.'"  onclick="CancelaPropuesta('.$x_convenio_propuesta_id.');" />';
					}
