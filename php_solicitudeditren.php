...
switch ($sAction)
{
	...
	case "U": // Update
		if (EditData($conn)) { // Update Record based on key
			$_SESSION["ewmsg"] = "La nueva solicitud ha sido generada.";
			phpmkr_db_close($conn);
			ob_end_clean();
			header("Location: php_solicitudlist.php");
			exit();
...
function mismosdomtit(){
	...
	if(EW_this.x_mismos_titluar.checked == true){
		...
		if(validada == false){
			...
			EW_this.x_delegacion_id_temp1.value = EW_this.x_delegacion_id.value;
			...
			var indice = EW_this.x_entidad_id2.selectedIndex;
			EW_this.x_entidad_id3_neg.options[indice].selected = true;
	
			EW_this.x_delegacion_id_temp1_2.value = EW_this.x_delegacion_id2.value;
...
//		EW_this.x_delegacion_id2.options[indice].selected = true;
		...
		EW_this.x_delegacion_id_temp2.value = EW_this.x_delegacion_id3.value;
		EW_this.a_add.value = "D";	
		EW_this.submit();	
...
<form name="solicitudedit" id="solicitudedit" action="php_solicitudeditren.php" method="post" >
...
?>
      ...
      <tr>
		...
		<?php
		/*
		$x_credito_tipo_idList = "<select name=\"x_credito_tipo_id\" class=\"texto_normal\">";
		$x_credito_tipo_idList .= "<option value=''>Seleccione</option>";
		$sSqlWrk = "SELECT `credito_tipo_id`, `descripcion` FROM `credito_tipo`";
		$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
		if ($rswrk) {
			$rowcntwrk = 0;
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_credito_tipo_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["credito_tipo_id"] == @$x_credito_tipo_id) {
					$x_credito_tipo_idList .= "' selected";
				}
				$x_credito_tipo_idList .= ">" . $datawrk["descripcion"] . "</option>";
				$rowcntwrk++;
			}
		}
		@phpmkr_free_result($rswrk);
		$x_credito_tipo_idList .= "</select>";
		echo $x_credito_tipo_idList;
		*/
		?>        </td>
...
<tr>
      ...
      <tr>
        <td class="texto_normal">Ocupación:</td>
        <td colspan="4"><span class="texto_normal">
      ...
      <tr>
        <td class="texto_normal">&nbsp;</td>
        <td colspan="4"><span class="texto_normal">&nbsp;</span></td>
      </tr>
    </table>
  ...
  <tr>
    <td colspan="3" align="left" valign="top"><table width="700" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="134" class="texto_normal">Otros Ingresos: </td>
        <td width="204"><input class="importe" name="x_otros_ingresos" type="text" id="x_otros_ingresos" value="<?php echo htmlspecialchars(@$x_otros_ingresos) ?>" size="10" maxlength="10" onKeyPress="return solonumeros(this,event)" /></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td class="texto_normal">Origen:</td>
        <td><input class="texto_normal" name="x_origen_ingresos" type="text" id="x_origen_ingresos" value="<?php echo htmlspecialchars(@$x_origen_ingresos) ?>" size="30" maxlength="150" /></td>
      </tr>
      <tr>
...
<tr>
  ...
  <tr>
    <td>&nbsp;</td>
    <td><div align="center">
	<input name="Action" type="button"class="boton_medium" value="Renovar" onclick="EW_checkMyForm();" />
    </div></td>
    <td>&nbsp;</td>
  </tr>
</table>
