...
if($x_convenio_propuesta_id > 0) {	
		...
		if($x_status == 3){
			$sql  = "UPDATE `convenio_propuesta` SET status_id = 4 WHERE convenio_propuesta_id= $x_convenio_propuesta_id";
			if (($_SESSION["php_project_esf_status_UserRolID"] == 1) || (($_SESSION["php_project_esf_status_UserRolID"] == 4))){
			$x_result .='<input type="checkbox" name="convenio_orden'.$x_convenio_propuesta_id.'" checked="checked" onclick="ConvenioAplicado('.$x_convenio_propuesta_id.');" />';
			$x_result .= "CONVENIO APLICADO";
			}
			$x_status = "CONVENIO APLICADO";		
			}else {
				...
				$x_result .='<input type="checkbox" name="aplica_propuesta" disabled="disabled"   />';
					...
					$x_result .='<input type="checkbox" name="aplica_propuesta'.$x_convenio_propuesta_id.'"  onclick="ConvenioAplicado('.$x_convenio_propuesta_id.');" />';
					}
