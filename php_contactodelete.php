...
while ($i < count($arRecKey)){
	...
	// Build the SQL
	$sDbWhere .= "`contacto_id`=" . $sRecKey . " AND ";
...
switch ($sAction)
{
	...
	case "D": // Delete
		if (DeleteData($sDbWhere,$conn)) {
			$_SESSION["ewmsg"] = "El contacto ha sido eliminado.";
			phpmkr_db_close($conn);
			ob_end_clean();
			header("Location: php_contactolist.php");
			exit();
...
<form action="php_contactodelete.php" method="post">
<p>
