...
if (!empty($x_usuario_status_id)) {
	if ($sKey <> "") { $sKey .= ","; }
	$sKey .= $x_usuario_status_id;
}else{
	$bSingleDelete = false;
}
if (!$bSingleDelete) {
...
while ($i < count($arRecKey)){
	...
	// Build the SQL
	$sDbWhere .= "`usuario_status_id`=" . $sRecKey . " AND ";
...
<p><span class="phpmaker">Delete from TABLE: STATUS DE USUARIOS<br><br><a href="php_usuario_statuslist.php">Back to List</a></span></p>
<form action="php_usuario_statusdelete.php" method="post">
<p>
...
{
	global $x_usuario_status_id;
	$sSql = "Delete FROM `usuario_status`";
	$sSql .= " WHERE " . $sqlKey;
