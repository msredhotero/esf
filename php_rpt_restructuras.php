...
if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
	$sSql = "SELECT sum(vencimiento_restructura.importe) as importe, sum(vencimiento_restructura.interes) as interes, sum(vencimiento_restructura.iva) as iva, sum(vencimiento_restructura.interes_moratorio) as interes_moratorio, sum(vencimiento_restructura.iva_mor) as iva_mor, sum(vencimiento_restructura.total_venc) as total_venc, credito.credito_num, credito.credito_tipo_id, solicitud.solicitud_id, credito.credito_id FROM vencimiento_restructura join credito on credito.credito_id = vencimiento_restructura.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id join promotor on promotor.promotor_id = solicitud.promotor_id ";
}else{
	$sSql = "SELECT sum(vencimiento_restructura.importe) as importe, sum(vencimiento_restructura.interes) as interes, sum(vencimiento_restructura.iva) as iva, sum(vencimiento_restructura.interes_moratorio) as interes_moratorio, sum(vencimiento_restructura.iva_mor) as iva_mor, sum(vencimiento_restructura.total_venc) as total_venc, credito.credito_num, credito.credito_tipo_id, solicitud.solicitud_id, credito.credito_id FROM vencimiento_restructura join credito on credito.credito_id = vencimiento_restructura.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id join promotor on promotor.promotor_id = solicitud.promotor_id ";
}
...
if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
//	$sDbWhere = "(vencimiento_restructura.vencimiento_status_id = 1) AND ";
...
if($_SESSION["x_fecha_desde"] != ""){
	$sDbWhere .= "(vencimiento_restructura.fecha_restructura >= '".ConvertDateToMysqlFormat($_SESSION["x_fecha_desde"])."') AND ";
	$sDbWhere .= "(vencimiento_restructura.fecha_restructura <= '".ConvertDateToMysqlFormat($_SESSION["x_fecha_hasta"])."') AND ";	
}
...
//	$sDbWhere = substr($ssrch_cre, 0, strlen($ssrch_cre)-5);		
}
...
$sSql .= " GROUP BY vencimiento_restructura.credito_id ";
...
//	$sSql .= " ORDER BY " . $sOrderBy;
	$sSql .= " ORDER BY vencimiento_restructura.fecha_restructura , vencimiento_restructura.vencimiento_num ASC ";	
//}
...
<?php }else{ ?>
<b>
RESTRUCTURAS (Peri&oacute;do del: <?php echo FormatDateTime($_SESSION["x_fecha_desde"],7); ?> al: <?php echo FormatDateTime($_SESSION["x_fecha_hasta"],7); ?>)
</b>
<?php } ?>
<?php if ($sExport == "") { ?>
<?php if($_SESSION["x_fecha_desde"] != ""){ ?>
&nbsp;&nbsp;<a href="php_rpt_restructuras.php?export=excel&x_fecha_desde=<?php echo $x_fecha_desde; ?>&x_fecha_hasta=<?php echo $x_fecha_hasta; ?>">Exportar a Excel</a>
<?php }else{ ?>
&nbsp;&nbsp;<a href="php_rpt_restructuras.php?export=excel">Exportar a Excel</a>
<?php } ?>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
...
<form action="php_rpt_restructuras.php" name="filtro" id="filtro" method="post">
...
?></td>
	...
	<tr>
	  ...
	  <td><a href="php_rpt_restructuras.php?cmd=reset">Mostrar Todos</a></td>
	  </tr>
</table>
...
<?php if ($sExport == "") { ?>
<form action="php_rpt_restructuras.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_rpt_restructuras.php?start=<?php echo $PrevStart; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_rpt_restructuras.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_rpt_restructuras.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_rpt_restructuras.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_rpt_restructuras.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_rpt_restructuras.php?start=<?php echo $NextStart; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
$sSqlWrk = "SELECT sum(vencimiento.importe) as importe, sum(vencimiento.interes) as interes, sum(vencimiento.iva) as iva, sum(vencimiento.interes_moratorio) as interes_moratorio, sum(vencimiento.iva_mor) as iva_mor, sum(vencimiento.total_venc) as total_venc FROM vencimiento where vencimiento.credito_id = $x_credito_id"; 

$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
$datawrk = phpmkr_fetch_array($rswrk);
$x_importe_res = $datawrk["importe"];
$x_interes_res = $datawrk["interes"];
$x_iva_res = $datawrk["iva"];
$x_interes_moratorio_res = $datawrk["interes_moratorio"];
$x_iva_mor_res = $datawrk["iva_mor"];
$x_total_venc_res = $datawrk["total_venc"];
@phpmkr_free_result($rswrk);  

$x_total_importe_res = $x_total_importe_res + $x_importe_res;
$x_total_interes_res = $x_total_interes_res + $x_interes_res;
$x_total_iva_res = $x_total_iva_res + $x_iva_res;		
$x_total_moratorios_res = $x_total_moratorios_res + $x_interes_moratorio_res;
$x_total_iva_mor_res = $x_total_iva_mor_res + $x_iva_mor_res;				
$x_total_total_res = $x_total_total_res + $x_total_venc_res;
...
?>
    ...
    <td align="right">R</td>
		...
		<td align="right"><span>
<?php echo (is_numeric($x_importe_res)) ? FormatNumber($x_importe_res,2,0,0,1) : $x_importe_res; ?>
</span></td>
		<!-- interes -->
		<td align="right"><span>
<?php echo (is_numeric($x_interes_res)) ? FormatNumber($x_interes_res,2,0,0,1) : $x_interes_res; ?>
</span></td>
		<td align="right"><?php echo (is_numeric($x_iva_res)) ? FormatNumber($x_iva_res,2,0,0,1) : $x_iva_res; ?></td>
		<!-- interes_moratorio -->
		<td align="right"><span>
<?php echo (is_numeric($x_interes_moratorio_res)) ? FormatNumber($x_interes_moratorio_res,2,0,0,1) : $x_interes_moratorio_res; ?>
</span></td>
		<td align="right"><?php echo (is_numeric($x_iva_mor_res)) ? FormatNumber($x_iva_mor_res,2,0,0,1) : $x_iva_mor_res; ?></td>
		<td align="right"><span>
<?php echo (is_numeric($x_total_venc_res)) ? FormatNumber($x_total_venc_res,2,0,0,1) : $x_total_venc_res; ?>
</span></td>
	</tr>
...
<td align="right"><span>
  <b>
  <?php echo FormatNumber($x_total_importe_res,2,0,0,1); ?>
  </b>
</span></td>
<td align="right"><span>
<b>
<?php echo FormatNumber($x_total_interes_res,2,0,0,1); ?>
</b>
</span></td>

<td align="right"><span>
<b>
<?php echo FormatNumber($x_total_iva_res,2,0,0,1); ?>
</b>
</span></td>

<td align="right"><span>
<b>
<?php echo FormatNumber($x_total_moratorios_res,2,0,0,1); ?>
</b>
</span></td>

<td align="right"><span>
<b>
<?php echo FormatNumber($x_total_iva_mor_res,2,0,0,1); ?>
</b>
</span></td>
<td align="right"><span>
<b>
<?php echo FormatNumber($x_total_total_res,2,0,0,1); ?>
</b>
</span></td>
</tr>
