...
if (empty($x_responsable_id)) {
	...
	$sSql = "select * from responsable_sucursal where responsable_sucursal_id = $x_responsable_id";
	$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
	$row = phpmkr_fetch_array($rs);
	$x_nombre = $row["nombre_completo"];
	$x_email = $row["email"];	
...
switch ($sAction)
{
	...
	case "A": // Add
		...
		$sSql = "update responsable_sucursal set usuario_id = $x_usuario_id where responsable_sucursal_id = ".$GLOBALS["x_responsable_id"];
		$x_result = phpmkr_query($sSql, $conn);
		if(!$x_result){
			echo phpmkr_error() . '<br>SQL: ' . $sSql;
			phpmkr_query('rollback;', $conn);	 
		...
		<!--
		window.opener.document.promotoredit.x_usuario_id.value=$x_usuario_id;			
		window.opener.document.promotoredit.a_edit.value='N';			
		window.opener.document.promotoredit.submit();
		window.close();
		//-->
		</script>";
//		echo "<div align=\"center\" class=\"phpmaker\"><strong><a href=\"javascript:window.close()\">Cuenta creada, Cerrar Ventana</a></strong></div>";
		break;
...
<form name="usuarioadd" id="usuarioadd" action="php_cat_usuario_responsableadd.php" method="post" onSubmit="return EW_checkMyForm(this);">
<p>
...
<p>
<div align="center" class="phpmaker"><strong><u>Usuarios para Responsable de Sucursal </u></strong></div>
<br>
...
function AddData($conn, $x_responsable_id)
{
