...
?>
	...
	<tr>
	  ...
	  <td>&nbsp;</td>
	  <td colspan="2"><a href="php_rpt_devengados_no_cobrados_activa.php" target="_blank">Ver detalle de DEVENGADOS</a></td>
	  </tr>
</table>
</form>
</span></p>
...
<table id="ewlistmain" class="ewTable" align="left">
	...
	<tr class="ewTableHeaderThin" >
		<td valign="top" rowspan="2"><span>Fecha</span></td>
		<td valign="top"  rowspan="2"><span>Otorgado</span></td>        
		<td valign="top"  rowspan="2"><span>Pagado</span></td>
		<td valign="top"  rowspan="2"><span>Cartera Total</span></td> 		 
		<td colspan="3" align="center">Devengados</td>
   		<td valign="top"  rowspan="2"><span>Cartera Total + Devengados</span></td>           
	</tr> <tr><td>Interes</td><td>&nbsp;Moratorio</td><td>Comision</td>
  </tr>
<?php
$nRecCount = 0;
...
// Build SQL
	...
	$x_importe_ccc = 0;
	$x_saldo_c = 0;
	$sSql = "SELECT credito_id  from credito join solicitud on solicitud.solicitud_id = credito.solicitud_id where credito.credito_status_id not in (2,5,4) and credito.fecha_otrogamiento <= '".$x_fecha_actual."'".$x_filtros;
	$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
...
#echo "<BR>".$sSql."<BR>";
	...
	$x_tot_moratorios = 0;
	$x_tot_total_fecha = 0;
	/*echo "<br>total otorgado ".$x_tot_otorgado;
	echo "<br>total pagado ".$x_tot_pagado;
	echo "<br>Int Dev no Pagado ".$x_interes_devengado_no_pagado;
	echo "<br>Mora Dev no Pagada ".$x_mora_devengado_no_pagado;
	echo "<br>Mora COM Dev no Pagada ".$x_mora_devengado_no_pagado_com;
echo "<br>total 1 = " .$x_tot_otorgado." - " .$x_tot_pagado." + (".$x_interes_devengado_no_pagado." + ". $x_mora_devengado_no_pagado." + ".$x_mora_devengado_no_pagado_com ." ) = <br>";
echo ($x_tot_otorgado -  $x_tot_pagado )+ ($x_interes_devengado_no_pagado + $x_mora_devengado_no_pagado +$x_mora_devengado_no_pagado_com);
echo "<br>";*/
	?>
		<!-- Table body -->
	...
	<?php echo FormatNumber($x_tot_otorgado,2,0,0,1); ?>
			...
			<!-- importe -->
			<td align="right"><span><?php echo FormatNumber($x_tot_pagado,2,0,0,1); ?></span></td>	
