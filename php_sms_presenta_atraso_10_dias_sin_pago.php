...
if($x_vencidos > 0){
	...
	// que su ultimo pago tenga mas de 10 dias de registrado.
		...
		$sqlUltimoPago="SELECT recibo . * 
						...
						WHERE vencimiento.credito_id = $x_credito_id ORDER BY fecha_registro DESC  LIMIT 0,1";				
		...
		$x_today = date("Y-m-d");
		$sqlDif = "SELECT DATEDIFF('$x_today','$x_fecha_ultimo_pago') AS dias";
	...
	while($rowVencimientos = phpmkr_fetch_array($rsVV)){			
		...
		//	echo $sqlClientel;
			...
			if(($x_cliente_idl > 0) && ($x_dias_sin_pago > 9)){
		 ...
		 //creditos que se liquidaron y que no tienen comision de cobranza generada.. solo incluye los que entran en el nuevo esquema de penalizaciones
			  ...
			  // los que no tienen comision generada
				  ...
				  $x_mensaje = "FINANCIERA CREA: SU CREDITO PRESENTA UN ATRASO";
				   $x_mensaje .= " POR $".FormatNumber($x_general,2,0,0,1)." CUIDE SU HISTORAL CREDITICIO Y EVITE MAYORES RECARGOS PAGUE A LA BREVEDAD";
				  
				 
				  echo "<BR> CREDITO_ID ".$x_credito_id." CLIENTE ID ".$x_cliente_idl." dias sin pago ".$x_dias_sin_pago." fecha ultimo pago ".$x_fecha_ultimo_pago;
				  echo "<br>". $x_mensaje;
