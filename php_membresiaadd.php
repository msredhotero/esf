...
<script language="javascript">
	...
	$('#x_tipo').change(function(e) {
        var tipo_membresia = $('#x_tipo').val();
		var valor = 0;
		if(tipo_membresia == 1){
			valor = 150;
			}else{
		...
		$('#x_monto_real').attr('value', valor);
		$('#x_monto').attr('value', valor);
		$("#precio_mercancia").load("php_membresia_lista_precio.php?x_tipo_membresia="+tipo_membresia);
		
    });	
	
	$('#x_monto').change(function(e) {
        alert("PARA CAMBIAR EL MONTO SELECCIONE EL TIPO DE MEMBRESIA");
		var valor_anterior = $('#x_monto_real').val();
...
<input type="hidden" name="x_fecha_registro_r" id="x_fecha_registro_r" value="<?php echo FormatDateTime(@$x_today,5); ?>">
    ...
    <tr>
        ...
        <span>
<?php			
				$x_estado_civil_idList = "<select name=\"x_tipo\" id=\"x_tipo\"  class=\"texto_normal\">";
				$x_estado_civil_idList .= "<option value=''>Seleccione</option>";				
				$sSqlWrk = "SELECT `membresia_tipo_id`, `descripcion`, costo FROM `membresia_tipo` ";				
				$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
		if ($rswrk) {
			$rowcntwrk = 0;
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_estado_civil_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["membresia_tipo_id"] == @$x_tipo) {
...
?>
	...
	<tr>
		...
		<td class="ewTableAltRow"><span>
        <input type="hidden" name="x_monto_real" id="x_monto_real" value="<?php echo htmlspecialchars(@$x_monto) ?>" />
...
function AddData($conn)
{
	...
	// Field monto
	$theValue = ($GLOBALS["x_tipo"] != "") ? " '" . doubleval($GLOBALS["x_tipo"]) . "'" : "NULL";
	$fieldList["`tipo_id`"] = $theValue;
	// Field monto
