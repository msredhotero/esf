...
$x_generar = $_POST["x_generar"];
if(empty($x_generar)){
	$x_generar = $_GET["x_generar"];
}
...
<p><span class="phpmaker">COSECHAS DE CARTERA TOTAL
<?php if ($sExport == "" && !empty($x_year) && !empty($x_mes)) { ?>
&nbsp;&nbsp;<a href="php_rpt_cosechas_ct.php?export=excel&x_mes=<?php echo $x_mes; ?>&x_year=<?php echo $x_year; ?>&x_empresa_id=<?php echo $x_empresa_id; ?>&x_fondeo_credito_id=<?php echo $x_fondeo_credito_id; ?>&x_sucursal_srch=<?php echo $x_sucursal_srch; ?>&x_promo_srch=<?php echo $x_promo_srch; ?>&x_generar=<?php echo $x_generar; ?>">Exportar a Excel</a><?php } ?>
</span></p>

<?php if ($sExport == "") { ?>
<p align="left"><span class="phpmaker">
<form action="php_rpt_cosechas_ct.php" name="filtro" id="filtro" method="post">
...
?>
	...
	<tr>
	  <td colspan="5">Fecha de Cosecha:</td>
	  </tr>
	<tr>
...
<br />
<?php } ?>
<?php if($x_generar == 1 && !empty($x_year) && !empty($x_mes)) { ?>
<br /><br /><br /><br /><br /><br />
<p align="left"><span class="phpmaker">
<div align="left">
<table id="ewlistmain" class="ewTable" align="left">
  ...
  Mes/Año</span></td>
		...
		<td width="175" valign="top" nowrap="nowrap">Total Otorgado</td>
		<td width="175" valign="top" nowrap="nowrap">Total Pagado</td>
...
//filtros
if((!empty($x_empresa_id) && $x_empresa_id > 0) && (!empty($x_fondeo_credito_id) && $x_fondeo_credito_id > 0)){
	$x_filtros = " AND cosecha_cartera_total.fondeo_credito_id = $x_fondeo_credito_id ";
}
if(!empty($x_sucursal_srch) && $x_sucursal_srch > 0){
	$x_filtros .= " AND cosecha_cartera_total.sucursal_id = $x_sucursal_srch ";
}

if(!empty($x_credito_tipo_id) && $x_credito_tipo_id > 0){
	$x_filtros .= " AND cosecha_cartera_total.credito_tipo_id = $x_credito_tipo_id ";
}
...
$sSql = "select cosecha_cartera_total.* from cosecha_cartera_total where cosecha_cartera_total.year = $x_year and cosecha_cartera_total.mes = $x_mes ".$x_filtros;
$sSql .= " order by cosecha_cartera_total.fecha_registro, cosecha_cartera_total.year, cosecha_cartera_total.mes, cosecha_cartera_total.sucursal_id, cosecha_cartera_total.fondeo_credito_id, cosecha_cartera_total.credito_tipo_id, cosecha_cartera_total.promotor_id ";
...
while($row = @phpmkr_fetch_array($rs)){
	...
	$x_total_otorgado = $row["otorgado"];
	$x_total_pagado = $row["pagado"];
	$x_cartera_total = $row["cartera_total"];		
...
?>
	...
	<?php echo $x_mes."/".$x_year; ?>
			...
			@phpmkr_free_result($rswrk);
            ?></td>
			<td align="right" nowrap="nowrap"><?php echo FormatNumber($x_total_otorgado,2,0,0,1); ?></td>
			<td align="right" nowrap="nowrap"><?php echo FormatNumber($x_total_pagado,2,0,0,1); ?></td>
			<td align="right" nowrap="nowrap"><?php echo FormatNumber($x_cartera_total,2,0,0,1); ?></td>
			<!-- importe -->
			<!-- importe -->
			<!-- importe -->
		  </tr>
