...
$sSql = "SELECT  pld_operacion_inusual.*  from  pld_operacion_inusual  ";
...
&nbsp;&nbsp;<a href="php_operaciones_inusualeslist.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="php_operaciones_inusualeslist.php?export=word">Export to Word</a>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
<p>
...
<form action="php_operaciones_inusualeslist.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_operaciones_inusualeslist.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_operaciones_inusualeslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_operaciones_inusualeslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_operaciones_inusualeslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_operaciones_inusualeslist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
			  <a href="php_operaciones_inusualeslist.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
<?php } ?>
  ...
  <?php if ($sExport <> "") { ?>
		  ID
  <?php }else{ ?>
		  <a href="php_operaciones_inusualeslist.php?order=<?php echo urlencode("convenio_liquidacion_id"); ?>" style="color: #FFFFFF;" onMouseDown="ewsort(event, this.href);">ID<?php if (@$_SESSION["convenio_liquidacion_x_convenio_liquidacion_id_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif (@$_SESSION["convenio_liquidacion_x_convenio_liquidacion_id_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></a>
  <?php } ?>
		  </span></td>
        	<td valign="top"><span>
  <?php if ($sExport <> "") { ?>
