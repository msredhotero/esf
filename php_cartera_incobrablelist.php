...
if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
	$sSql = "select vencimiento.credito_id from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id where (credito.credito_status_id = 5) AND (vencimiento.vencimiento_status_id = 3) AND (solicitud.promotor_id = ".$_SESSION["php_project_esf_status_PromotorID"]. ") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].") $sWhere group by vencimiento.credito_id order by credito.credito_num+0";
}else{
	if($_SESSION["php_project_esf_status_UserRolID"] == 5) {
		$sSql = "select vencimiento.credito_id from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id where (credito.credito_status_id = 5) AND (vencimiento.vencimiento_status_id = 3) AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].")".$sWhere;
	}else{
		$sSql = "select vencimiento.credito_id from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id where (credito.credito_status_id = 5) AND (vencimiento.vencimiento_status_id = 3) AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].")".$sWhere;
	}
...
CARTERA CREDITOS INCOBRABLES (de <?php echo $_SESSION["x_dias_ini"]; ?> a <?php echo $_SESSION["x_dias_fin"]; ?> d&iacute;as de vencimiento)
<br /><br />
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_cartera_incobrablelist.php?export=excel&x_dias_ini=<?php echo $x_dias_ini; ?>&x_dias_fin=<?php echo $x_dias_fin; ?>">Exportar a Excel</a>
<?php } ?>
</span></p>

<?php if ($sExport == "") { ?>
<form action="php_cartera_incobrablelist.php" name="filtro" id="filtro" method="post">
  <table width="785" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
      ...
      <td>&nbsp;</td>
      <td><span class="phpmaker"><a href="php_cartera_incobrablelist.php?cmd=reset">Mostrar Todos</a>&nbsp;&nbsp; </span></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
...
<table id="ewlistmain" class="ewTable">
	...
	<tr class="ewTableHeader">
		<td width="68" valign="top"><span>
Ver Solicitud
		</span></td>
		<td width="60" valign="top"><span>
Ver Crédito
		</span></td>
		<td width="86" valign="top"><span>        
Fecha incobrable
</span></td>
<td width="52" valign="top"><span>        
Credito Num.
</span></td>		
		<td width="66" valign="top"><span>
Promotor
		</span></td>
		<td width="50" valign="top"><span>
Cliente
		</span></td>
		<td width="83" valign="top"><span>
...
Total de Capital
		...
		<td width="78" valign="top"><span>
Total de Intereses
		</span></td>
		<td width="83" valign="top"><span>
Total de Moratorios
		</span></td>
		<td width="63" valign="top"><span>
Total Vencido
		</span></td>
...
while ($row = @phpmkr_fetch_array($rs)){
	...
	$sSqlWrk = "select credito.credito_num,credito.fecha_incobrable, credito.credito_tipo_id, solicitud.solicitud_id from credito join solicitud on solicitud.solicitud_id = credito.solicitud_id where credito.credito_id  = $x_credito_id";
	$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
	...
	$x_fecha_incobrable = $rowwrk["fecha_incobrable"];	
	@phpmkr_free_result($rswrk);
...
<a href="php_solicitud_print.php?solicitud_id=<?php echo $x_solicitud_id; ?>" target="_blank">Ver</a>
<?php }else{ ?>
