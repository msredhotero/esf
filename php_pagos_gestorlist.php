...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=reporte_pagos'.date("Y-m-d").'.xls');
}
if ($sExport == "word") {
...
// Build SQL
$sSql = "SELECT credito.credito_id, credito.solicitud_id, credito.credito_status_id, credito.credito_num, vencimiento.vencimiento_num, vencimiento.importe as impvenc, vencimiento.interes, vencimiento.iva, vencimiento.interes_moratorio, vencimiento.iva_mor, vencimiento.total_venc, recibo.*, solicitud.solicitud_id,  solicitud.promotor_id
...
if($_SESSION["php_project_esf_status_UserRolID"] == 13){
	// si es el rol juridoc solo se dbe mostrar los pagos de los credito que estan en juridico
	$sDbWhere_juridico = " AND credito.credito_status_id = 4 ";
	}
...
if($filter["x_gestor"]){	
// seleccionamos todos los credito que pertencen la gestor
$x_listado_credito = "";
$sqlgestor = "SELECT credito_id FROM gestor_credito where usuario_id =  ".$filter["x_gestor"]."" ;
$rsgestor = phpmkr_query($sqlgestor, $conn) or die ("error la seleccionar los pagos del getor ".phpmkr_error()."sql:".$sqlgestor);
while($rowgestor =  phpmkr_fetch_array($rsgestor)){
	$x_listado_credito = $x_listado_credito.$rowgestor["credito_id"].", ";
	}
$x_listado_credito = trim($x_listado_credito,", ");	
$sDbWhere .= "((credito.credito_id in ($x_listado_credito)) ) AND ";	
}
...
<form action="php_pagos_gestorlist.php" name="filtro" id="filtro" method="post">
...
?></td>
    ...
    <tr>
      ...
      <td>&nbsp;</td>
      <td><a href="php_pagos_gestorlist.php?cmd=reset"></a></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    ...
    <tr>
      ...
      <td><a href="php_pagos_gestorlist.php?cmd=reset">Mostrar Todos</a></td>
    </tr>
    <tr>
...
<?php if ($sExport == "") { ?>
<form action="php_pagos_gestorlist.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_pagos_gestorlist.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_pagos_gestorlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_pagos_gestorlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_pagos_gestorlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_pagos_gestorlist.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_pagos_gestorlist.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
Cr�dito num.
<!---
	<a href="php_pagos_gestorlist.php?order=<?php //echo urlencode("credito_id"); ?>">Cr�dito num.<?php // if (@$_SESSION["vencimiento_x_credito_id_Sort"] == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php //} elseif (@$_SESSION["vencimiento_x_credito_id_Sort"] == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php// } ?></a>
-->    
<?php } ?>
		</span></td>
