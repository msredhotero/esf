...
$x_zona_srch = $_POST["x_zona_srch"];
$x_crm_cartera_status_srch = $_POST["x_crm_cartera_status_srch"];
$x_telefono_status_srch = $_POST["x_telefono_status_srch"];
...
$x_posteo = $x_nombre_srch.$x_apepat_srch.$x_apemat_srch.$x_crenum_srch.$x_clinum_srch.$x_promo_srch.$x_sucursal_srch.$x_crm_cartera_status_srch.$x_zona_srch.$x_telefono_status_srch;
...
if(!empty($x_zona_srch)){
	$_SESSION["x_zona_srch"] = $x_zona_srch;
}else{
	if(strlen($x_posteo) > 0){
		$_SESSION["x_zona_srch"] = "";
	}
}
...
if(!empty($x_telefono_status_srch)){
	if($x_telefono_status_srch < 1000){
		$_SESSION["x_telefono_status_srch"] = $x_telefono_status_srch;
	}else{
		$_SESSION["x_telefono_status_srch"] = "";
	}
}else{
	if(strlen($x_posteo) > 0){
		$_SESSION["x_telefono_status_srch"] = "";
...
if(!empty($x_crm_cartera_status_srch)){
	if($x_crm_cartera_status_srch < 1000){
		$_SESSION["x_crm_cartera_status_srch"] = $x_crm_cartera_status_srch;
	}else{
		$_SESSION["x_crm_cartera_status_srch"] = "";
	}
}else{
	if(strlen($x_posteo) > 0){
		$_SESSION["x_crm_cartera_status_srch"] = "";
...
if(!empty($_SESSION["x_zona_srch"])){
	if(!empty($_SESSION["x_zona_srch"]) && ($_SESSION["x_zona_srch"] != "1000")){	
	
	$ssrch_zona = " ( solicitud.zona_id = ".$_SESSION["x_zona_srch"]." ) AND ";
	}
}
...
if(!empty($_SESSION["x_crenum_srch"])){
	...
	if((strlen($ssrch_cre) > 0 ) &&  (!empty($_SESSION["x_promo_srch"])) && (!empty($_SESSION["x_clinum_srch"])) && (!empty($x_sucursal_srch))  ){
...
if(!empty($_SESSION["x_sucursal_srch"])){
	if(!empty($_SESSION["x_sucursal_srch"]) && ($_SESSION["x_sucursal_srch"] != "1000")){
		...
		if($_SESSION["php_project_esf_status_UserRolID"] == 12 || (!empty($_SESSION["x_crenum_srch"])) || (!empty($_SESSION["x_crm_cartera_status_srch"]) && ($_SESSION["x_crm_cartera_status_srch"] != "1000")) || (!empty($_SESSION["x_telefono_status_srch"]) && ($_SESSION["x_telfono_status_srch"] != "1000"))|| (!empty($_SESSION["x_crenum_srch"])) || (!empty($_SESSION["x_crenum_srch"])) ){
			echo "2";
			}else{
...
if(!empty($_SESSION["x_crm_cartera_status_srch"])){
	if(!empty($_SESSION["x_crm_cartera_status_srch"]) && ($_SESSION["x_crm_cartera_status_srch"] != "1000")){
		$x_join_status_crm = " join crm_credito_status on  crm_credito_status.credito_id = credito.credito_id ";
		$ssrch_cre .= "(crm_credito_status.crm_cartera_status_id = ".$_SESSION["x_crm_cartera_status_srch"].") AND ";
		$x_staus = 1;
...
if(!empty($_SESSION["x_telefono_status_srch"])){
	if(!empty($_SESSION["x_telefono_status_srch"]) && ($_SESSION["x_telefono_status_srch"] != "1000")){
		$ssrch_cre .= "(telefono_contacto_status.telefono_status_id = ".$_SESSION["x_telefono_status_srch"].") AND ";
$x_join_status_contacto = " join telefono_contacto_status on telefono_contacto_status.credito_id = credito.credito_id ";
		$x_staus = 1;
		if($_SESSION["php_project_esf_status_UserRolID"] == 12){
			//echo "2";
			}else{
...
if(!empty($_SESSION["x_empresa_id"])){
	if(!empty($_SESSION["x_empresa_id"]) && ($_SESSION["x_empresa_id"] != "999999999")){
		...
		if(!empty($_SESSION["x_fondeo_credito_id"])){
			if($x_staus == 1){
				$ssrch_cre .= " AND (fondeo_credito.fondeo_credito_id = ".$_SESSION["x_fondeo_credito_id"].") AND ";
				}else{
...
$sWhere = $ssrch_cli.$ssrch_zona.$ssrch_cre.$sCreW;
...
if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
	$sSql = "select vencimiento.credito_id, credito.solicitud_id, solicitud.formato_nuevo as formato from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id join promotor on promotor.promotor_id = solicitud.promotor_id  $x_join_status_crm  $x_join_status_contacto where (credito.credito_status_id in (1,3)) AND (vencimiento.vencimiento_status_id = 3) AND (solicitud.promotor_id = ".$_SESSION["php_project_esf_status_PromotorID"]. ") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].$sWhere.") group by vencimiento.credito_id order by credito.credito_num+0";
}else{
	if($_SESSION["php_project_esf_status_UserRolID"] == 5) {
		$sSql = "select vencimiento.credito_id,solicitud.formato_nuevo as formato from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id join promotor on promotor.promotor_id = solicitud.promotor_id $x_join_status_crm $x_join_status_contacto where (credito.credito_status_id = 4) AND (vencimiento.vencimiento_status_id = 3) AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].$sWhere.")";
	}else{
		$sSql = "select vencimiento.credito_id,solicitud.formato_nuevo as formato, solicitud.promotor_id from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id join promotor on promotor.promotor_id = solicitud.promotor_id $x_join_status_crm  $x_join_status_contacto where (credito.credito_status_id in (1,3,4)) AND (vencimiento.vencimiento_status_id = 3) AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].$sWhere.")";
...
<link rel="stylesheet" href="colorbox-master/colorbox-master/colorbox.css"  />
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js"></script>
<script src="colorbox-master/colorbox-master/jquery.colorbox.js"></script>
...
<div id="txtlineas" style=" float: left;">
  ...
  <tr>
    <td class="phpmaker">Zona</td>
    <td>&nbsp;</td>
    <td><?php
		$x_estado_civil_idList = "<select name=\"x_zona_srch\" class=\"texto_normal\">";				
		...
		if ($rswrk) {
			...
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_estado_civil_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["zona_id"] == $_SESSION["x_zona_srch"]) {
					$x_estado_civil_idList .= "' selected";
				}
  ...
  <tr>
   ...
   <tr>
    <td class="phpmaker">Status de cobranza</td>
    <td>&nbsp;</td>
    <td><span class="phpmaker">
      <?php
		$x_estado_civil_idList = "<select name=\"x_crm_cartera_status_srch\" class=\"texto_normal\">";
		
			$sSqlWrk = "SELECT `crm_cartera_status_id`, `descripcion` FROM `crm_cartera_status`";	
			$x_estado_civil_idList .= "<option value=\"1000\">TODOS</option>";	
		...
		if ($rswrk) {
			...
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_estado_civil_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["crm_cartera_status_id"] == $_SESSION["x_crm_cartera_status_srch"]) {
					$x_estado_civil_idList .= "' selected";
				}
				$x_estado_civil_idList .= ">" . $datawrk["descripcion"] . "</option>";
				$rowcntwrk++;
  ...
  <tr>
    <td class="phpmaker">Status Contacto</td>
    <td>&nbsp;</td>
    <td><span class="phpmaker">
      <?php
		$x_estado_civil_idList = "<select name=\"x_telefono_status_srch\" class=\"texto_normal\">";
		
			$sSqlWrk = "SELECT `telefono_status_id`, `descripcion` FROM `telefono_status`";	
			$x_estado_civil_idList .= "<option value=\"1000\">TODOS</option>";	
		...
		if ($rswrk) {
			...
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_estado_civil_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["telefono_status_id"] == $_SESSION["x_telefono_status_srch"]) {
					$x_estado_civil_idList .= "' selected";
				}
				$x_estado_civil_idList .= ">" . $datawrk["descripcion"] . "</option>";
				$rowcntwrk++;
...
while ($row = @phpmkr_fetch_array($rs)){
	...
	$sqlDias = "SELECT fecha  FROM crm_credito_status WHERE credito_id = $x_credito_id";
	$rsDias = phpmkr_query($sqlDias,$conn) or die ("erro en dias".phpmkr_error()."sql;".$sqlDias);
	$rowDias = phpmkr_fetch_array($rsDias);
	$x_fecha_status = $rowDias["fecha"];
	$x_fecha_compara = date("Y-m-d");	
	 $dias_diferencia = 0;
	$diasDif = " SELECT DATEDIFF('$x_fecha_compara','$x_fecha_status')  as  dias_dif ";
	$rsdif = phpmkr_query($diasDif,$conn) or die ("Erro".phpmkr_error()."sql:".$diasDif);
	$rowDisf = phpmkr_fetch_array($rsdif);
	$dias_diferencia = $rowDisf["dias_dif"];
	 
	 $sqlStatusContacto = "SELECT telefono_status.descripcion as status_telefono, telefono_contacto_status.usuario_id as usuario, usuario.nombre as nombre_user, telefono_contacto_status.fecha  FROM telefono_contacto_status join telefono_status on telefono_status.telefono_status_id =  telefono_contacto_status.telefono_status_id  join usuario on usuario.usuario_id = telefono_contacto_status.usuario_id where credito_id = $x_credito_id ";
	 $rsStatuscontecto = phpmkr_query($sqlStatusContacto,$conn) or die ("Error al seleccionar le estatus del contacto".phpmkr_error()."sql;".$sqlStatusContacto);
	 $rowStatusContacto = phpmkr_fetch_array($rsStatuscontecto);
	
	$x_estatus_contacto = $rowStatusContacto["status_telefono"];
	$x_usuario_contacto =  $rowStatusContacto["usuario"];
	$x_fecha_contacto = $rowStatusContacto["fecha"];
	$x_user_contacto = $rowStatusContacto["nombre_user"];
	$x_modifica_contacto = $x_user_contacto." ".$x_fecha_contacto;
	
	$sqlStatus = "SELECT crm_cartera_status.descripcion as descripcion
    FROM  crm_credito_status   join crm_cartera_status
    on crm_cartera_status.crm_cartera_status_id =  crm_credito_status.crm_cartera_status_id WHERE credito_id = $x_credito_id ";
	$rsStatus = phpmkr_query($sqlStatus,$conn) or die ("Error al seleccionar el ststaus".phpmkr_error()."sql:".$sqlStatus);
	$rowStatus = phpmkr_fetch_array($rsStatus);
	$x_status_tarea = $rowStatus["descripcion"];
...
order by vencimiento_num+0	
	...
	// seleccionamos el caso id = 
	$sqlCaso = "SELECT * FROM crm_caso WHERE crm_caso_tipo_id = 3 and credito_id = $x_credito_id  order by crm_caso_id DESC";//and  ";
	$rsCaso = phpmkr_query($sqlCaso, $conn) or die ("Error al seleccionar el caso del credito ".phpmkr_error()."sql :".$sqlCaso);
	$rowCaso = phpmkr_fetch_array($rsCaso);
	$x_crm_caso_id  = $rowCaso["crm_caso_id"];
	//echo "crm_caso_id = ".$x_crm_caso_id."<br>";
...
<td><span><a href="php_telefono_contacto_statusedit.php?x_credito_id=<?php echo $x_credito_id; ?>" ><?php echo $x_estatus_contacto; ?></a></span><?php echo "\n ".$x_modifica_contacto;?></td>
<td><a href="modulos/php_agrega_telefono.php?key2=<?php echo $x_solicitud_id; ?>&credito_id=<?php echo $x_credito_id;?>" >Agrega tel&eacute;fono</a></td>
<td><span><?php echo $dias_diferencia ?></span></td>
<td><span><?php echo $x_status_tarea; ?></span></td>
<td><span><a class='iframe' href="modulos/casos_view.php?key=<?php echo $x_crm_caso_id;?>" target="_blank"> Capturar PP </a></span></td>
<td><span><a href="php_imprime_carta_menu.php?credito_id=<?php echo $x_credito_id;?>" target="_blank">Cartas Mora </a></span></td>
<td><span><a href="php_imprime_estado_cuenta.php?credito_id=<?php echo $x_credito_id;?>" target="_blank"><center> Edo Cuenta</center> </a></span></td>
<td><span><a href="php_demanda_print.php?x_solicitud_id=<?php echo $x_solicitud_id;?>" target="_blank"><center> Demanda</center> </a></span></td>
	</tr>
<?php
