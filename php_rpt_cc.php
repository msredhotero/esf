...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=reporte_cc.xls');
}
?>
<?php include ("db.php") ?>
...
<p><span class="phpmaker">REPORTE DE CUENTAS POR COBRAR
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_rpt_cc.php?export=excel&x_fecha_desde=<?php echo FormatDateTime(@$x_fecha_desde,7); ?>&x_fecha_hasta=<?php echo FormatDateTime(@$x_fecha_hasta,7); ?>">Exportar a Excel</a><?php } ?>
</span></p>

<?php if ($sExport == "bloqueado") { ?>
<p><span class="phpmaker">
<form action="php_rpt_cc.php" name="filtro" id="filtro" method="post">
...
echo "Mientras fecha_desde sea menor o igual a fecha hasta<br><br>";
	...
	$sSql = "SELECT sum(vencimiento.importe) as importe, sum(interes) as interes, sum(interes_moratorio) as interes_moratorio, sum(total_venc) as total_venc from vencimiento where (vencimiento.vencimiento_status_id in(1,3))";
//	 AND (vencimiento.fecha_vencimiento <= '".$x_fecha_actual."')";

	echo $sSql."<br><br>";
	echo "DETALLE : sumamos importe, interes, interes_moratorio, total_vencimiento de todos loS vencimientos que esten PENDIENTES o VENCIDOS <br><br>";
...
*/
	...
	echo "<BR>===== LA SUMATORIA ESTA TOMANDO EN CUENTA LOS CREDITOS QUE ESTAN CANCELADOS ====<BR><BR> SI QUITAMOS LOS CREDITOS CANCELADOS LOS RESULADOS SON LOS SIGUIENTES: <BR><BR>importe = 5348940.862<BR>
interes = 1662569.024<BR>
interes_moratorio = 1640977.655<BR>
total_venc = 8827681.557 <BR>";
