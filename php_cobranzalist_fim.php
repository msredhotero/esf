...
if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
	...
	$sSql = "SELECT vencimiento.*, credito.credito_num, credito.credito_tipo_id, solicitud.solicitud_id FROM vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id  JOIN fondeo_colocacion ON fondeo_colocacion.credito_id = credito.credito_id ";
}
...
if($_SESSION["php_project_esf_status_UserRolID"] == 7) {
	...
	$sDbWhere = " (fondeo_colocacion.fondeo_credito_id = 7) AND (vencimiento.vencimiento_status_id = 1) AND (credito.credito_status_id = 1) AND";
}
...
&nbsp;&nbsp;<a href="php_cobranzalist_fim.php?export=excel&x_fecha_desde=<?php echo $x_fecha_desde; ?>&x_fecha_hasta=<?php echo $x_fecha_hasta; ?>">Exportar a Excel</a>
<?php }else{ ?>
&nbsp;&nbsp;<a href="php_cobranzalist_fim.php?export=excel">Exportar a Excel</a>
<?php } ?>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
...
<form action="php_cobranzalist_fim.php" name="filtro" id="filtro" method="post">
<table class="phpmaker">
	...
	<tr>
	  ...
	  <td><a href="php_cobranzalist_fim.php?cmd=reset">Mostrar Todos</a></td>
	  </tr>
</table>
...
<?php if ($sExport == "") { ?>
<form action="php_cobranzalist_fim.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_cobranzalist_fim.php?start=<?php echo $PrevStart; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_cobranzalist_fim.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_cobranzalist_fim.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_cobranzalist_fim.php?start=<?php echo $x; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_cobranzalist_fim.php?start=<?php echo $x; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_cobranzalist_fim.php?start=<?php echo $NextStart; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
