...
$sSql = "select sum(vencimiento.importe) as capital, sum(interes) as interes, sum(interes_moratorio) as moratorios, sum(vencimiento.iva) as iva, sum(iva_mor) as ivmor from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id where   credito.credito_staus_id = 1 and year(vencimiento.fecha_vencimiento) = $x_year and month(vencimiento.fecha_vencimiento) = $x_mes".$x_filtros;
...
$sSql = "select sum(vencimiento.importe) as capital, sum(vencimiento.interes) as interes, sum(vencimiento.interes_moratorio) as moratorios, sum(vencimiento.iva) as iva, sum(vencimiento.iva_mor) as ivmor from vencimiento, credito , recibo_vencimiento, recibo  where credito.credito_id = vencimiento.credito_id and credito.credito_status_id = 4  and vencimiento.vencimiento_status_id = 2 and vencimiento.vencimiento_id = recibo_vencimiento.vencimiento_id and  recibo.recibo_id = recibo_vencimiento.recibo_id and  year(vencimiento.fecha_vencimiento) = $x_year and month(vencimiento.fecha_vencimiento) = 1 and  year(recibo.fecha_pago) = $x_year and month(recibo.fecha_pago) = 1 ".$x_filtros;
}
...
$sSql = "select credito.credito_id, (vencimiento.importe) as capital, (vencimiento.interes) as interes, (vencimiento.interes_moratorio) as moratorios, (vencimiento.iva) as iva, (vencimiento.iva_mor) as ivmor , recibo.fecha_pago  as pagado from vencimiento, credito , recibo_vencimiento, recibo  where credito.credito_id = vencimiento.credito_id and credito.credito_status_id = 4  and vencimiento.vencimiento_status_id = 2 and vencimiento.vencimiento_id = recibo_vencimiento.vencimiento_id and  recibo.recibo_id = recibo_vencimiento.recibo_id and  year(vencimiento.fecha_vencimiento) = $x_year and month(vencimiento.fecha_vencimiento) = 1 and  year(recibo.fecha_pago) = $x_year and month(recibo.fecha_pago) = 1 ".$x_filtros;
$rsSep = phpmkr_query($sSql,$conn) or die ("Error al seleccionar los campos separados");
while($rowSEP = phpmkr_fetch_array($rsSep)){
	echo "<br> credito_id ".$rowSEP["credito_id"] ." importe ".$rowSEP["capital"]." interes".$rowSEP["interes"]. "pagado ".$rowSEP["pagado"];
