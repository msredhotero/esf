...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=listado_convenio_orden_semanal.xls');
}
if ($sExport == "word") {
	header('Content-Type: application/vnd.ms-word');
	header('Content-Disposition: attachment; filename=listado_convenio_orden_semanal.doc');
}
?>
<?php include ("db.php") ?>
...
$sSql .= "  convenio_orden.fecha_registro >= \"$x_fecha_inicio\" and convenio_orden.fecha_registro <= \"$x_fecha_fin\"  and  (convenio_orden.usuario_id IN ($x_promotores) ) order by promotor.sucursal_id,convenio_orden.fecha_registro  asc";
		 
	 }else{


$sSql = "SELECT  convenio_orden.*, promotor.nombre_completo as promotor,sucursal.nombre as suc from  convenio_orden join promotor on promotor.promotor_id = convenio_orden.usuario_id  join sucursal on promotor.sucursal_id = sucursal.sucursal_id  where ";
$sSql .= "  convenio_orden.fecha_registro >= \"$x_fecha_inicio\" and convenio_orden.fecha_registro <= \"$x_fecha_fin\" order by promotor.sucursal_id,convenio_orden.fecha_registro  asc";
	 }
echo $sSql;
// Load Default Filter
$sDefaultFilter = "";
...
?>
<p><span class="phpmaker">Listado de orden de convenio semanal
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_listado_convenio_orden_semanal.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
