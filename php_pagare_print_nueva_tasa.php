...
//CREDITO
$sSql = "SELECT credito.tarjeta_num, credito.iva, credito.num_pagos, fecha_vencimiento, referencia_pago, tasa, tasa_moratoria, fecha_otrogamiento, credito.credito_id FROM credito where solicitud_id = $x_solicitud_id";
$rs = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
$row = phpmkr_fetch_array($rs);
$x_credito_id = $row["credito_id"];
$x_cheque_valor = $row["referencia"];
$x_tasa_valor = $row["tasa"];
$GLOBALS["x_valor_tasa_nueva"] = $row["tasa"];
$GLOBALS["x_numero_de_pagos"] = $row["num_pagos"];
$GLOBALS["x_iva_c"] = $row["iva"];
...
////////////////////////////////////////////////////////////////calculo del credito con tasa nueva
	...
	$x_num_pagos = $GLOBALS["x_numero_de_pagos"];	
	//echo "umero de pagos".	$GLOBALS[x_numero_de_pagos];
	$x_importe_c = 	$GLOBALS["x_importe_solicitado"];
	$x_tasa_n = $GLOBALS[x_valor_tasa_nueva];
	//echo "tasa nueva =".$x_tasa_n;
	$x_tasa_50 = (($x_tasa_n * 50)/100);
	//echo "tasa 50 -" . $x_tasa_50;
	$x_importe_c = str_replace(",","",$x_importe_c);
	$x_tasa_nueva = $x_tasa_50;
	...
	while($x_num_ven_tasa_new <= $GLOBALS["x_total_numero_vencimeintos"]){
		...
		if($GLOBALS["x_iva_c"] == 1){
							
							$x_iva_venc = round($interes * .16);	
