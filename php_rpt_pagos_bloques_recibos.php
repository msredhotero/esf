...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=reporte_pagos_bloques_recibos'.date("Y-m-d").'.xls');
}
if ($sExport == "word") {
...
$x_listado_creditos_id ='';

$rs_t = phpmkr_query($sSql,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sSql);
$nTotalRecs_t = phpmkr_num_rows($rs_t);

while ($row_t = @phpmkr_fetch_array($rs_t))  {
		$x_credito_num_t = $row_t["credito_num"];
		$x_credito_id_t = $row_t["credito_id"];
		$x_listado_creditos_id .= $x_credito_id_t.", ";
}
		
	$x_listado_creditos_id = trim($x_listado_creditos_id,", ");
if($x_listado_creditos_id==''){
	$x_listado_creditos_id = 0;
	}


$sql2 = "SELECT credito.credito_id, credito.solicitud_id, credito.credito_status_id, credito.credito_num, vencimiento.vencimiento_num, vencimiento.importe as impvenc, vencimiento.interes, vencimiento.iva, vencimiento.interes_moratorio, vencimiento.iva_mor, vencimiento.total_venc, recibo.*, solicitud.solicitud_id,  solicitud.promotor_id, solicitud_cliente.cliente_id, cliente.cliente_id
FROM vencimiento join credito 
on credito.credito_id = vencimiento.credito_id join recibo_vencimiento
on recibo_vencimiento.vencimiento_id = vencimiento.vencimiento_id join recibo
on recibo.recibo_id = recibo_vencimiento.recibo_id join solicitud on solicitud.solicitud_id = credito.solicitud_id  join promotor on promotor.promotor_id = solicitud.promotor_id join solicitud_cliente ON solicitud_cliente.solicitud_id = solicitud.solicitud_id join cliente on cliente.cliente_id = solicitud_cliente.cliente_id 	AND  ";
$sql2 .= $sWhere . " AND  credito.credito_id IN ($x_listado_creditos_id)" ;
...
// Set up Record Set
$rs = phpmkr_query($sql2,$conn) or die("Failed to execute query: " . phpmkr_error() . '<br>SQL: ' . $sql2);
...
?>
<p><span class="phpmaker">REPORTE DE PAGOS POR MONTOS  
<?php if ($sExport == "") { ?>
  ...
  <td><table width="895" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td width="119"><?php
		...
		$x_estado_civil_idList .= "<option value='500000' selected>MXN $500,000 MXN </option>";
		$x_estado_civil_idList .= "<option value='300000' selected>MXN $300,000</option>";
		$x_estado_civil_idList .= "<option value='200000' selected>MXN $200,000</option>";
		$x_estado_civil_idList .= "<option value='100000' selected>MXN $100,000</option>";
		$x_estado_civil_idList .= "<option value='50000' selected>MXN $50,000</option>";
		$x_estado_civil_idList .= "<option value='20000' selected>MXN $20,000</option>";
		$x_estado_civil_idList .= "<option value='10000' selected>MXN $10,000</option>";
...
?></td>
    ...
    <tr>
      ...
      <td>&nbsp;</td>
      <td><a href="php_rpt_pagos_bloques_recibos.php?cmd=reset"></a></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    ...
    <tr>
      ...
      <td><a href="php_rpt_pagos_bloques_recibos.php?cmd=reset">Mostrar Todos</a></td>
    </tr>
    <tr>
...
<?php if ($sExport == "") { ?>
<form action="php_rpt_pagos_bloques_recibos.php" name="ewpagerform" id="ewpagerform">
...
if ($nTotalRecs > 0) {
	...
	if ($nTotalRecs > $nDisplayRecs) {
		...
		<a href="php_rpt_pagos_bloques_recibos.php?start=<?php echo $PrevStart; ?>&<?php echo $_QS; ?>"><b>Anterior</b></a>
		<?php
		}
		if ($isPrev || (!$rsEof)) {
			$x = 1;
		...
		<b><?php echo $y; ?></b>
					<?php } else { ?>
		<a href="php_rpt_pagos_bloques_recibos.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
					<?php }
					$x += $nDisplayRecs;
					$y += 1;
				} elseif (($x >= ($dx1-$nDisplayRecs*$nRecRange)) && ($x <= ($dx2+$nDisplayRecs*$nRecRange))) {
					if ($x+$nRecRange*$nDisplayRecs < $nTotalRecs) { ?>
		<a href="php_rpt_pagos_bloques_recibos.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo ($y+$nRecRange-1);?></b></a>
					<?php } else {
						$ny=intval(($nTotalRecs-1)/$nDisplayRecs)+1;
							if ($ny == $y) { ?>
		<a href="php_rpt_pagos_bloques_recibos.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?></b></a>
							<?php } else { ?>
		<a href="php_rpt_pagos_bloques_recibos.php?start=<?php echo $x; ?>&<?php echo $_QS; ?>"><b><?php echo $y; ?>-<?php echo $ny; ?></b></a>
							<?php }
					}
		...
		if (!$rsEof) {
			...
			$isMore = True;  ?>
		<a href="php_rpt_pagos_bloques_recibos.php?start=<?php echo $NextStart; ?>&<?php echo $_QS; ?>"><b>Siguiente</b></a>
		<?php } else {
			$isMore = False;
		} ?>
		<br>
...
$x_tot_moratorios = 0;
$x_tot_total_pago = 0;
$x_credito_id_temporal = '';
$x_importe_temp = 0;
$x_interes_temp = 0;
$x_iva_temp = 0;	
$x_interes_moratorio_temp = 0 ;
$x_iva_moratorio_temp = 0;		
$x_total_venc_temp = 0;
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount >= $nStartRec) {
		...
		$x_credito_id_temporal =  (empty($x_credito_id_temporal))?$x_credito_id:$x_credito_id_temporal;
...
$x_cliente = $rowFondeoa["nombre_completo"]." ".$rowFondeoa["apellido_paterno"]." ".$rowFondeoa["apellido_materno"];
		...
		if($x_credito_id_temporal == $x_credito_id){
			$x_importe_temp = $x_importe_temp  + $x_importe;
			$x_interes_temp = $x_interes_temp + $x_interes;
			$x_iva_temp = $x_iva_temp + $x_iva;	
			$x_interes_moratorio_temp = $x_interes_moratorio_temp + $x_interes_moratorio;
			$x_iva_moratorio_temp = $x_iva_moratorio_temp + $x_iva_moratorio;		
			$x_total_venc_temp = $x_total_venc_temp + $x_total_venc;		
			}else{
				...
				echo "<tr style='background-color:#bdbdbd'>
					...
					<td style='text-align:right'><b>". number_format($x_importe_temp,"2",".",",")."</b> </td>
					<td style='text-align:right'><b>". number_format( $x_interes_temp,"2",".",",")."</b></td>
					<td style='text-align:right'><b>". number_format( $x_iva_tem,"2",".",",")." </b></td>	
					<td style='text-align:right'><b>". number_format($x_interes_moratorio_temp,"2",".",",")."</b></td>
					<td style='text-align:right'><b>". number_format($x_total_venc_temp,"2",".",",")." </b></td></tr>";				
					
					$x_importe_temp = $x_importe;
					$x_interes_temp = $x_interes;
					$x_iva_temp = $x_iva;	
					$x_interes_moratorio_temp = $x_interes_moratorio;
					$x_iva_moratorio_temp =  $x_iva_moratorio;		
					$x_total_venc_temp =  $x_total_venc;	
					$x_credito_id_temporal = $x_credito_id;
...
<?php
	...
	if($nRecCount == ($nTotalRecs)){
					...
					<td style='text-align:right'><b><b>". number_format($x_importe_temp,"2",".",",")."</b> </td>
					<td style='text-align:right'><b><b>". number_format( $x_interes_temp,"2",".",",")."</b></td>
					<td style='text-align:right'><b><b>". number_format( $x_iva_tem,"2",".",",")." </b></td>	
					<td style='text-align:right'><b><b>". number_format($x_interes_moratorio_temp,"2",".",",")."</b></td>
					<td style='text-align:right'><b><b>". number_format($x_total_venc_temp,"2",".",",")." </b></td>				
