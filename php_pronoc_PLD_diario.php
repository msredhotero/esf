...
$SqlTipoCambio = " SELECT * FROM tipo_cambio  ORDER BY tipo_cambio_id DESC LIMIT 0, 1";
...
#A LA FECHA DE HOY LE QUITAMOS 16 DIAS QUE SON LOS QUE SE PUEDE TARDAR EN APLICAR EL PAGO
############################################
...
# A la fecha actual menos los 16 dias le quitamos un mes para hacer la suma de los pagos
$SqlFI = " SELECT DATE_SUB('".$fecha_fin_mes."', INTERVAL 1 MONTH) as fecha_INICIO_mes ";
$rsFI = phpmkr_query($SqlFI,$conn) or die ("Error al seleccionar la fecha inicio".phpmkr_error()."sql.".$SqlF);
$rowFI = phpmkr_fetch_array($rsFI);
$fecha_inico_mes = $rowFI["fecha_INICIO_mes"]; 
		...
		#$row = phpmkr_fetch_array($rs);
			...
			if($x_pld==0){
				...
				$sSql = "INSERT INTO `reporte_cnbv` (`reporte_cnbv_id`, `cliente_id`, `solicitud_id`, `tipo_reporte_id`, `fecha_registro`, `razon_reporte`, `monto`)";
				 ...
				 $sSql .= "VALUES (NULL,".$x_cliente_id.",".$x_solicitud_id.", '2', '".$x_today."', 'PAGO MAYOR A 7,500 USD EN UN MES CALENDARIO EN EL CREDITO ".$x_credito_num."',".$suma_pagos.");";
