		...
		if(strlen($x_ref_loc) == 0){
			...
			if(strpos($x_ref_loc,"/") == 0){
				...
				if(phpmkr_num_rows($rswrk) > 0){
					...
					// selecciomaos los pagos que corresponde a la garatia liquida
					
					$sqlGrantiaLiquida = "SELECT garantia_liquida, num_pagos,forma_pago_id FROM credito where credito_id = $x_credito_id ";
					$rsGarantiaLi = phpmkr_query($sqlGrantiaLiquida,$conn)or die("Error al seleccionar el numero de pagos".phpmkr_error()."sql; ".$sqlGrantiaLiquida);
					#echo $sqlGrantiaLiquida;
					$rowGarantia = phpmkr_fetch_array($rsGarantiaLi);
					$x_gar_liq = $rowGarantia["garantia_liquida"];
					$x_num_pagos = $rowGarantia["num_pagos"];
					$x_c_forma_pago = $rowGarantia["forma_pago_id"];
				#	echo "<br>gar liq ".$x_gar_liq."num pagos ".$x_num_pagos." forma pag".$x_c_forma_pago;
					if($x_gar_liq>0){
						//kuki
						if($x_c_forma_pago == 1){
							#echo "Semanal";
							if($x_num_pagos == 40){
								...
								$x_vencimientos_where = "AND vencimiento.vencimiento_num NOT IN (40,39,38) ";
								}else{		
									#$x_vencimientos_pendintes_por_cambio = 2;
									$x_vencimientos_where = "AND vencimiento.vencimiento_num NOT IN (20,19) ";
								}
						}else{
							#echo "forma pago_id".$x_c_forma_pago." ";
							if((($x_c_forma_pago == 2) || ($x_c_forma_pago == 4)) && ($x_num_pagos == 24)){
								#$x_vencimientos_pendintes_por_cambio = 2;					
								$x_vencimientos_where = "AND vencimiento.vencimiento_num NOT IN (24,23)";
								}else{
									#$x_vencimientos_pendintes_por_cambio = 1;
									$x_vencimientos_where = "AND vencimiento.vencimiento_num NOT IN (12)";
								}
				...
				#	echo "ven whre ".$x_vencimientos_where;
					
				$sSql = "select count(*) as salteado from vencimiento where credito_id = $x_credito_id and vencimiento_num < $x_vencimiento_num and vencimiento_status_id in (3) $x_vencimientos_where ";
				...
				// seleccionamos la fecha de cierre de periodo
				$sqlCierrePeriodo = "SELECT fecha FROM cierre_periodo order by cierre_periodo_id DESC LIMIT 0,1";
				$rsCierre =phpmkr_query($sqlCierrePeriodo,$conn) or die("Error en cierre");
				$rowCierre = phpmkr_fetch_array($rsCierre);
				$x_fecha_max = $rowCierre["fecha"];
...
var fecha_de_pago = EW_this.x_fecha_pago.value;
alert(fecha_de_pago);
var ma1 = fecha_de_pago.split("/");
var anio_pp =  ma1[2];  
var mes_pp = (ma1[1] -1);
var dia_pp = ma1[0];
//alert(ma1);
var fecha_pp  = new Date(anio_pp,mes_pp,dia_pp) ;
var fecha_max = new Date(EW_this.x_fecha_max.value);

alert ("fecha  de pago "+fecha_pp);
alert("fecha maxima"+fecha_max);
//alert("fecha minima"+fecha_min);


if( fecha_max  > fecha_pp){
	//alert("entra al primer if");
	alert("La fecha de pago NO puede ser menor  la fecha de cierre del periodo,consulta con administracion la fecha de cierre");
	validada = false;
}
...
<input type="hidden" name="x_fecha_max" id="x_fecha_max" value="<?php echo $x_fecha_max; ?>" />
...
//VALIDA PAGOS FALTANTES	
	...
	if($x_c_forma_pago == 1){
			...
			$SQLpp = "SELECT monto, status as garantia_status FROM garantia_liquida where credito_id = $x_credito_id ";
			...
			$x_status_garantia = $ROWpp["garantia_status"];
			
			if($x_status_garantia == 1){
			
			
			// seleccionamos los vencimientos que tienen estatus de 9 y que corresponde al credito
				$sqlV9 = "SELECT * FROM vencimiento WHERE credito_id = $x_credito_id AND vencimiento_status_id IN (1,9,3) "; 
			...
			}// la garatia aun esta vigente, no se ha aplicado a comision
	...
	$sSqlWrk = "SELECT solicitud_id, credito_tipo_id, credito_num  FROM credito where credito_id = $x_credito_id";
	$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
	$rowwrk = phpmkr_fetch_array($rswrk);
...
echo "eamail ".$email ."<br>";
		 ...
		 if(!empty($x_no_celular) &&  $x_no_celular != "5555555555" && $x_no_celular != "0000000000"){
			 ...
			 $x_fecha_de_pago = ConvertDateToMysqlFormat($GLOBALS["x_fecha_pago"]);
						...
						$x_fecha = date("Y-m-d");
	 $sqlInsertsms =  "INSERT INTO `sms_enviados` (`id_sms_enviado`, `id_tipo_mensaje`, `contenido`, `no_credito`, `no_celular`, `fecha_registro`, `tipo_envio`, `destino`, `cliente_id`) VALUES (NULL, '2','" .$x_mensaje."', $x_credito_num, '".$x_no_celular."', '".$x_fecha."', '1', '1',$x_cliente_idl)";
...
//    echo "Esta direcci�n de correo ($email_a) es v�lida.";
mail($email, $titulo2, $mensajeMail, $cabeceras2);	
$sqlInsertsms =  "INSERT INTO `sms_mail_enviados` (`id_sms_mail_enviado`, `id_tipo_mensaje`, `contenido`, `no_credito`, `email`, `fecha_registro`, `tipo_envio`, `destino` , `cliente_id`) VALUES (NULL, '2','" .$mensajeMail."', $x_credito_num, '".$email."', '".$x_fecha."', '1', '1',$x_cliente_idl)";
$rsInsert = phpmkr_query($sqlInsertsms, $conn) or die ("Error al inserta en sms tabla nueva". phpmkr_error()."sql :". $sqlInsertsms);
	...
	//GENERAMOS LA ALERTA PLD PARA UN CASO DONDE EL PAGO SEA MAYOR A 500 USD Y SE TRATE DE UN PAGO EN EFECTIVO
	if($GLOBALS["x_medio_pago_id"] == 4){
		// buscamos el ultimo tipo de cambio 
		...
		$x_monto_genera_alerta_1 = $tipoCambio * 500 ;
		$x_importe_pago = $GLOBALS["x_importe"];		
		$x_cliente_id = $x_cliente_idl;
		
		 if(($x_importe_pago >= $x_monto_genera_alerta_1) ){
       // ha pagado mas de lo del monto para generar la alerta; se registra la alerta 
       			$x_today = date("Y-m-d");
					
				// se registra la alerta
				$_SESSION["ewmsg"] .= '<BR><strong>SE REGISTRO UNA ALERTA PLD PARA ESTE CREDITO, CAUSA: PAGO MAYOR A 500 USD EN EFECTIVO </strong><br>';
				$sSql = "INSERT INTO `reporte_cnbv` (`reporte_cnbv_id`, `cliente_id`, `solicitud_id`, `tipo_reporte_id`, `fecha_registro`, `razon_reporte`, `monto`)";
				
				 $sSql .= "VALUES (NULL,".$x_cliente_id.",".$x_solicitud_id.", '2', '".$x_today."', 'PAGO MAYOR A 500 USD ',".$x_importe_pago.");";
				 $rs = phpmkr_query($sSql,$conn) or die("Error al seleccionar los datos de la solicitud ".phpmkr_error()."sql :".$sSql);
						...
						$cabeceras = 'From: zortiz@createc.mx';	
						$x_mensaje	= 'SE REGISTRO UNA ALERTA PLD PARA ESTA SOLICITUD, CAUSA: PAGO MAYOR A 500 USD EN EFECTIVO SOLICITUD_ID = '.$x_solicitud_id;				
						$mensajeMail = $x_mensaje."\n \n * Este mensaje ha sido enviado de forma automatica, por favor no lo responda. \n \n";			 
						// Mail it						
		...
		$rowfvr2 = phpmkr_fetch_array($responseFVR2);
		$x_credito_status_id = $rowfvr2["credito_status_id"];
		// si se liquido el credito verificacmos si todos los pagos son de la misma fecha, si es asi se genera la alerta
		$x_fecha_a_bucar = $GLOBALS["x_fecha_pago"] ;
		$x_misma_fecha = true;
		if($x_credito_status_id ==3){
			...
			$rsVen_rec=  phpmkr_query($sqlVencimientos,$conn)or die("Error al buscar todos los PAGOS del credito liquidado".$sqlVencimientos );
