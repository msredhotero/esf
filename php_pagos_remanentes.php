...
$sSql .= "  WHERE fecha_vencimiento > \"2013-02-01\" and vencimiento_status_id=2  and vencimiento_num> 900 and vencimiento_num<2000 order by credito_id desc ";
?>
...
<table width="785" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
	  ...
	  <td>&nbsp;</td>
	  <td><span class="phpmaker"><a href="php_condonacionlist.php?cmd=reset">Mostrar Todos</a>&nbsp;&nbsp; </span></td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>&nbsp;</td>
...
while (($row = @mysql_fetch_assoc($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount % 2 <> 0) {
		...
		//seleccionamos los vencimietos
		$sqlVencimiento = "SELECT * FROM  recibo_vencimiento WHERE vencimiento_id =  $vencimiento_id ";
		$rsVencimiento =  phpmkr_query($sqlVencimiento,$conn) or die("1Error al seleccionar lso vencimientos".phpmkr_error()."");
		$rowVencimiento = phpmkr_fetch_array($rsVencimiento);
		$x_recibo_id =  $rowVencimiento["recibo_id"];
		$sqlVencimiento = "SELECT * FROM  recibo WHERE recibo_id =  $x_recibo_id ";
		$rsVencimiento =  phpmkr_query($sqlVencimiento,$conn) or die("2Error al seleccionar lso vencimientos".phpmkr_error()."");
		$rowVencimiento = phpmkr_fetch_array($rsVencimiento);
		$x_recibo_status_id =  $rowVencimiento["recibo_status_id"];
		$x_importe_c =  $rowVencimiento["importe"];
		$x_fecha_pago_c =  $rowVencimiento["fecha_pago"];
		
		$sqlVencimiento = "SELECT credito_num FROM  credito WHERE credito_id =  $credito_id ";
		$rsVencimiento =  phpmkr_query($sqlVencimiento,$conn) or die("3Error al seleccionar lso vencimientos".phpmkr_error()."");
		$rowVencimiento = phpmkr_fetch_array($rsVencimiento);
		$x_credito_num =  $rowVencimiento["credito_num"];
...
<td><span>
  <?php echo $vencimiento_num; ?>
</span></td>
<td><span>
  <?php echo $vencimiento_status_id; ?>
</span></td>
<td><span>
...
<td><span>
  <?php echo $fecha_genera_remanente; ?>
</span></td>
<td><span>
  <?php echo $x_recibo_id.".."; ?>
</span></td>
<td><span>
  <?php echo $x_fecha_pago_c; ?>
</span></td>
<td><span>
  <?php echo $x_recibo_status_id; ?>
</span></td>
<td><span>
  <?php echo $x_importe_c; ?>
</span></td>
</tr>
<?php
	}
