...
switch ($sAction)
{
	...
	case "A": // Add
		if (AddData($conn)) { // Add New Record
			$_SESSION["ewmsg"] = "La empresa ha sido registrada.";
			phpmkr_db_close($conn);
			ob_end_clean();
...
<form name="empresaAdd" id="empresaAdd" action="" method="post" >
<p>
...
<table width="700" border="0" cellspacing="2" cellpadding="1" align="center">
  ...
  <tr>
    ...
    <td><?php
		...
		?></td>
    <td colspan="3" class="texto_normal"><div id="txtHint1" class="texto_normal"></div> </td>
    </tr>
  <tr>
 ...
 <tr>
  ...
  <tr>
    ...
    <tr>
      ...
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td><input name="Action" type="button"  value="Agregar Empresa" onClick="EW_checkMyForm();" /></td>
      <td>&nbsp;</td>
    </tr>
  </table>
...
// Field nombre
	...
	$fieldList["`empresa_id`"] = $x_empresa_id;
	...
	// insert into database
	$strsql = "INSERT INTO `representante_legal` (";
	$strsql .= implode(",", array_keys($fieldList));
	$strsql .= ") VALUES (";
	$strsql .= implode(",", array_values($fieldList));
	$strsql .= ")";
