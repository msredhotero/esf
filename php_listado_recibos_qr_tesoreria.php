...
$sSql = "SELECT recibo_qr.*,sucursal.nombre as suc FROM `recibo_qr` join promotor on promotor.promotor_id = recibo_qr.promotor_id  join sucursal on promotor.sucursal_id = sucursal.sucursal_id where ";
$sSql .= "  recibo_qr.fecha >= \"$x_fecha_inicio\" and recibo_qr.fecha <= \"$x_fecha_fin\"  and recibo_qr.promotor_id in ($x_promotores) order by promotor.sucursal_id,fecha  asc";
		 
	 }else{

// Build SQL
$sSql = "SELECT recibo_qr.*,sucursal.nombre as suc FROM `recibo_qr` join promotor on promotor.promotor_id = recibo_qr.promotor_id  join sucursal on promotor.sucursal_id = sucursal.sucursal_id where ";
$sSql .= "  recibo_qr.fecha >= \"$x_fecha_inicio\" and recibo_qr.fecha <= \"$x_fecha_fin\" order by promotor.sucursal_id,fecha  asc";
	 }
//echo $sSql;
// Load Default Filter
$sDefaultFilter = "";
...
?>
<p><span class="phpmaker">Listado de recibo de pago QR 
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_listado_recibos_qr_tesoreria.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
...
<td width="61">Folio QR</td>
<td width="56">Sucursal</td>
		<td width="122" valign="top"><span>
  Fecha registro</span></td>
		<td width="97" valign="top"><span>
  Monto</span></td>
		<td width="68" valign="top">Cheque</td>
		<td width="178" valign="top">Banco </td>
		<td width="282" valign="top">Nombre</td>
        <td width="49" valign="top">Credito</td>
        <td width="47" valign="top">Cliente</td>
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount % 2 <> 0) {
		...
		$x_cliente = $row["cliente"];
		$x_folio = $row["folio"];
	//	echo "cliente ".$x_cliente;
		$x_cliente_num = $row["cliente_num"];
		$x_sucursal_id = $row["sucursal_id"];
		$x_sucursal = $row["suc"];
		$x_monto = $row["monto"];
		$x_fecha = $row["fecha"];
		$x_credito_num = $row["credito"];
				
		//sql cliente 
		
		$sqCliente = "SELECT * FROM credito join banco on banco.banco_id = credito.banco_id where cliente_num = $x_cliente_num ";	
		$rsC = phpmkr_query($sqCliente,$conn) or die("rrr".phpmkr_error().$sqCliente);
		$rosC = phpmkr_fetch_array($rsC);	
		$x_cheque = $rosC["referencia_pago"];
		$x_banco = $rosC["nombre"];
		if(empty($x_cheque)){
			$sqCliente = "SELECT * FROM credito join banco on banco.banco_id = credito.banco_id where credito_num = $x_credito_num ";	
		$rsC = phpmkr_query($sqCliente,$conn) or die("rrr".phpmkr_error().$sqCliente);
		$rosC = phpmkr_fetch_array($rsC);	
		$x_cheque = $rosC["referencia_pago"];
		$x_banco = $rosC["nombre"];
...
<td align="left"><?php echo $x_folio; ?></td>
<td align="left"><span>
<?php echo $x_sucursal; ?>
</span></td>
		<!-- sucursal_id -->
...
<?php echo $x_fecha; ?>
		...
		<!-- nombre -->
		<td align="left"><?php echo $x_monto; ?></td>
		<!-- entidad_id -->
		<td align="left"><span>
...
<?php echo $x_banco; ?>
		...
		<!-- colonia -->
		<td><span><?php echo $x_cliente; ?></span></td>
        <td><span><?php echo $x_credito_num; ?></span></td>
        <td><span><?php echo $x_cliente_num; ?></span></td>
