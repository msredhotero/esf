...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=cuadro_dias_sin_pagar.xls');
}
?>
<?php include ("db.php") ?>
...
?>
  &nbsp;&nbsp;<a href="php_cuadro_dias_sin_pagar.php?export=excel&x_credito_status=<?php echo $x_credito_status; ?>&x_sucursal_srch=<?php echo $_SESSION["x_sucursal_srch"]; ?>&x_promo_srch=<?php echo $_SESSION["x_promo_srch"];?>&x_gestor_srch=<?php echo $x_gestor_srch;?>">Exportar a Excel</a>
  <?php } ?>
    <?php } ?>
</span></p>
  ...
  <tr>
    ...
    <option value="0">Seleccionar</option>    
      ...
      <option value="7188" <?php if ($x_gestor_srch == 7188){?> selected="selected"<?php }?>>Cesar Olvera</option>
       <option value="7202" <?php if ($x_gestor_srch == 7201){?> selected="selected"<?php }?>>Jose Trejo</option>
       <option value="7203" <?php if ($x_gestor_srch == 7203){?> selected="selected"<?php }?>>Cristian catellon</option>
      </select>-->
     <select name="x_gestor_srch" >  
    <option value="0">Seleccionar</option>    
      <option value="7184" <?php if ($x_gestor_srch == 7184){?> selected="selected"<?php }?>>Angelica Tabares</option>
      ...
      <option value="7202" <?php if ($x_gestor_srch == 7201){?> selected="selected"<?php }?>>Jose Trejo</option>
      <option value="7203" <?php if ($x_gestor_srch == 7203){?> selected="selected"<?php }?>>Cristian catellon</option>
       <option value="6824" <?php if ($x_gestor_srch == 6824){?> selected="selected"<?php }?>>(RES SUC)Lourdes Alejandra</option>
      <option value="6823" <?php if ($x_gestor_srch == 6823){?> selected="selected"<?php }?>>(RES SUC)Marysi Pineda</option>
          <option value="6822" <?php if ($x_gestor_srch == 6822){?> selected="selected"<?php }?>>(RES SUC)Daniela Cruz</option>
           <option value="6812" <?php if ($x_gestor_srch == 6812){?> selected="selected"<?php }?>>(RES SUC)Maria Victoria</option>
            <option value="6937" <?php if ($x_gestor_srch == 6937){?> selected="selected"<?php }?>>(RES SUC)Frances Delarrue</option>
...
$x_rango_ven_0 = 0;
$x_rango_vig_0 = 0;
$sSql = " select credito.credito_id from credito where credito_status_id in ($x_credito_status) and credito_id not in (15)  order by credito.credito_id ";
...
while ($row = @phpmkr_fetch_array($rs)){
	...
	$x_numero_vencimientos_vencidos= 0;
	// buscamos si tiene algun vencido; si tiene algun vencido entra en el proceso,sino se saca del proceso
	$sqlVencidos = "SELECT count(*) as vencidos FROM vencimiento WHERE credito_id = $x_credito_id and vencimiento_status_id = (3) ";
	$rsVencidos = phpmkr_query($sqlVencidos,$conn) or die ("Error al seleccionar los vencidos ".phpmkr_error()."sql :".$sqlVencidos);
	$rowVencidos = phpmkr_fetch_array($rsVencidos);
	$x_numero_vencimientos_vencidos = $rowVencidos["vencidos"]+0;
	
#echo "VENC VENCIDOS ".$x_numero_vencimientos_vencidos;
#echo "sql  vencidos".$sqlVencidos."<br>";
	

#if($x_numero_vencimientos_vencidos > 0 ){
	#echo " ENTRA VENCIDOS MAYOR QUE 0 <BR>";
	...
	#echo "CREDITO NUMERO ".$x_credito_num."ID .........".$x_credito_id."<BR>";
...
";
//echo "sql fecha".$sSql;
// la fecha que debemos buscar es la fecha del ultimo pago que se regristro de ese cliente
// lo tomamos por la fecha de movimiento

$sSql = "SELECT fecha_pago FROM recibo JOIN recibo_vencimiento on recibo_vencimiento.recibo_id = recibo.recibo_id join vencimiento on vencimiento.vencimiento_id = recibo_vencimiento.vencimiento_id WHERE vencimiento.credito_id = $x_credito_id order by recibo.recibo_id DESC LIMIT 0,1 ";
...
//echo "<br>sql ".$sSql."<br>";
	...
	$x_fecha_primer_vencido = $rowwrk["fecha_pago"];
	phpmkr_free_result($rswrk);	
//echo "fecha ULTIMO PAGO ".$x_fecha_primer_vencido;

	if(empty($x_fecha_primer_vencido)){
		// buscamos la fecha del primer vencimiento
		$sqlPrimervencimiento = "SELECT fecha_vencimiento FROM vencimiento where credito_id = $x_credito_id order by vencimiento_num asc LIMIT 0,1 ";
		$rsPrimer = phpmkr_query($sqlPrimervencimiento,$conn) or die ("Error al seleccionar los venc".phpmkr_error()."sql:".$sqlPrimervencimiento);
		$rowPrimer = phpmkr_fetch_array($rsPrimer);
		$x_fecha_v =  $rowPrimer["fecha_vencimiento"];
		$x_fecha_primer_vencido = $x_fecha_v;
		}
