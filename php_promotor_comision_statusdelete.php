...
if (!empty($x_promotor_comision_status_id)) {
	if ($sKey <> "") { $sKey .= ","; }
	$sKey .= $x_promotor_comision_status_id;
}else{
	$bSingleDelete = false;
}
if (!$bSingleDelete) {
...
while ($i < count($arRecKey)){
	...
	// Build the SQL
	$sDbWhere .= "`promotor_comision_status_id`=" . $sRecKey . " AND ";
...
<p><span class="phpmaker">Delete from TABLE: Status de Comisiones<br><br><a href="php_promotor_comision_statuslist.php">Back to List</a></span></p>
<form action="php_promotor_comision_statusdelete.php" method="post">
<p>
...
while ($i < count($arRecKey)) {
	...
	$x_promotor_comision_status_id = $sRecKey;
	if (LoadData($conn)) {
?>
...
{
	global $x_promotor_comision_status_id;
	$sSql = "Delete FROM `promotor_comision_status`";
	$sSql .= " WHERE " . $sqlKey;
