...
where credito.credito_status_id in (1,4) and vencimiento.fecha_vencimiento < '$currdate' and vencimiento.vencimiento_status_id in (1,3,6) and credito.fecha_otrogamiento >= \"2012-06-01\" and credito.credito_id not in (6573) order by vencimiento.fecha_vencimiento ";
...
//seleccionamo la fecha de otorgamiento del credito
	...
	if($x_caso_abierto == 0){		
      ...
      //$x_responsable_susursal_usuario_id_ext = $x_responsable_susursal_usuario_id;
		...
		$x_ya_no_se_usa_el_ciclo == 0; // ya no entra; si ya se genero una tarea se debe pasar a la segunda tarea de manera manual;
		if($x_ya_no_se_usa_el_ciclo == 1){
	...
	$sqlTreaHoy = "SELECT COUNT(*) AS tarea_asignada_hoy FROM crm_tarea WHERE crm_caso_id = $x_crm_caso_id AND crm_tarea_status_id in  (1,2)";
...
$x_comision_generada = 0;
	...
	if(($x_carta_1 == 0) && ($x_carta_2 == 0) && ($x_carta_3 == 0) && ($x_carta_D == 0) && ($x_entra_a_carta_1 == 0)){
		// se cicla en fase uno se generaran promesas de pago hasta que beto pase la tarea a carta 1
		...
		// ya se le generaron PP
		//No puede vanza a acarta 1 hasta que cobranza temprana lo mande
		// ya tiene una o mas promesas de pago...
				#verificamos que no tenga ninguna PP con status de pendiente;
		echo "entra  ciclo PP <br>";
		
		$x_PP = 0; //pendiente,vencida o completa O CERRADA
		$sqlPP = "SELECT COUNT(*) AS PP FROM crm_tarea WHERE crm_caso_id = $x_crm_caso_id AND crm_tarea_tipo_id = 5 AND orden= 1 AND crm_tarea_status_id in (1,2)";
		$rsPP = phpmkr_query($sqlPP ,$conn)or die ("Error al seleccionar la carta 1".phpmkr_error()."sql:".$sqlPP );
		$rowPP = phpmkr_fetch_array($rsPP);
		$x_PP = $rowPP["PP"] +0;
		echo "busca PP pendiente".$sqlPP ."<br><br>";
		
		// contamos si ya tiene una PP pero esta vencida; si es asi... buscamos su promesa de pago 
		$x_PP_vencida = 0;
		$sqlPP_vencida = "SELECT COUNT(*) AS PP_v FROM crm_tarea WHERE crm_caso_id = $x_crm_caso_id AND crm_tarea_tipo_id = 5 AND orden= 5 AND crm_tarea_status_id in (2,3)";
		$rsPP_vencida = phpmkr_query($sqlPP_vencida,$conn)or die ("Error al seleccionar la carta 1".phpmkr_error()."sql:".$sqlPP_vencida);
		$rowPP_vencida = phpmkr_fetch_array($rsPP_vencida);
		$x_PP_vencida = $rowPP_vencida["PP_v"] +0;
		echo "busca PP vencidas,o cerradas".$sqlPP ."<br><br>";
		...
		$sqlFPP .= " SELECT crm_tarea_id FROM crm_tarea WHERE crm_caso_id = $x_crm_caso_id AND crm_tarea_tipo_id = 5 AND orden= 1 AND crm_tarea_status_id in (3) ORDER BY crm_tarea_id DESC ) ";
		...
		echo "hoy menos dos dias para validar que se realiza el pago ".$x_hoy_menos_dos_dias;
		...
		echo "<br><br> ===========================DIAS DE ESPERA".$x_dias_espera."<BR>";
		...
		echo "fecha vencimeinto".$fecha_venc."<br>";
	...
	#	 echo "sql promotor".$sqlPromotor."<br>";
		...
		$x_usuario_id = $x_user_CT; // la tarea se le genera al  reponsable de cobranza temprana;
		@phpmkr_free_result($rswrk);
	...
	if(($x_carta_1 > 0) &&($x_comision_generada < 1)){
		...
		$sSqlWrk = "
		SELECT cliente.cliente_id, solicitud.solicitud_id
		FROM 
			cliente join solicitud_cliente on solicitud_cliente.cliente_id = cliente.cliente_id join solicitud on solicitud.solicitud_id = solicitud_cliente.solicitud_id join credito on credito.solicitud_id = solicitud.solicitud_id
		WHERE 
			credito.credito_id = $x_credito_id
		...
		if($x_promotor_id_c > 0){
			...
			if($x_sucursal_id_c > 0){
				...
				echo "usuario responsable suc.---".$x_responsable_susursal_usuario_id."-----<br> sqlResponsable".$sqlResponsable;	
...
while($rowCredito = phpmkr_fetch_array($rsCredito)){
	...
	$x_vencimiento_num = $rowCredito["vencimiento_num"];
		...
		$x_no_pendientes = $rowCount["pendientes"];
...
$x_manda_tarea = 0;
		...
		LIMIT 1
		";
		// latarea se manda a beto ahora
		// ausuario id = 7013 cobranza temprana
		
		$x_responsable_susursal_usuario_id =  7013;
...
echo "sql: ".$sSql."<br>";
			...
			// si no hay un caso abierto lo tengo que abrir y registrar la tarea.
		...
		$x_usuario_id = 7013;
			...
			$sSql = "INSERT INTO crm_caso values (0,3,1,1,$x_cliente_id,'".$currdate."',$x_origen,$x_usuario_id,'$x_bitacora','".$currdate."',NULL,$x_credito_id_a)";	
		$x_result = phpmkr_query($sSql, $conn) or die ("error al insertar carta 1". phpmkr_error()."sql.".$sSql);
		echo "INSERT  :<BR>".$sSql."<BR>";
		$x_crm_caso_id_a = mysql_insert_id();	
			
			
			if(!empty($x_crm_caso_id_a)){			
			$sSql = "INSERT INTO crm_tarea values (0,$x_crm_caso_id_a, $x_orden, $x_tarea_tipo_id, $x_prioridad_id,'".$currdate."', '$currtime','$fecha_venc',NULL,NULL,NULL, 1, 1, 2, $x_usuario_id , NULL,NULL, '$x_asunto','$x_descripcion',1)";	
//	$rsU = phpmkr_query($sSql,$conn)or die("error al inserta tarea recordatorio de pago de penalizaciones".phpmkr_error()." sql".$sSql);
$x_result = phpmkr_query($sSql, $conn) or die ("error al insertar Recordatorio Penalizaciones". phpmkr_error()."sql.".$sSql);
echo "sql: ".$sSql."<br>";
	}
