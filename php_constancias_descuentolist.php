...
?>
<p><span class="phpmaker">Descuentos Fashion Price
  <?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_constancias_descuentolist.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="php_constancias_descuentolist.php?export=word">Export to Word</a>
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
<p>
...
<table width="785" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
	  ...
	  <td><span class="phpmaker">
	    <?php
		$x_estado_civil_idList = "<select name=\"x_tipo_id\" class=\"texto_normal\">";
		...
		$sSqlWrk = "SELECT `constancia_descuento_tipo_id`, `descripcion` FROM `constancia_descuento_tipo`";
		$rswrk = phpmkr_query($sSqlWrk,$conn) or die("Failed to execute query" . phpmkr_error() . ' SQL:' . $sSqlWrk);
		if ($rswrk) {
			$rowcntwrk = 0;
			while ($datawrk = phpmkr_fetch_array($rswrk)) {
				$x_estado_civil_idList .= "<option value=\"" . htmlspecialchars($datawrk[0]) . "\"";
				if ($datawrk["constancia_descuento_tipo_id"] == $filter["x_tipo_id"]) {
...
while (($row = @mysql_fetch_assoc($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount % 2 <> 0) {
		...
		foreach($row as $nombre => $valor){
			...
			$x_vendedor = "";
		$sqlVendedor = "SELECT nombre_completo FROM vendedor_fashion_price where vendedor_fashion_price_id = $vendedor_id ";
		$rsVendedor = phpmkr_query($sqlVendedor,$conn) or die("Erro");
		$rowVendedor = phpmkr_fetch_array($rsVendedor);	
		$x_vendedor = $rowVendedor["nombre_completo"];
		$vendedor_id = "";
		
		$x_tipo =  "";
		$sqlVendedor = "SELECT descripcion FROM constancia_descuento_tipo where constancia_descuento_tipo_id = $tipo_constancia_id ";
		$rsVendedor = phpmkr_query($sqlVendedor,$conn) or die("Erro");
		$rowVendedor = phpmkr_fetch_array($rsVendedor);	
		$x_tipo = $rowVendedor["descripcion"];
		$tipo_constancia_id = "";
...
<?php } ?>
		...
		<td><span>
<?php echo $constancia_descuento_id; ?>
</span></td>
<td><span>
<?php echo $folio; ?>
</span></td>
