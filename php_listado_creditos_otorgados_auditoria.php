...
// Build SQL
$sSql = "SELECT solicitud.*,sucursal.nombre as suc, credito.credito_id, credito.fecha_otrogamiento,credito.referencia_pago, credito.credito_num FROM `solicitud` join promotor on promotor.promotor_id = solicitud.promotor_id  join sucursal on promotor.sucursal_id = sucursal.sucursal_id join credito on credito.solicitud_id = solicitud.solicitud_id where ";
$sSql .= "  credito.fecha_otrogamiento >= \"$x_fecha_inicio\" and credito.fecha_otrogamiento <= \"$x_fecha_fin\" and solicitud.promotor_id in ($x_promotores) order by promotor.sucursal_id,credito.fecha_otrogamiento  asc";
...
// Build SQL
$sSql = "SELECT solicitud.*,sucursal.nombre as suc, credito.credito_id, credito.fecha_otrogamiento,credito.referencia_pago, credito.credito_num FROM `solicitud` join promotor on promotor.promotor_id = solicitud.promotor_id  join sucursal on promotor.sucursal_id = sucursal.sucursal_id join credito on credito.solicitud_id = solicitud.solicitud_id where ";
$sSql .= "  credito.fecha_otrogamiento >= \"$x_fecha_inicio\" and credito.fecha_otrogamiento <= \"$x_fecha_fin\" order by promotor.sucursal_id,credito.fecha_otrogamiento  asc";
...
?>
<p><span class="phpmaker">Listado de cr&eacute;ditos otorgados 
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_listado_creditos_otorgados_auditoria.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;
<?php } ?>
</span></p>
<?php if ($sExport == "") { ?>
...
<td width="66">CR&Eacute;DITO</td>
<td width="104">sucursal</td>
		<td width="109" valign="top"><span>
  F. otorgamiento</span></td>
		<td width="235" valign="top"><span>
  Nombre
</span></td>
		<td width="112" valign="top">Cheque</td>
        <td width="112" valign="top">Garant&iacute;a</td>
        <td width="113" valign="top">Pagar&eacute;</td>
		<td width="108" valign="top">Contrato</td>
		<td width="117" valign="top">Poliza de seguro</td>
...
while (($row = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount % 2 <> 0) {
		...
		$sqlgarantia = "SELECT garantia_liquida FROM credito WHERE credito_id = $x_credito_id ";
		$rsGarantia = phpmkr_query($sqlgarantia,$conn)or die ("Error la seleccionar el query".phpmkr_error().$sqlgarantia);
		$rowGra = phpmkr_fetch_array($rsGarantia);
		$x_garantia = $rowGra["garantia_liquida"];
		if($x_garantia == "1"){
			$x_gar = "Si";
			}else{
				$x_gar = "--------------";
		...
		$x_aval = "";
		$sqlA = "SELEcT * FROM datos_aval WHERE cliente_id = $x_cliente_id";
...
<td align="left"><?php echo $x_credito_num; ?></td>
<td align="left"><span>
<?php echo $x_sucursal; ?>
</span></td>
		<!-- sucursal_id -->
		<td align="left"><span>
<?php echo $x_fecha_otorgamiento; ?>
</span></td>
		<!-- nombre -->
		<td align="left"><span>
<?php 
...
?>
		...
		<!-- entidad_id -->
		<td align="left"><?php echo $x_referencia_pago;?></td>
<td align="left"><?php echo $x_gar; ?></td>
<!-- calle -->
		<td> </td>
		<!-- calle -->
		<td>&nbsp;</td>
