...
$sqlObservaciones =  "Select * from comite_credito where comite_credito_id = ".$_GET["key"];
$rsComite = phpmkr_query($sqlObservaciones,$conn)or die (phpmkr_error());
$rowComite =  phpmkr_fetch_array($rsComite);
$x_observaciones_generales =  $rowComite["observaciones_generales"];
$x_fecha = $rowComite["fecha"];
?>

<table >
<tr >

<td colspan="6" width="100%" height="45" style="background-image:url(images/images/t2.jpg); background-repeat:repeat-x; color:#FFFFFF"><h4><center><strong>COMIT&Eacute; DE CR&Eacute;DITO<br /><?php echo $x_fecha;?></strong></center></h4>
</td>
</tr>
...
<?php
$x_no = 1;
//echo "llave ".$_GET["key"]."<br>";

$sqlSol =  "SELECT * FROM comite_credito_solicitud WHERE comite_credito_id = ".$_GET["key"];
...
while($rowSol = phpmkr_fetch_array($rsSol)){
	...
	$x_numero_cliente = $rowSol["no_cliente"];	
	$x_nombre_cliente = $rowSol["nombre_cliente"];
	$x_cantidad_solicitada = $rowSol["cantidad_solicitadad"];	
	$x_pld = $rowSol["pld"];
	$x_documentos = $rowSol["documentos"];	
	$x_cantidad_otorgada = $rowSol["cantidad_otorgada"];	
	$x_status = $rowSol["status"];
	$x_observaciones = $rowSol["observaciones"];	
	if($x_pld == 1){
		$x_sel_1 = "selected=\"selected\"";
		}else{
			$x_sel_2 = "selected=\"selected\"";
			}
			
	if($x_status+0 == 1){
		$x_sel_1_s = "selected=\"selected\"";
		}else{
			$x_sel_2_s = "selected=\"selected\"";
			}		
	//echo "status".$x_status. "--sel 1".$x_sel_1_s."sele dos --".$x_sel_2_s;
	...
	$x_registro =  "<tr id=\"x_registro_".$x_no."\" class=\"x_comite_solicitud\">					
					...
					<td><select   name=\"x_pld_".$x_no."\" id=\"x_pld_".$x_no."\"><option value=\"\">Seleccione</option><option value=\"1\" ". $x_sel_1." > Si </option><option value=\"2\"". $x_sel_2." > No </option></select></td>
					<td><input type=\"text\" class=\"validate[required]\"  name=\"x_documentos_".$x_no."\" id=\"x_documentos_".$x_no."\" value=\"$x_documentos\" /></td>
					<td><input type=\"text\" class=\"validate[required]\"  name=\"x_cantidad_otorgada_".$x_no."\"  id=\"x_cantidad_otorgada_".$x_no."\" value=\"$x_cantidad_otorgada\" /> </td>
					<td><select name=\"x_status_".$x_no."\"  id=\"x_status_".$x_no."\">
					<option value=\"\">Seleccione</option><option value=\"1\"". $x_sel_1_s."> Aprobada </option><option value=\"2\"". $x_sel_2_s."> Rechazada </option></select></td>
					<td><textarea class=\"validate[required]\"  name=\"x_observaciones_".$x_no."\" id=\"x_observaciones_".$x_no."\" cols=\"30\" rows=\"1\">$x_observaciones </textarea></td>
					<td>&nbsp;</td>
					</tr>					
					";
...
<td colspan="5"><textarea name="x_observaciones_generales" cols="100" id="x_observaciones_generales"><?php echo $x_observaciones_generales ?></textarea></td>
...
<?php
$sqlIntegrantes = "	SELECT * FROM comite_credito_participantes_activos, comite_credito_participantes_lista where comite_credito_participantes_activos.comite_credito_participante_id = comite_credito_participantes_lista.comite_credito_participantes_lista_id AND comite_credito_id   = ".$_GET["key"];
...
$x_integrante_id = $rowIntegrantes["comite_credito_participantes_lista_id"];
	//AQUI los <TD>
	$celdacontenido = "<td><input type=\"checkbox\"  checked=\"checked\" class=\"x_participantes_comite\"   name=\"x_participante\" id=\"x_participante\" value=\"$x_integrante_id\" />&nbsp;$x_nombre</td><td></td><td>&nbsp;</td>";
...
$x_compara = 1;
 ...
 $x_inicio_tabla .= "<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td></td></tr>";
...
<tr ><td colspan="6" width="100%" height="45" align="right"><?php echo $x_fecha;?></td>
</tr>
</table>
