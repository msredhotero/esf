...
// Build SQL
$sSql = "select vencimiento.credito_id from vencimiento join credito on credito.credito_id = vencimiento.credito_id join solicitud on solicitud.solicitud_id = credito.solicitud_id JOIN fondeo_colocacion ON fondeo_colocacion.credito_id = credito.credito_id  where (fondeo_colocacion.fondeo_credito_id = 7) AND (credito.credito_status_id = 4) AND (vencimiento.vencimiento_status_id = 3) AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) >= ".$_SESSION["x_dias_ini"].") AND ((TO_DAYS('".ConvertDateToMysqlFormat($currdate)."') - TO_DAYS(vencimiento.fecha_vencimiento)) <= ".$_SESSION["x_dias_fin"].")".$sWhere;
...
<?php if ($sExport == "") { ?>
<form action="php_cartera_vencidalist2_fim.php" name="filtro" id="filtro" method="post">
  <table width="785" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
      ...
      <td>&nbsp;</td>
      <td><span class="phpmaker"><a href="php_cartera_vencidalist2_fim.php?cmd=reset">Mostrar Todos</a>&nbsp;&nbsp; </span></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
