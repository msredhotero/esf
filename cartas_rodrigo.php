...
$sqlCAsos = "SELECT * FROM crm_caso where  crm_caso_status_id = 1 limit 0,97 ";
...
while($rowcasos = phpmkr_fetch_array($rsCaso)){
	...
	while($rowTareas = phpmkr_fetch_array($rsTarea)){
		...
		$x_responsable = $rowTareas["destino"];
		
		$sqlRespon = "select * from usuario where usuario_id = $x_responsable ";
		$rsD = phpmkr_query($sqlRespon,$conn) or die("error ".phpmkr_error()."sql:".$sqlRespon);
		$rowD = phpmkr_fetch_array($rsD);
		$encargado = $rowD["nombre"];
		
		if($x_asunto == "Carta 1"){
			echo "encargado carta 1 ".$encargado."<br>";
	...
	while($rowTareasi = phpmkr_fetch_array($rsTareai)){
		...
		echo "encargado carta  ".$encargado."<br>";
