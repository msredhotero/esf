...
if ($sExport == "excel") {
	...
	header('Content-Disposition: attachment; filename=reporte_cvp.xls');
}
?>
<?php include ("db.php") ?>
...
<p><span class="phpmaker">REPORTE DE CARTERA VIGENTE PROMEDIO
<?php if ($sExport == "") { ?>
&nbsp;&nbsp;<a href="php_rpt_cvp.php?export=excel&x_fecha_desde=<?php echo FormatDateTime(@$x_fecha_desde,7); ?>&x_fecha_hasta=<?php echo FormatDateTime(@$x_fecha_hasta,7); ?>">Exportar a Excel</a><?php } ?>
</span></p>

<?php if ($sExport == "") { ?>
<p><span class="phpmaker">
<form action="php_rpt_cvp.php" name="filtro" id="filtro" method="post">
...
while($x_fecha_actual <= $x_fecha_hasta_sql){
	...
	// Build SQL
	$sSql = "SELECT sum(importe) as capital from credito where fecha_otrogamiento <= '".$x_fecha_actual."'";
...
//	$sSql = "SELECT sum(importe) as pagado from recibo where fecha_pago <= '".$x_fecha_actual."'";
	...
	$x_tot_otorgado_p =  $x_tot_otorgado_p + $x_tot_otorgado;
	$x_tot_pagado_p = $x_tot_pagado_p + $x_tot_pagado;		
	$x_tot_vencido_p = $x_tot_vencido_p + $x_tot_vencido;			
...
$x_tot_cartera_p = ($x_tot_otorgado_p - ($x_tot_pagado_p + $x_tot_vencido_p));

$x_tot_otorgado_p = ($x_tot_otorgado_p / $nRecCount);
$x_tot_pagado_p = ($x_tot_pagado_p / $nRecCount);
$x_tot_vencido_p = ($x_tot_vencido_p / $nRecCount);
$x_tot_cartera_p = ($x_tot_cartera_p / $nRecCount);		
...
<?php echo FormatDateTime($x_fecha_hasta,7); ?>
        ...
        <!-- importe -->
        <td align="right"><span>
<?php echo FormatNumber($x_tot_otorgado_p,2,0,0,1); ?>
</span></td>	
        <!-- importe -->
        <td align="right"><span>
<?php echo FormatNumber($x_tot_pagado_p,2,0,0,1); ?>
</span></td>	
        <td align="right"><span>
<?php echo FormatNumber($x_tot_vencido_p,2,0,0,1); ?>
</span></td>	
        <!-- importe -->
        <td align="right"><span>
<?php echo FormatNumber($x_tot_cartera_p,2,0,0,1); ?>
</span></td>	
    </tr>
</table>
</span></p>
