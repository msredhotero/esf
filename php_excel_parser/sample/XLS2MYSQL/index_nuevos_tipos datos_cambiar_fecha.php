...
$x_mensaje = "cargfa id en POST".$x_carga_id;
...
*/
	...
	if ( empty ($err) ) {
		...
		while ($row = phpmkr_fetch_array($rs)){
			...
			$x_numero_cliente = (string)$x_numero_cliente ;	
			$x_numero_cliente = substr($x_numero_cliente,1);
			
			//var_dump($x_numero_cliente); 
			...
			$x_fecha_c = explode(".",$x_fecha_movimiento);
			$x_mes_c = $x_fecha_c[0];
			$x_dia_c = $x_fecha_c[1];
			$x_anio_c = $x_fecha_c[2];
		...
		if(($x_validado == true) && ($numero_archivos < 1)){	
			...
			La informaci&oacute;n ha sido cargada,  presione el siguiente boton para cerrar esta ventana. $x_carga_id  $x_mensaje <br><br>
			<br>
