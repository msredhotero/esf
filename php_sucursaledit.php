...
if (($sAction == "") || ((is_null($sAction)))) {
	...
	$x_sucursal_dependiente_id = @$_POST["x_sucursal_dependiente_id"];
}
...
<p><span class="phpmaker">Sucursal<br><br>
<a href="php_sucursallist.php">Regresar al listado</a></span></p>
<form name="sucursaledit" id="sucursaledit" action="php_sucursaledit.php" method="post" onSubmit="return EW_checkMyForm(this);">
<p>
...
<?php echo $x_sucursal_id; ?><input type="hidden" name="x_sucursal_id" value="<?php echo $x_sucursal_id; ?>">
</span></td>
	</tr>
	<tr>
...
<input type="text" name="x_contacto_email" id="x_contacto_email" size="30" maxlength="150" value="<?php echo htmlspecialchars(@$x_contacto_email) ?>">
	...
	<tr>
		...
		<td class="ewTableAltRow"><span>
<?php
$x_sucursal_dependiente_idList = "<select name=\"x_sucursal_dependiente_id\">";
...
if ($rswrk) {
	...
	while ($datawrk = phpmkr_fetch_array($rswrk)) {
		...
		if ($datawrk["sucursal_id"] == @$x_sucursal_dependiente_id) {
...
function EditData($sKey,$conn)
{
	...
	if (phpmkr_num_rows($rs) == 0) {
		...
		$theValue = ($GLOBALS["x_sucursal_dependiente_id"] != "") ? intval($GLOBALS["x_sucursal_dependiente_id"]) : "NULL";
		$fieldList["`sucursal_dependiente_id`"] = $theValue;

		// update
		$sSql = "UPDATE `sucursal` SET ";
