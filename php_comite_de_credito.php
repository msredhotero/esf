...
while($rowSol = phpmkr_fetch_array($rsSol)){
	...
	$x_registro =  "<tr id=\"x_registro_".$x_no."\" class=\"x_comite_solicitud\">					
					...
					<td><input type=\"button\"  name=\"x_quitar_registro\"  class=\"x_quita_registro\"  id=\"x_quitar_registro_".$x_no."\" value=\"Quitar de comite\" />
					<input type=\"hidden\"   name=\"x_solicitud_id_".$x_no."\"  id=\"x_solicitud_id_".$x_no."\" value=\"$x_solicitud_id\" />
					</td>
					</tr>					
					";
...
function AddData($conn)
{
	...
	for($i=1; $i<=$x_total_registros; $i++){
		$x_solicitud_id = "x_solicitud_id_".$i;
	...
	$sqlInsertCCS .= " VALUES (NULL ,$x_comite_credito_id,".$$x_solicitud_id.", ".$$x_cliente." ,  \"".$$x_nombre."\", ".$$x_solicitada.", \"".$$x_pld."\",  \"".$$x_doctos."\", ".$$x_otorgada.", ". $$x_status.",  \"". $$x_obs."\") ";
