...
$sSql = "SELECT credito.credito_id  FROM vencimiento, credito  WHERE credito.credito_id = vencimiento.credito_id and vencimiento.fecha_vencimiento >= \"$x_primer_dia_mes\"  and vencimiento.fecha_vencimiento <= \"$x_ultimo_dia_mes\" and credito.credito_id not in(1489)  and credito.credito_status_id = 1 GROUP BY credito_id ";
//$rsCreditosFactura = phpmkr_query($sqlCreditosFactura,$conn)or die ("Error al selccionar los credito de la factura".phpmkr_error()."sql:".$sqlCreditosFactura);
/*while($rowCreditosFactura = phpmkr_fetch_array($rsCreditosFactura)){
	...
	$sqlVencimientosMes = "SELECT * FROM vencimiento WHERE credito_id = $x_credito_id fecha_vencimiento >= \"$x_primer_dia_mes\"  and fecha_vencimiento <= \"$x_ultimo_dia_mes\" order by vencimiento_id ";
	...
	while($rowVencimientosMes = phpmkr_fetch_array($rsVencimientosMes)){
		...
		$x_cliente_id = 0;
		$SQLsOLICITU = "SELECT solicitud_id FROM credito WHERE credito_id = $x_credito_id ";
		...
		$x_fecha_factura = $x_anio.$x_mes.$x_dia;
		$x_serie_docto = "AAAA";
		$x_folio = "=CONCATENAR($x_folio_siguiente_factura,REPETIR(\" \",6-LARGO($x_folio_siguiente_factura)))";
		$x_clave_cliente = "=CONCATENAR($x_cliente_id,REPETIR(\" \",5-LARGO($x_cliente_id)))";
		$x_id_credito = "=CONCATENAR($x_credito_id,REPETIR(\" \",5-LARGO($x_credito_id)))";
...
<?php if ($sExport == "") { ?>
<a href="php_factura_notas_saldos.php?export=excel">Exportar a Excel</a>
<br /><br />
<?php } ?>
...
while (($rowCreditosFactura = @phpmkr_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	while($rowVencimientosMes = phpmkr_fetch_array($rsVencimientosMes)){
		...
		$x_iva_mor_penalizacion = $rowVencimientosMes["iva_mor"];	
		 ...
		 $x_codonacion_ordinario = "CONIORD";
		 $x_Condonacion_mora = "CONIMOR";
