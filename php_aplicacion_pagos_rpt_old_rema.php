			...
			//	echo "dias vencidos :".$x_dias_vencidos."<br>";
				...
				if($x_recibo == 1){
					...
					*/
					...
					//echo "entramos a caso 2 <br> el ".$x_importe."es emnor que el vencmiento".$x_s_importe_total."<br>";
					/*if(){
					$x_por1 = (($x_s_importe * 100) / $x_s_importe_total);
					$x_d_importe = ($x_importe * ($x_por1 / 100));
				
					$x_por2 = (($x_s_interes * 100) / $x_s_importe_total);
					$x_d_interes = ($x_importe * ($x_por2 / 100));
				
					if($x_s_interes_moratorio > 0){
						$x_por3 = (($x_s_interes_moratorio * 100) / $x_s_importe_total);
						$x_d_interes_moratorio = ($x_importe * ($x_por3 / 100));
					}else{
					...
					if($x_s_iva_mor > 0){
						...
						$x_d_iva_mor = ($x_importe * ($x_por6 / 100));
					}else{
						$x_d_iva_mor = 0;
					}
					...
					if($x_d_importe_total > 100){
					$sSql = "insert into vencimiento values(0,$x_s_credito_id, $x_s_vencimiento_num,1, '$x_s_fecha_vencimiento', $x_d_importe, $x_d_interes, $x_d_interes_moratorio, $x_d_iva, $x_d_iva_mor, $x_d_importe_total)";
					$x_result = phpmkr_query($sSql, $conn);
					if(!$x_result){
						echo phpmkr_error() . '<br>SQL: ' . $sSql;
						//phpmkr_query('rollback;', $conn);	 
					...
					}else if($x_d_importe_total < 100){
						//seleccionamos el ultimo vencimiento y  le sumamos la catidad que queda por pagar de este vencimeinto.
						$sqlUV = "SELECT * FROM vencimiento where credito_id = $x_s_credito_id ORDER BY vencimiento_num ORDER BY `vencimiento_num` DESC LIMIT 1 ";
						$responseUV = phpmkr_query($sqlUV,$conn) or die ("error al seleccionar el ultimo vencimiento".phpmkr_error()."sql : ".$sqlUV) ;
						$rowUV = phpmkr_fetch_array($responseUV);
						$x_ultimo_vencimiento_id = $rowUV["vencimiento_id"];
						$x_total_vencimiento_u  = $rowUV["total_venc"]
						phpmkr_free_result($responseUV);
						
						
						$x_total_ven_u =  doubleval($x_total_vencimiento_u + $x_d_importe_total);
						$sqlUUV = "UPDATE VENCIMIENTO SET total_venc = $x_total_ven_u WHERE  vencimiento_id = $x_ultimo_vencimiento_id";
						$responseUUV = phpmkr_query($sqlUUV, $conn) or die ("Error al ejecutar el query".phpmkr_error()."sql : ".$sqlUUV);
						
						}
					}//fin codigo anteriror */
					
					//amortiza mor iva_mor int iva_int cap iva porcentual a int
